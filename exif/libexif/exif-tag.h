...
#define EXIF_TAG_GPS_DIFFERENTIAL 0x001e
    ...
    EXIF_EXPORT ExifTag exif_tag_from_name(const char*);
    EXIF_EXPORT const char* exif_tag_get_name_in_ifd(ExifTag, ExifIfd);
    EXIF_EXPORT const char* exif_tag_get_title_in_ifd(ExifTag, ExifIfd);
    EXIF_EXPORT const char* exif_tag_get_description_in_ifd(ExifTag, ExifIfd);
    EXIF_EXPORT ExifSupportLevel
        exif_tag_get_support_level_in_ifd(ExifTag, ExifIfd, ExifDataType);

    /* Don't use these functions. They are here for compatibility only. */
    EXIF_EXPORT const char* exif_tag_get_name(ExifTag tag);
    EXIF_EXPORT const char* exif_tag_get_title(ExifTag tag);
    EXIF_EXPORT const char* exif_tag_get_description(ExifTag tag);
