...
extern "C"
{
     ...
     */
    EXIF_EXPORT void exif_loader_unref(ExifLoader* loader);

    /*! Write a file to the ExifLoader
     * \param[in] loader the loader
     * \param[in] fname the path to the file to read
     */
    EXIF_EXPORT void exif_loader_write_file(ExifLoader* loader,
                                            const char* fname);
     ...
     * \returns 1 while EXIF data is read (or while there is still hope that
     * there will be EXIF data later on), 0 otherwise.
     */
    EXIF_EXPORT unsigned char
    exif_loader_write(ExifLoader* loader, unsigned char* buf, unsigned int sz);

    /*! Reset the ExifLoader
     * \param[in] loader the loader
     */
    EXIF_EXPORT void exif_loader_reset(ExifLoader* loader);
    /*! Get an ExifData out of an ExifLoader
     * \param[in] loader the loader
     * \returns and allocated ExifData
     */
    EXIF_EXPORT ExifData* exif_loader_get_data(ExifLoader* loader);

    EXIF_EXPORT void exif_loader_log(ExifLoader*, ExifLog*);
