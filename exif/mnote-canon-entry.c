...
static struct canon_entry_table_t
{
    ...
    {{0, 1, N_("Macro")},
     ...
     {21, 0x173,
      N_("Canon EF 180mm Macro f/3.5L or Sigma 180mm EX HSM Macro f/3.5")},
     ...
     {40, 0x0504, N_("1/3")},
     {0, 0, NULL}},
  entries_focal_length[] = {{0, 1, N_("Fixed")},
                            {0, 2, N_("Zoom")},
                            {0, 0, NULL}},
  entries_settings_2[] = {{6, 0, N_("Auto")},
                          {6, 1, N_("Sunny")},
                          {6, 2, N_("Cloudy")},
                          {6, 3, N_("Tungsten")},
                          {6, 4, N_("Fluorescent")},
                          ...
                          {0, 0, NULL}},
  entries_panorama[] = {
      {0, 0, N_("Left to right")},          {0, 1, N_("Right to left")},
      {0, 2, N_("Bottom to top")},          {0, 3, N_("Top to bottom")},
      {0, 4, N_("2x2 matrix (clockwise)")}, {0, 0, NULL}};
                                     ...
                                     unsigned int maxlen)
{
    ...
    for (j = 0; table[j].name
                && ((table[j].subtag < t)
                    || ((table[j].subtag == t) && table[j].value <= vs));
         j++)
    {
 ...
 */
static double apex_value_to_aperture(double x) { return pow(2, x / 2.); }

static double apex_value_to_shutter_speed(double x) { return 1.0 / pow(2, x); }

static double apex_value_to_iso_speed(double x) { return 3.125 * pow(2, x); }

char* mnote_canon_entry_get_value(const MnoteCanonEntry* entry, unsigned int t,
                                  char* val, unsigned int maxlen)
{
    ...
    case MNOTE_CANON_TAG_SETTINGS_1:
        ...
        case 15:
            ...
            else
            {
                ...
                canon_search_table_value(entries_settings_1, t, vs, val,
                                         maxlen);
            }
            break;
        case 22:
    ...
    case MNOTE_CANON_TAG_FIRMWARE:
        CF(entry->format, EXIF_FORMAT_ASCII, val, maxlen);
        /*		CC2 (entry->components, 24, 32, val, maxlen); Can also
         * be 22 */
        strncpy(val, (char*)data, MIN(entry->size, maxlen));
        break;
    ...
    case MNOTE_CANON_TAG_IMAGE_NUMBER:
        ...
        snprintf(val, maxlen, "%03lu-%04lu", (unsigned long)vl / 10000,
