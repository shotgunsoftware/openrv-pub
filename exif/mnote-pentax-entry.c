...
static struct
{
    ...
    {MNOTE_PENTAX_TAG_QUALITY,
     {{0, N_("Good")}, {1, N_("Better")}, {2, N_("Best")}, {0, NULL}}},
    {MNOTE_PENTAX_TAG_FOCUS, {{2, N_("Custom")}, {3, N_("Auto")}, {0, NULL}}},
    ...
    {MNOTE_PENTAX_TAG_SHARPNESS,
     {{0, N_("Normal")}, {1, N_("Soft")}, {2, N_("Hard")}, {0, NULL}}},
    {MNOTE_PENTAX_TAG_CONTRAST,
     {{0, N_("Normal")}, {1, N_("Low")}, {2, N_("High")}, {0, NULL}}},
    ...
    {MNOTE_PENTAX_TAG_COLOR,
     {{1, N_("Full")}, {2, N_("Black & White")}, {3, N_("Sepia")}, {0, NULL}}},
...
} items2[] = {{MNOTE_PENTAX2_TAG_IMAGE_SIZE,
               {{0, 0, "2304x1728"},
                {4, 0, "1600x1200"},
                {5, 0, "2048x1536"},
                {8, 0, "2560x1920"},
                                   ...
                                   unsigned int maxlen)
{
    ...
    case MNOTE_CASIO2_TAG_BESTSHOT_MODE:
        ...
        if (entry->components == 1)
        {
            ...
            /* find the value */
            for (j = 0;
                 items[i].elem[j].string && (items[i].elem[j].index < vs); j++)
                ;
            if (items[i].elem[j].index != vs)
            {
                snprintf(val, maxlen, _("Internal error (unknown value %i)"),
                         vs);
                break;
            }
        ...
        else
        {
            ...
            /* find the value */
            for (j = 0; items2[i].elem[j].string
                        && ((items2[i].elem[j].index2 < vs2)
                            || ((items2[i].elem[j].index2 == vs2)
                                && (items2[i].elem[j].index1 < vs)));
                 j++)
                ;
            if ((items2[i].elem[j].index1 != vs)
                || (items2[i].elem[j].index2 != vs2))
            {
                snprintf(val, maxlen, _("Internal error (unknown value %i %i)"),
                         vs, vs2);
                break;
            }
    ...
    case MNOTE_PENTAX_TAG_PRINTIM:
        ...
        snprintf(val, maxlen, _("%i bytes unknown data"), entry->size);
        break;
