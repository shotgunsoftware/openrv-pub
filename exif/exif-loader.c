...
static unsigned int exif_loader_copy(ExifLoader* eld, unsigned char* buf,
                                     unsigned int len)
{
...
unsigned char exif_loader_write(ExifLoader* eld, unsigned char* buf,
                                unsigned int len)
{
    unsigned int i;
    ...
    for (i = 0; i < sizeof(eld->b); i++)
        ...
        {
        case EL_EXIF_FOUND:
            if (!exif_loader_copy(eld, eld->b + i, sizeof(eld->b) - i))
                return 0;
            return exif_loader_copy(eld, buf, len);
        case EL_SKIP_BYTES:
            eld->size--;
        ...
        default:
            ...
            case JPEG_MARKER_APP1:
                if (!memcmp(eld->b + i + 3, ExifHeader,
                            MIN(sizeof(ExifHeader),
                                MAX(0, sizeof(eld->b) - i - 3))))
                {
                    eld->data_format = EL_DATA_FORMAT_EXIF;
                }
                else
                {
                    eld->data_format =
                        EL_DATA_FORMAT_JPEG; /* Probably JFIF - keep searching
                                                for APP1 EXIF*/
                }
                eld->size = 0;
                eld->state = EL_READ_SIZE_BYTE_08;
            ...
            case JPEG_MARKER_SOI:
                break;
            default:
                exif_log(eld->log, EXIF_LOG_CODE_CORRUPT_DATA, "ExifLoader",
                         _("The data supplied "
...
void exif_loader_reset(ExifLoader* loader)
{
    if (!loader)
        return;
    exif_mem_free(loader->mem, loader->buf);
    loader->buf = NULL;
    loader->size = 0;
    loader->bytes_read = 0;
    loader->state = 0;
