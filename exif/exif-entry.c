...
void exif_entry_fix(ExifEntry* e)
{
            ...
            for (i = 0; i < e->components; i++)
                exif_set_short(
                    ...
                    (ExifShort)exif_get_long(
                        e->data + i * exif_format_get_size(EXIF_FORMAT_LONG),
                        o));
        ...
        case EXIF_FORMAT_SRATIONAL:
            ...
            for (i = 0; i < e->components; i++)
            {
                sr = exif_get_srational(
                    e->data + i * exif_format_get_size(EXIF_FORMAT_SRATIONAL),
                    o);
                r.numerator = (ExifLong)sr.numerator;
                r.denominator = (ExifLong)sr.denominator;
                exif_set_rational(
                    e->data + i * exif_format_get_size(EXIF_FORMAT_RATIONAL), o,
                    r);
            }
    ...
    case EXIF_TAG_USER_COMMENT:
        ...
        for (i = 0; (i < e->size) && !e->data[i]; i++)
            ;
        if (!i)
            for (; (i < e->size) && (e->data[i] == ' '); i++)
                ;
        if ((i >= 8) && (i < e->size))
        {
            exif_entry_log(
                e, EXIF_LOG_CODE_DEBUG,
         ...
         * assume ASCII.
         */
        if (memcmp(e->data, "ASCII\0\0\0", 8) && memcmp(e->data, "UNICODE\0", 8)
...
static struct
{
    ...
    {EXIF_TAG_YCBCR_POSITIONING, {"", N_("centered"), N_("co-sited"), NULL}},
    ...
    {EXIF_TAG_SCENE_CAPTURE_TYPE,
     {N_("Standard"), N_("Landscape"), N_("Portrait"), N_("Night scene"),
      NULL}},
    {EXIF_TAG_GAIN_CONTROL,
     {N_("Normal"), N_("Low gain up"), N_("High gain up"), N_("Low gain down"),
      N_("High gain down"), NULL}},
    {EXIF_TAG_SATURATION,
...
static struct
{
    ...
    {EXIF_TAG_EXPOSURE_PROGRAM,
     {{0, {N_("Not defined"), NULL}},
      ...
      {5,
       {N_("Creative program (biased toward depth of field)"), N_("Creative"),
        NULL}},
      {6,
       {N_("Creative program (biased toward fast shutter speed)"), N_("Action"),
        NULL}},
      {7,
    ...
    {EXIF_TAG_FLASH,
     {{0x0000, {N_("Flash did not fire."), N_("no flash"), NULL}},
      ...
      {0x0005,
       {N_("Strobe return light not detected."), N_("W/o strobe"), NULL}},
      ...
      {0x001d,
       {N_("Flash fired, auto mode, return light not detected."), NULL}},
      ...
      {0x004f,
       {N_("Flash fired, compulsory flash mode, red-eye reduction mode, "
        ...
        NULL}},
      {0x0058,
       {N_("Flash did not fire, auto mode, red-eye reduction mode."), NULL}},
...
const char* exif_entry_get_value(ExifEntry* e, char* val, unsigned int maxlen)
{
    ...
    static struct
    {
        char* label;
        char major, minor;
    } versions[] = {{"0110", 1, 1}, {"0120", 1, 2}, {"0200", 2, 0},
                    {"0210", 2, 1}, {"0220", 2, 2}, {"0221", 2, 21},
                    {NULL, 0, 0}};
    ...
    case EXIF_TAG_USER_COMMENT:
         ...
         */
        if ((e->format != EXIF_FORMAT_ASCII) || (e->size <= 8)
        ...
        for (i = 0; (i < e->size) && (!e->data[i] || (e->data[i] == ' ')); i++)
            ;
        if (i == e->size)
            break;
        ...
        for (; (i < e->size) && (strlen(val) < maxlen - 1); i++)
        {
            ...
            val[strlen(val)] = isprint(e->data[i]) ? e->data[i] : '.';
        }
        break;
    ...
    case EXIF_TAG_EXIF_VERSION:
        ...
        for (i = 0; versions[i].label; i++)
        {
            ...
            {
                snprintf(val, maxlen, _("Exif Version %d.%d"),
    ...
    case EXIF_TAG_FOCAL_LENGTH:
         ...
         */
        d = 0.;
        entry = exif_content_get_entry(e->parent->parent->ifd[EXIF_IFD_0],
                                       EXIF_TAG_MAKE);
        if (entry && entry->data && !strncmp((char*)entry->data, "Minolta", 7))
        {
            entry = exif_content_get_entry(e->parent->parent->ifd[EXIF_IFD_0],
                                           EXIF_TAG_MODEL);
            if (entry && entry->data)
        ...
        if (d)
            snprintf(
                b, sizeof(b), _(" (35 equivalent: %d mm)"),
    ...
    case EXIF_TAG_FILE_SOURCE:
        ...
        case 0x03:
            ...
            break;
        default:
            snprintf(val, maxlen, "0x%02x", e->data[0]);
            break;
        }
        break;
    ...
    case EXIF_TAG_YCBCR_SUB_SAMPLING:
        ...
        v_short2 = exif_get_short(e->data + exif_format_get_size(e->format), o);
    ...
    case EXIF_TAG_MAKER_NOTE:
        CF(e, EXIF_FORMAT_UNDEFINED, val, maxlen);
        snprintf(val, maxlen, _("%i bytes unknown data"), (int)e->components);
        break;
    case EXIF_TAG_SUBJECT_AREA:
        CF(e, EXIF_FORMAT_SHORT, val, maxlen);
        switch (e->components)
        ...
        case 2:
            ...
            snprintf(val, maxlen, "(x,y) = (%i,%i)", v_short, v_short2);
            break;
        case 3:
            v_short = exif_get_short(e->data, o);
            v_short2 = exif_get_short(e->data + 2, o);
            ...
            snprintf(val, maxlen,
                     _("Within distance %i of "
                       "(x,y) = (%i,%i)"),
                     v_short3, v_short, v_short2);
            break;
        case 4:
            v_short = exif_get_short(e->data, o);
            v_short2 = exif_get_short(e->data + 2, o);
            ...
            snprintf(val, maxlen,
                     _("Within rectangle "
                       ...
                       "(x,y) = (%i,%i)"),
                     v_short3, v_short4, v_short, v_short2);
            break;
        default:
            snprintf(val, maxlen,
                     _("Unexpected number "
    ...
    case EXIF_TAG_COLOR_SPACE:
        ...
        /* Find the value */
        for (j = 0;
             list2[i].elem[j].values[0] && (list2[i].elem[j].index < v_short);
             j++)
            ;
        if (list2[i].elem[j].index != v_short)
        {
            snprintf(val, maxlen,
                     _("Internal error (unknown "
                       "value %i)."),
                     v_short);
            break;
        }
    ...
    case EXIF_TAG_INTEROPERABILITY_VERSION:
        ...
        /* Fall through - EXIF_TAG_GPS_LATITUDE is same as
    ...
    default:
        ...
        case EXIF_FORMAT_SSHORT:
            ...
            for (i = 1; i < e->components; i++)
            {
                v_short = exif_get_short(
                    e->data + exif_format_get_size(e->format) * i, o);
        ...
        case EXIF_FORMAT_LONG:
            ...
            for (i = 1; i < e->components; i++)
            {
                v_long = exif_get_long(
                    e->data + exif_format_get_size(e->format) * i, o);
                snprintf(b, sizeof(b), ", %li", (long int)v_long);
                strncat(val, b, maxlen);
                maxlen -= strlen(b);
                if ((signed)maxlen <= 0)
                    break;
        ...
        case EXIF_FORMAT_RATIONAL:
            ...
            {
                snprintf(val, maxlen, "%i/%i", v_rat.numerator,
                         v_rat.denominator);
            }
            maxlen -= strlen(val);
            for (i = 1; i < e->components; i++)
        ...
        case EXIF_FORMAT_SRATIONAL:
            ...
            {
                snprintf(val, maxlen, "%i/%i", v_srat.numerator,
                         v_srat.denominator);
            }
            maxlen -= strlen(val);
            for (i = 1; i < e->components; i++)
...
void exif_entry_initialize(ExifEntry* e, ExifTag tag)
{
    ...
    case EXIF_TAG_BITS_PER_SAMPLE:
        ...
        exif_set_short(e->data + 2 * exif_format_get_size(e->format), o, 8);
        break;
    case EXIF_TAG_YCBCR_SUB_SAMPLING:
        e->components = 2;
        e->format = EXIF_FORMAT_SHORT;
    ...
    case EXIF_TAG_REFERENCE_BLACK_WHITE:
        ...
        r.numerator = 255;
        exif_set_rational(e->data + 3 * exif_format_get_size(e->format), o, r);
        r.numerator = 0;
        exif_set_rational(e->data + 4 * exif_format_get_size(e->format), o, r);
        r.numerator = 255;
        exif_set_rational(e->data + 5 * exif_format_get_size(e->format), o, r);
        break;
    ...
    case EXIF_TAG_DATE_TIME_DIGITIZED:
        ...
        if (!e->data)
            break;
        snprintf((char*)e->data, e->size, "%04i:%02i:%02i %02i:%02i:%02i",
