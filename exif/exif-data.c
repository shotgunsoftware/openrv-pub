...
static int exif_data_load_data_entry(ExifData* data, ExifEntry* entry,
                                     const unsigned char* d, unsigned int size,
                                     unsigned int offset)
{
    unsigned int s, doff;
...
static void exif_data_save_data_entry(ExifData* data, ExifEntry* e,
                                          ...
                                          ExifLong size)
{
    ...
    {
        exif_log(data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
                 "Bogus thumbnail offset and size: %i < %i + %i.", (int)ds,
                 (int)offset, (int)size);
        return;
    }
    if (data->data)
...
static void exif_data_load_data_content(ExifData* data, ExifIfd ifd,
    ...
    for (i = 0; i < n; i++)
    {
        ...
        case EXIF_TAG_JPEG_INTERCHANGE_FORMAT:
            ...
            case EXIF_TAG_INTEROPERABILITY_IFD_POINTER:
                ...
                exif_data_load_data_content(data, EXIF_IFD_INTEROPERABILITY, d,
                                            ds, o, recursion_depth + 1);
                break;
            case EXIF_TAG_JPEG_INTERCHANGE_FORMAT:
                thumbnail_offset = o;
                if (thumbnail_offset && thumbnail_length)
                    exif_data_load_data_thumbnail(data, d, ds, thumbnail_offset,
                                                  thumbnail_length);
                break;
            case EXIF_TAG_JPEG_INTERCHANGE_FORMAT_LENGTH:
                thumbnail_length = o;
        ...
        default:
            ...
            if (!exif_tag_get_name_in_ifd(tag, ifd))
            {
                ...
                exif_log(data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
                         "Unknown tag 0x%04x (entry %i in '%s'). Please report "
                         "this tag "
...
static int cmp_func(const unsigned char* p1, const unsigned char* p2,
                    ExifByteOrder o)
{
...
{
    return cmp_func((const unsigned char*)elem1, (const unsigned char*)elem2,
...
static void exif_data_save_data_content(ExifData* data, ExifContent* ifd,
                                        unsigned char** d, unsigned int* ds,
                                        unsigned int offset)
{
    ...
    *d = exif_mem_realloc(data->priv->mem, *d, *ds);
    ...
    /* Save each entry */
    exif_log(data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
             "Saving %i entries (IFD '%s', offset: %i)...", ifd->count,
    ...
    case EXIF_IFD_EXIF:
        ...
        if (data->ifd[EXIF_IFD_INTEROPERABILITY]->count)
        {
            ...
            exif_data_save_data_content(
                data, data->ifd[EXIF_IFD_INTEROPERABILITY], d, ds, *ds - 6);
            offset += 12;
        }
    ...
    qsort(*d + 6 + offset - (ifd->count + n_ptr + n_thumb) * 12,
          ...
          (data->priv->order == EXIF_BYTE_ORDER_INTEL) ? cmp_func_intel
                                                       : cmp_func_motorola);
...
static ExifDataTypeMakerNote exif_data_get_type_maker_note(ExifData* d)
{
    ...
    /* Olympus & Nikon */
    if ((e->size >= 8)
        && (!memcmp(e->data, "OLYMP", 6) || !memcmp(e->data, "OLYMPUS", 8)
    ...
    {
        if (!strncasecmp(exif_entry_get_value(em, value, sizeof(value)),
                         "Nikon", 5))
...
void exif_data_load_data(ExifData* data, const unsigned char* d_orig,
                         unsigned int ds_orig)
{
    ...
    else
    {
        ...
        len = (d[0] << 8) | d[1];
        exif_log(data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData",
                 "We have to deal with %i byte(s) of EXIF data.", len);
        d += 2;
        ds -= 2;
    }
    ...
    exif_log(data->priv->log, EXIF_LOG_CODE_DEBUG, "ExifData", "IFD 0 at %i.",
             (int)offset);
    ...
    case EXIF_DATA_TYPE_MAKER_NOTE_CANON:
        data->priv->md =
            exif_mnote_data_canon_new(data->priv->mem, data->priv->options);
        break;
    default:
        break;
    }
    ...
    if (data->data)
    {
        ...
        if (data->size >= 4)
        {
            printf("0x%02x 0x%02x ... 0x%02x 0x%02x\n", data->data[0],
                   data->data[1], data->data[data->size - 2],
...
void exif_data_foreach_content(ExifData* data, ExifDataForeachContentFunc func,
                               void* user_data)
{
    unsigned int i;
...
static void entry_set_byte_order(ExifEntry* e, void* data)
{
    ...
    exif_array_set_byte_order(e->format, e->data, e->components, d->old,
                              d->new);
}
    ...
    case EXIF_IFD_1:
        ...
        else
        {
            exif_log(c->parent->priv->log, EXIF_LOG_CODE_DEBUG, "exif-data",
                     "No thumbnail but entries on thumbnail. These entries "
                     "have been "
                     "removed.");
            while (c->count)
