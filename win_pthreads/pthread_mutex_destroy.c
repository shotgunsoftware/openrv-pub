    ...
    if (*mutex < PTHREAD_ERRORCHECK_MUTEX_INITIALIZER)
    {
        ...
        if (result == 0)
        {
            if (mx->kind != PTHREAD_MUTEX_RECURSIVE || 1 == mx->recursive_count)
            {
                 ...
                 * *mutex != NULL.
                 *
                 * Note that this would be an unusual situation because it is
                 * not common that mutexes are destroyed while they are still in
            ...
            {
                /*
                 * The mutex must be recursive and already locked by us (this
                 * thread).
                 */
                mx->recursive_count--; /* Undo effect of pthread_mutex_trylock()
                                          above */
                result = EBUSY;
            }
        }
