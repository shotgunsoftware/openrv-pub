 ...
 */
{
    ...
    if (thread.p != NULL)
    {
        ...
        do
        {
            ...
            /*
             * The pointer to the next assoc is stored in the thread struct so
             * that the assoc destructor in pthread_key_delete can adjust it if
             * it deletes this assoc. This can happen if we fail to acquire both
             * locks below, and are forced to release all of our locks, leaving
             * open the opportunity for pthread_key_delete to get in before us.
             */
            ...
            for (;;)
            {
                 ...
                 * both assoc guards, but in the reverse order to our
                 * convention, so we must be careful to avoid deadlock.
                 */
                (void)pthread_mutex_lock(&(sp->threadLock));
                ...
                else
                {
                     ...
                     * removed from our chain while we hold at least one lock.
                     * If the assoc was on our key chain then the key has not
                     * been deleted yet.
                     *
                    ...
                    if (pthread_mutex_trylock(&(assoc->key->keyLock)) == EBUSY)
                    {
                        pthread_mutex_unlock(&(sp->threadLock));
                        Sleep(
                            1); // Ugly but necessary to avoid priority effects.
                        /*
                         * Go around again.
                         * If pthread_key_delete has removed this assoc in the
                         * meantime, sp->nextAssoc will point to a new assoc.
                         */
                        continue;
                    }
                }
                    ...
                    && iterations <= PTHREAD_DESTRUCTOR_ITERATIONS)
                {
                     ...
                     * destructors, and that probably includes with this key as
                     * target. pthread_setspecific can also be run from
                     * destructors and also needs to be able to access the
                     * assocs.
                     */
                    (void)pthread_mutex_unlock(&(sp->threadLock));
