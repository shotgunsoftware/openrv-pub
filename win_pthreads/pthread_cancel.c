        ...
        && tp->state < PThreadStateCanceling)
    {
        ...
        else
        {
            ...
            if (WaitForSingleObject(threadH, 0) == WAIT_TIMEOUT)
            {
                ...
                tp->cancelState = PTHREAD_CANCEL_DISABLE;
                /*
                 * If alertdrv and QueueUserAPCEx is available then the
                 * following will result in a call to QueueUserAPCEx with the
                 * args given, otherwise this will result in a call to
                 * ptw32_RegisterCancelation and only the threadH arg will be
                 * used.
                 */
