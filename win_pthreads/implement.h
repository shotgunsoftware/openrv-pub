...
#else
/*
 * The routine pthread_win32_process_attach_np() in
 * pthread_win32_attach_detach_np.c checks at runtime that
 * InterlockedCompareExchange is supported within KERNEL32.DLL (or COREDLL.DLL
 * for WinCE). This allows the same dll to run on all Win32 versions from Win95
 * onwards. Not sure if this is required for WinCE, but should work just the
 * same anyway.
 */
