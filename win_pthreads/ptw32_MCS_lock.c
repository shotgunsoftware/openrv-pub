...
INLINE void ptw32_mcs_flag_set(LONG* flag)
{
    /*
      ...
      */
#ifdef _WIN64
    HANDLE e = (HANDLE)InterlockedCompareExchange64((LONGLONG*)flag,
                                                    (LONGLONG)-1, (LONGLONG)0);
#else
    HANDLE e =
        (HANDLE)InterlockedCompareExchange((LPLONG)flag, (LONG)-1, (LONG)0);
#endif
    if ((HANDLE)0 != e)
    {
...
#ifdef _WIN64
    if (0
        == InterlockedExchangeAdd64((LONGLONG*)flag,
                                    (LONGLONG)0)) /* MBR fence */
#else
...
#ifdef _WIN64
        if (0
            == InterlockedCompareExchange64((LONGLONG*)flag, (LONGLONG)e,
                                            (LONGLONG)0))
#else
        if (0 == InterlockedCompareExchange((LPLONG)flag, (LONG)e, (LONG)0))
#endif
        {
...
#ifdef _WIN64
    pred = (ptw32_mcs_local_node_t*)InterlockedExchange64((LONGLONG*)lock,
                                                          (LONGLONG)node);
#else
    pred =
        (ptw32_mcs_local_node_t*)InterlockedExchange((LPLONG)lock, (LONG)node);
#endif
...
INLINE void ptw32_mcs_lock_release(ptw32_mcs_local_node_t* node)
{
...
#ifdef _WIN64
        InterlockedExchangeAdd64((LONGLONG*)&node->next,
                                 (LONGLONG)0); /* MBR fence */
#else
    ...
    PTW32_INTERLOCKED_COMPARE_EXCHANGE((PTW32_INTERLOCKED_LPLONG)lock,
                                       ...
                                       */
#ifdef _WIN64
                InterlockedCompareExchange64((LONGLONG*)lock, (LONGLONG)0,
                                             (LONGLONG)node)
#else
                InterlockedCompareExchange((LPLONG)lock, (LONG)0, (LONG)node)
#endif
        )
        {
