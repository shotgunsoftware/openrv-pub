        ...
        Found_rsrc_section:
                 ...
                 i++)
            {
                     ...
                     j++)
                {
                    ...
                    if (FT_STREAM_SEEK(pe32_section.pointer_to_raw_data
                                       + dir_entry2.offset)
                        || FT_STREAM_READ_FIELDS(winpe_rsrc_dir_fields,
                                                 &lang_dir))
                        goto Exit;
                         ...
                         k++)
                    {
                        if (FT_STREAM_SEEK(lang_dir_offset + 16 + k * 8)
                            || FT_STREAM_READ_FIELDS(
                                winpe_rsrc_dir_entry_fields, &dir_entry3))
                            goto Exit;
...
static FT_UInt fnt_cmap_char_next(FNT_CMap cmap, FT_UInt32* pchar_code)
{
    FT_UInt gindex = 0;
    FT_UInt32 result = 0;
...
static FT_Error FNT_Face_Init(FT_Stream stream, FNT_Face face,
                               ...
                               FT_UInt glyph_index, FT_Int32 load_flags)
{
    ...
    if (!face || !font || glyph_index >= (FT_UInt)(FT_FACE(face)->num_glyphs))
    {
...
static const FT_Service_WinFntRec winfnt_service_rec = {winfnt_get_header};
