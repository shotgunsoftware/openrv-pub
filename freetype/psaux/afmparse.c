...
afm_parser_next_key(AFM_Parser parser, FT_Bool line, FT_UInt* len)
{
    AFM_Stream stream = parser->stream;
    ...
    if (len)
        *len = (key) ? AFM_STREAM_KEY_LEN(stream, key) : 0;

    return key;
}

static AFM_Token afm_tokenize(const char* key, FT_UInt len)
{
    int n;
    ...
    while ((key = afm_parser_next_key(parser, 1, &len)) != 0)
    {
        ...
        case AFM_TOKEN_KPY:
        {
            ...
            else
            {
                ...
                kp->y =
                    (token == AFM_TOKEN_KP && r == 4) ? shared_vals[3].u.i : 0;
            }
        }
        break;
...
static FT_Error afm_parser_skip_section(AFM_Parser parser, FT_UInt n,
