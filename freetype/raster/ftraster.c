...
static Bool Line_To(RAS_ARGS Long x, Long y)
{
    ...
    case Ascending_State:
        if (y < ras.lastY)
        {
            if (End_Profile(RAS_VAR) || New_Profile(RAS_VARS Descending_State))
                return FAILURE;
        }
        break;
    ...
    case Descending_State:
        if (y > ras.lastY)
        {
            if (End_Profile(RAS_VAR) || New_Profile(RAS_VARS Ascending_State))
                return FAILURE;
        }
        break;
    ...
    do
    {
        ...
        else
        {
            ...
            if (ras.state != state_bez)
            {
                ...
                if (ras.state != Unknown_State && End_Profile(RAS_VAR))
                    goto Fail;
...
static Bool Convert_Glyph(RAS_ARGS int flipped)
{
    ...
    for (i = 0; i < ras.outline.n_contours; i++)
    {
        ...
        if (Decompose_Curve(RAS_VARS(unsigned short) start,
                            ras.outline.contours[i], flipped))
            return FAILURE;
...
static void ft_black_reset(PRaster raster, char* pool_base, long pool_size)
{
    if (raster)
    {
        if (pool_base && pool_size >= (long)sizeof(TWorker) + 2048)
        {
            ...
            raster->buffer_size =
                ((pool_base + pool_size) - (char*)raster->buffer)
                / sizeof(Long);
...
static void ft_black_set_mode(PRaster raster, unsigned long mode,
                              const char* palette)
{
#ifdef FT_RASTER_OPTION_ANTI_ALIASING
...
static int ft_black_render(PRaster raster, const FT_Raster_Params* params)
{
    ...
    return ((params->flags & FT_RASTER_FLAG_AA) ? Render_Gray_Glyph(RAS_VAR)
