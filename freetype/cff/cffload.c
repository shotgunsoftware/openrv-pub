...
static FT_Error cff_index_load_offsets(CFF_Index idx)
{
    ...
    if (idx->count > 0 && idx->offsets == NULL)
    {
        ...
        if (FT_NEW_ARRAY(idx->offsets, idx->count + 1)
            || FT_STREAM_SEEK(idx->start + 3) || FT_FRAME_ENTER(data_size))
            goto Exit;
                                  ...
                                  FT_ULong base_offset, FT_ULong offset)
{
    ...
    if (offset > 1)
    {
        ...
        if (FT_STREAM_SEEK(encoding->offset) || FT_READ_BYTE(encoding->format)
            || FT_READ_BYTE(count))
            goto Exit;
    ...
    else
    {
        ...
        Populate:
            ...
            error =
                cff_charset_compute_cids(charset, num_glyphs, stream->memory);
            if (error)
                goto Exit;
...
static FT_Error cff_subfont_load(CFF_SubFont font, CFF_Index idx,
...
#define FT_STRUCTURE CFF_FontRec
    ...
    error = cff_subfont_load(&font->top_font, &font->font_dict_index,
                             face_index, stream, base_offset);
    if (error)
        goto Exit;
    ...
    error =
        cff_index_get_pointers(&font->global_subrs_index, &font->global_subrs);
