    ...
    for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++)
    {
        ...
        for (; p < limit && *p; p++)
        {
            ...
            if (best_point >= 0)
            {
                ...
                round = FT_BOOL(FT_CURVE_TAG(glyph->outline.tags[prev])
                                    != FT_CURVE_TAG_ON
                                || FT_CURVE_TAG(glyph->outline.tags[next])
                                       != FT_CURVE_TAG_ON);
        ...
        for (;;)
        {
            ...
            if (on_edge)
            {
                ...
                if (point->out_dir != segment_dir || point == last)
                {
                    ...
                    segment->height =
                        (FT_Short)(segment->max_coord - segment->min_coord);
        ...
        else
        {
            ...
            else
            {
                ...
                cur_len = af_latin_compute_stem_width(
                    hints, dim, org_len, (AF_Edge_Flags)edge->flags,
                    (AF_Edge_Flags)edge2->flags);
...
static FT_Error af_latin_hints_apply(AF_GlyphHints hints, FT_Outline* outline,
                                     AF_LatinMetrics metrics)
{
    FT_Error error;
    int dim;
