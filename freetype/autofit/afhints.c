...
af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, AF_Direction dir,
                       FT_Memory memory, AF_Edge* aedge)
{
    FT_Error error = AF_Err_Ok;
    AF_Edge edge = NULL;
    AF_Edge edges;
...
void af_glyph_hints_dump_points(AF_GlyphHints hints)
{
    ...
    for (point = points; point < limit; point++)
    {
        printf("  [ %5d | %5d | %5d | %-5.2f | %-5.2f "
               ...
               point - points, point->fx, point->fy, point->ox / 64.0,
               point->oy / 64.0, point->x / 64.0, point->y / 64.0,
    ...
    for (dimension = 1; dimension >= 0; dimension--)
    {
        ...
        for (seg = segments; seg < limit; seg++)
        {
            printf("  [ %5d | %5.2g | %5s | %4d | %5d | %5d | %5d | %s ]\n",
                   ...
                   AF_INDEX_NUM(seg->serif, segments), seg->height,
...
void af_glyph_hints_dump_points(AF_GlyphHints hints) { FT_UNUSED(hints); }

void af_glyph_hints_dump_segments(AF_GlyphHints hints) { FT_UNUSED(hints); }

void af_glyph_hints_dump_edges(AF_GlyphHints hints) { FT_UNUSED(hints); }
