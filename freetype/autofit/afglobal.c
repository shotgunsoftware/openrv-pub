...
#endif
    &af_latin_script_class,  &af_cjk_script_class,
    &af_indic_script_class,  NULL /* do not remove */
};
...
static FT_Error af_face_globals_compute_script_coverage(AF_FaceGlobals globals)
{
    ...
    FT_MEM_SET(globals->glyph_scripts, AF_SCRIPT_LIST_NONE,
               globals->glyph_count);
...
af_face_globals_new(FT_Face face, AF_FaceGlobals* aglobals)
{
    ...
    if (!FT_ALLOC(globals,
                  sizeof(*globals) + face->num_glyphs * sizeof(FT_Byte)))
    {
        globals->face = face;
                            ...
                            FT_UInt options, AF_ScriptMetrics* ametrics)
{
    ...
    const FT_UInt script_max =
        sizeof(af_script_classes) / sizeof(af_script_classes[0]);
    FT_Error error = AF_Err_Ok;
