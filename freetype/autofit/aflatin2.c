    ...
    for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++)
    {
        ...
        for (; p < limit && *p; p++)
        {
...

            {
                ...
                for (nn = 0; nn < glyph->outline.n_contours;
                     first = last + 1, nn++)
                {
            ...
            /* lies, then inspect its previous and next points              */
            {
                ...
                round = FT_BOOL(FT_CURVE_TAG(glyph->outline.tags[start])
                                    != FT_CURVE_TAG_ON
                                || FT_CURVE_TAG(glyph->outline.tags[end])
                                       != FT_CURVE_TAG_ON);
...
af_latin2_metrics_init(AF_LatinMetrics metrics, FT_Face face)
{
    ...
    static const FT_Encoding latin_encodings[] = {
        FT_ENCODING_UNICODE, FT_ENCODING_APPLE_ROMAN,
        FT_ENCODING_ADOBE_STANDARD, FT_ENCODING_ADOBE_LATIN_1,
    ...
    if (dim == AF_DIMENSION_VERT)
    {
        ...
        if (blue)
        {
            ...
            if (scaled != fitted)
            {
                ...
                AF_LOG(
                    ("== scaled x-top = %.2g  fitted = %.2g, scaling = %.4g\n",
    ...
    if (dim == AF_DIMENSION_VERT)
    {
        ...
        for (nn = 0; nn < axis->blue_count; nn++)
        {
            ...
            if (dist <= 48 && dist >= -48)
            {
                ...
                AF_LOG((">> activating blue zone %d:  ref.cur=%.2g "
                        "ref.fit=%.2g shoot.cur=%.2g shoot.fit=%.2g\n",
...
af_latin2_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    for (seg = segments; seg < segment_limit; seg++)
    {
        ...
        if (!found)
        {
            ...
            error =
                af_axis_hints_new_edge(axis, seg->pos, seg->dir, memory, &edge);
            if (error)
                goto Exit;
...
static void af_latin2_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim,
                                        AF_Edge base_edge, AF_Edge stem_edge)
{
    FT_Pos dist = stem_edge->opos - base_edge->opos;

    FT_Pos fitted_width = af_latin2_compute_stem_width(
        hints, dim, dist, (AF_Edge_Flags)base_edge->flags,
...
static void af_latin2_align_serif_edge(AF_GlyphHints hints, AF_Edge base,
                                       AF_Edge serif)
{
    FT_UNUSED(hints);
...
af_latin2_hint_edges(AF_GlyphHints hints, AF_Dimension dim)
{
    ...
    if (dim == AF_DIMENSION_VERT && AF_HINTS_DO_BLUES(hints))
    {
        for (edge = edges; edge < edge_limit; edge++)
        {
            ...
            if (!anchor)
            {
                ...
                if (edge2)
                    anchor_drift =
                        (anchor_drift + (edge2->pos - edge2->opos)) >> 1;
            }
        }
    }
    ...
    for (edge = edges; edge < edge_limit; edge++)
    {
        ...
        else
        {
            ...
            AF_LOG(("ALIGN: left=%.2f right=%.2f ", org_left / 64.0,
                    org_right / 64.0));
            ...
            else
            {
                ...
                if (cur_len <= 96)
                {
                     ...
                     * to 25/75%, since this is much more pleasant to the eye
                     * with very acceptable distortion
                     */
        ...
        for (edge = edges; edge < edge_limit; edge++)
        {
            ...
            else
            {
                    ...
                    && after > edge)
                {
                    ...
                    AF_LOG(("SERIF_LINK1: edge %d (opos=%.2f) snapped to "
                            "(%.2f) from %d (opos=%.2f)\n",
                            edge - edges, edge->opos / 64.0, edge->pos / 64.0,
                ...
                else
                {
                    ...
                    AF_LOG(
                        ("SERIF_LINK2: edge %d (opos=%.2f) snapped to (%.2f)\n",
                         edge - edges, edge->opos / 64.0, edge->pos / 64.0));
                }
            }
...
static FT_Error af_latin2_hints_apply(AF_GlyphHints hints, FT_Outline* outline,
                                      AF_LatinMetrics metrics)
{
    FT_Error error;
    int dim;
...
FT_CALLBACK_TABLE_DEF const AF_ScriptClassRec af_latin2_script_class = {
    AF_SCRIPT_LATIN2,
    af_latin2_uniranges,
