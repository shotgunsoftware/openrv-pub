...
parse_font_matrix(CID_Face face, CID_Parser* parser)
{
    ...
    if (parser->num_dict >= 0)
    {
        ...
        root->units_per_EM =
            (FT_UShort)(FT_DivFix(0x10000L, FT_DivFix(temp_scale, 1000)));
...
#include "cidtoken.h"
    ...
    T1_FIELD_CALLBACK("FDArray", parse_fd_array,
                      0) T1_FIELD_CALLBACK("FontMatrix", parse_font_matrix, 0)
...
static FT_Error cid_hex_to_binary(FT_Byte* data, FT_Long data_len,
