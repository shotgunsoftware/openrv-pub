...
static FT_Pos ft_pos_abs(FT_Pos x) { return x >= 0 ? x : -x; }
...
static FT_Error ft_stroker_outside(FT_Stroker stroker, FT_Int side)
{
    ...
    else
    {
        ...
        if (miter) /* this is a miter (broken angle) */
        {
            ...
            length = FT_MulFix(
                radius, FT_DivFix(0x10000L - sigma, ft_pos_abs(FT_Sin(theta))));
    ...
    while (arc >= bez_stack)
    {
        ...
        /* border                                                          */
        {
            ...
            for (side = 0; side <= 1; side++)
            {
                ...
                error = ft_stroke_border_conicto(stroker->borders + side, &ctrl,
                                                 &end);
                if (error)
                    goto Exit;
            }
                   ...
                   FT_Vector* to)
{
    ...
    while (arc >= bez_stack)
    {
        ...
        if (arc < limit
            && !ft_cubic_is_small_enough(arc, &angle_in, &angle_mid,
                                         &angle_out))
        {
            ft_cubic_split(arc);
            arc += 3;
...
FT_Glyph_Stroke(FT_Glyph* pglyph, FT_Stroker stroker, FT_Bool destroy)
{
...

    {
        ...
        error =
            FT_Outline_New(glyph->library, num_points, num_contours, outline);
        if (error)
            goto Fail;
                      ...
                      FT_Bool destroy)
{
...

    {
        ...
        (void)FT_Stroker_GetBorderCounts(stroker, border, &num_points,
                                         &num_contours);
