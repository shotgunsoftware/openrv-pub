                                 ...
                                 FT_Pointer glyph_data)
{
    ...
    if (!FT_NEW_ARRAY(table->maps, num_glyphs))
    {
        ...
        if (count == 0)
        {
            ...
            if (!error)
                error =
                    PSnames_Err_Invalid_Argument; /* No unicode chars here! */
        }
        else
        {
...
static FT_ULong ps_unicodes_char_next(PS_Unicodes table, FT_UInt32* unicode)
{
    FT_UInt result = 0;
...
static const FT_ServiceDescRec pscmaps_services[] = {
    {FT_SERVICE_ID_POSTSCRIPT_CMAPS, &pscmaps_interface}, {NULL, NULL}};
