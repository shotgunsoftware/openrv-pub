...
gxv_BinSrchHeader_validate(FT_Bytes table, FT_Bytes limit,
                           FT_UShort* unitSize_p, FT_UShort* nUnits_p,
                           GXV_Validator valid)
{
    FT_Bytes p = table;
...
static void gxv_LookupTable_fmt0_validate(FT_Bytes table, FT_Bytes limit,
                                          GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_UShort i;
...
static void gxv_LookupTable_fmt2_validate(FT_Bytes table, FT_Bytes limit,
                                          GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_UShort gid;
    ...
    for (unit = 0, gid = 0; unit < nUnits; unit++)
    {
        ...
        if (lastGlyph < firstGlyph)
        {
            GXV_TRACE(("reverse ordered range specification at unit %d:",
                       " lastGlyph %d < firstGlyph %d ", unit, lastGlyph,
                       firstGlyph));
            if (valid->root->level >= FT_VALIDATE_PARANOID)
                FT_INVALID_GLYPH_ID;
...
static void gxv_LookupTable_fmt4_validate(FT_Bytes table, FT_Bytes limit,
                                          GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_UShort unit;
...
static void gxv_LookupTable_fmt6_validate(FT_Bytes table, FT_Bytes limit,
                                          GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_UShort unit;
...
static void gxv_LookupTable_fmt8_validate(FT_Bytes table, FT_Bytes limit,
                                          GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_UShort i;
...
static void gxv_ClassTable_validate(FT_Bytes table, FT_UShort* length_p,
                                    FT_UShort stateSize, FT_Byte* maxClassID_p,
                                    GXV_Validator valid)
{
    FT_Bytes p = table;
...
static void gxv_StateArray_validate(FT_Bytes table, FT_UShort* length_p,
                                    FT_Byte maxClassID, FT_UShort stateSize,
                                    FT_Byte* maxState_p, FT_Byte* maxEntry_p,
                                    GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_Bytes limit = table + *length_p;
...
static void
gxv_EntryTable_validate(FT_Bytes table, FT_UShort* length_p, FT_Byte maxEntry,
                        FT_UShort stateArray, FT_UShort stateArray_length,
                        FT_Byte maxClassID, FT_Bytes statetable_table,
                        FT_Bytes statetable_limit, GXV_Validator valid)
{
    ...
    for (entry = 0; entry <= maxEntry; entry++)
    {
        ...
        if (NULL != valid->statetable.entry_validate_func)
            valid->statetable.entry_validate_func(state, flags, glyphOffset,
                                                  statetable_table,
                                                  statetable_limit, valid);
    }
...
gxv_StateTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator valid)
{
    ...
    if (classTable != 0)
        gxv_ClassTable_validate(table + classTable, &classTable_length,
                                stateSize, &maxClassID, valid);
    else
        maxClassID = (FT_Byte)(stateSize - 1);

    if (stateArray != 0)
        gxv_StateArray_validate(table + stateArray, &stateArray_length,
                                maxClassID, stateSize, &maxState, &maxEntry,
                                valid);
    else
    {
    ...
    if (entryTable != 0)
        gxv_EntryTable_validate(table + entryTable, &entryTable_length,
                                maxEntry, stateArray, stateArray_length,
                                maxClassID, table, limit, valid);
...
static GXV_LookupValueDesc gxv_XClassTable_lookupfmt4_transit(
    FT_UShort relative_gindex, GXV_LookupValueDesc base_value,
...
static void gxv_XStateArray_validate(FT_Bytes table, FT_ULong* length_p,
                                     FT_UShort maxClassID, FT_ULong stateSize,
                                     FT_UShort* maxState_p,
                                     FT_UShort* maxEntry_p, GXV_Validator valid)
{
    FT_Bytes p = table;
    FT_Bytes limit = table + *length_p;
    FT_UShort clazz;
...
static void
gxv_XEntryTable_validate(FT_Bytes table, FT_ULong* length_p, FT_UShort maxEntry,
                         FT_ULong stateArray_length, FT_UShort maxClassID,
                         ...
                         GXV_Validator valid)
{
    ...
    for (entry = 0; entry <= maxEntry; entry++)
    {
        ...
        if (NULL != valid->xstatetable.entry_validate_func)
            valid->xstatetable.entry_validate_func(state, flags, glyphOffset,
                                                   xstatetable_table,
                                                   xstatetable_limit, valid);
    }
...
gxv_XStateTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator valid)
{
    ...
    setup_func(limit - table, classTable, stateArray, entryTable,
               &classTable_length, &stateArray_length, &entryTable_length,
               valid);
    ...
    if (classTable != 0)
    {
        ...
        gxv_LookupTable_validate(table + classTable,
                                 table + classTable + classTable_length, valid);
    ...
    if (stateArray != 0)
        gxv_XStateArray_validate(table + stateArray, &stateArray_length,
                                 valid->xstatetable.maxClassID,
                                 valid->xstatetable.nClasses, &maxState,
                                 &maxEntry, valid);
    else
    {
        maxState = 1; /* 0:start of text, 1:start of line are predefined */
    ...
    if (entryTable != 0)
        gxv_XEntryTable_validate(
            ...
            valid->xstatetable.maxClassID, table, limit, valid);
...
static int gxv_compare_ranges(FT_Bytes table1_start, FT_ULong table1_length,
                              FT_Bytes table2_start, FT_ULong table2_length)
{
