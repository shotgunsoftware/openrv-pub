...
/*                                                                       */
static void Get_HMetrics(TT_Face face, FT_UInt idx, FT_Bool check,
...
/*                                                                       */
static void Get_VMetrics(TT_Face face, FT_UInt idx, FT_Bool check,
                         FT_Short* tsb, FT_UShort* ah)
{
    FT_UNUSED(check);
                                           ...
                                           FT_UInt start_contour)
{
...

    {
        ...
        {
            FT_TRACE0(
                ("TT_Process_Composite_Glyph: Too many instructions (%d)\n",
                 n_ins));
                                    ...
                                    FT_UInt recurse_count)
{
    ...
    error = face->access_glyph_frame(
        loader, glyph_index, loader->glyf_offset + offset, loader->byte_len);
    if (error)
        goto Exit;
    ...
    else if (loader->n_contours == -1)
    {
        ...
        if (face->doblend)
        {
            ...
            if ((error = TT_Vary_Get_Glyph_Deltas(face, glyph_index, &deltas,
                                                  gloader->current.num_subglyphs
                                                      + 4))
                != 0)
                goto Exit;
