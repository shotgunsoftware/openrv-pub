    ...
    for (i = 0; i < blend->num_axis; ++i)
    {
        ...
        else if (blend->normalizedcoords[i] < tuple_coords[i])
        {
            temp = FT_MulDiv(blend->normalizedcoords[i] - im_start_coords[i],
                             0x10000L, tuple_coords[i] - im_start_coords[i]);
        ...
        else
        {
            temp = FT_MulDiv(im_end_coords[i] - blend->normalizedcoords[i],
                             0x10000L, im_end_coords[i] - tuple_coords[i]);
            apply = FT_MulDiv(apply, temp, 0x10000L);
        }
    }
...
#define FT_STRUCTURE GX_FVar_Axis
        ...
        FT_FRAME_ULONG(minValue), FT_FRAME_ULONG(defaultValue),
        FT_FRAME_ULONG(maxValue), FT_FRAME_USHORT(flags),
    ...
    if (face->blend == NULL)
    {
        ...
        if ((error = face->goto_table(face, TTAG_fvar, stream, &table_len))
            != 0)
            goto Exit;
        ...
        if (fvar_head.version != (FT_Long)0x00010000L
            || fvar_head.countSizePairs != 2 || fvar_head.axisSize != 20
...
TT_Set_MM_Blend(TT_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    ...
    {
        for (i = 0; i < num_coords && blend->normalizedcoords[i] == coords[i];
             ++i)
            ;
        if (i == num_coords)
...
TT_Set_Var_Design(TT_Face face, FT_UInt num_coords, FT_Fixed* coords)
{
    ...
    for (i = 0; i < mmvar->num_axis; ++i, ++a)
    {
        ...
        {
            normalized[i] =
                -FT_MulDiv(coords[i] - a->def, 0x10000L, a->minimum - a->def);
        }
        ...
        {
            normalized[i] =
                FT_MulDiv(coords[i] - a->def, 0x10000L, a->maximum - a->def);
        }
    }
...
tt_face_vary_cvt(TT_Face face, FT_Stream stream)
{
    ...
    for (i = 0; i < (tupleCount & 0xFFF); ++i)
    {
        ...
        deltas = ft_var_readpackeddeltas(
            stream, point_count == 0 ? face->cvt_size : point_count);
        ...
        else if (localpoints == ALL_POINTS)
        {
            ...
            for (j = 0; j < face->cvt_size; ++j)
                face->cvt[j] =
                    (FT_Short)(face->cvt[j] + FT_MulFix(deltas[j], apply));
        }
    ...
    *deltas = delta_xy;
    ...
    for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); ++i)
    {
        ...
        deltas_x = ft_var_readpackeddeltas(
            stream, point_count == 0 ? n_points : point_count);
