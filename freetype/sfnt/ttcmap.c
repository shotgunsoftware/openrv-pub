...
static FT_UInt tt_cmap4_char_map_linear(TT_CMap cmap, FT_UInt* pcharcode,
                                        FT_Bool next)
{
...
static FT_UInt tt_cmap4_char_map_binary(TT_CMap cmap, FT_UInt* pcharcode,
                                        FT_Bool next)
{
    FT_UInt num_segs2, start, end, offset;
    FT_Int delta;
...
const TT_CMap_ClassRec tt_cmap4_class_rec = {
    {sizeof(TT_CMap4Rec), (FT_CMap_InitFunc)tt_cmap4_init,
     (FT_CMap_DoneFunc)NULL, (FT_CMap_CharIndexFunc)tt_cmap4_char_index,
        ...
        if (offset && offset <= face->cmap_size - 2)
        {
            ...
            for (; *pclazz; pclazz++)
            {
                ...
                if (clazz->format == format)
                {
                    ...
                    if (ft_setjmp(
                            *((ft_jmp_buf*)&FT_VALIDATOR(&valid)->jump_buffer))
                        == 0)
                    {
