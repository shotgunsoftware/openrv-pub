                           ...
                           PSH_Glyph glyph)
{
    ...
    if (!psh_hint_is_fitted(hint))
    {
        ...
        if (dimension == 1)
            psh_blues_snap_stem(&globals->blues, hint->org_pos + hint->org_len,
                                hint->org_pos, &align);
...
psh_corner_is_flat(FT_Pos x_in, FT_Pos y_in, FT_Pos x_out, FT_Pos y_out)
{
    FT_Pos ax = x_in;
    FT_Pos ay = y_in;
...
static FT_Int psh_corner_orientation(FT_Pos in_x, FT_Pos in_y, FT_Pos out_x,
                                     FT_Pos out_y)
{
    FT_Int result;
