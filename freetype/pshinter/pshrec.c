...
static FT_Int ps_mask_table_test_intersect(PS_Mask_Table table, FT_Int index1,
                                           FT_Int index2)
{
...
static FT_Error ps_mask_table_merge(PS_Mask_Table table, FT_Int index1,
                                    FT_Int index2, FT_Memory memory)
{
    FT_UInt temp;
    FT_Error error = 0;
                                         ...
                                         FT_Memory memory)
{
    ...
    {
        if (ps_mask_test_bit(counter, hint1) || ps_mask_test_bit(counter, hint2)
...
static void ps_hints_stem(PS_Hints hints, FT_Int dimension, FT_UInt count,
                          FT_Long* stems)
{
    if (!hints->error)
    {
        ...
        case PS_HINT_TYPE_2: /* Type 2 "hstem" or "vstem" operator */
        {
            ...
            for (; count > 0; count--, stems += 2)
            {
                ...
                error = ps_dimension_add_t1stem(dim, (FT_Int)stems[0],
                                                (FT_Int)stems[1], memory, NULL);
                if (error)
                {
...
static void ps_hints_t1stem3(PS_Hints hints, FT_Int dimension, FT_Long* stems)
{
    FT_Error error = 0;
...
static void t1_hints_stem(T1_Hints hints, FT_Int dimension, FT_Long* coords)
{
...
static void t2_hints_stems(T2_Hints hints, FT_Int dimension, FT_Int count,
                           FT_Fixed* coords)
{
    FT_Pos stems[32], y, n;
    FT_Int total = count;
