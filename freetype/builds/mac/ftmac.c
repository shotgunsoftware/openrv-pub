        ...
        if (status == 0)
        {
            ...
            *face_index = 0;
            ...
            while (stat2 == 0 && !the_font)
            {
                ...
                stat2 = FMGetNextFontFamilyInstance(&instIter, &font, &style,
                                                    &size);
                if (stat2 == 0 && size == 0)
                {
                    char fullName[256];
...
ft_FSp_stream_io(FT_Stream stream, unsigned long offset, unsigned char* buffer,
                 unsigned long count)
{
    FT_FILE* file;
...
static OSErr FT_FSPathMakeSpec(const UInt8* pathname, FSSpec* spec_p,
                               Boolean isDirectory)
{
    const char *p, *q;
    short vRefNum;
...
static OSErr FT_FSPathMakeRes(const UInt8* pathname, short* res)
{
...
static OSType get_file_type_from_path(const UInt8* pathname)
{
    ...
    if (noErr
        != FSGetCatalogInfo(&ref, kFSCatInfoFinderInfo, &info, NULL, NULL,
                            NULL))
        return (OSType)0;
...
static void parse_fond(char* fond_data, short* have_sfnt, short* sfnt_id,
                       Str255 lwfn_file_name, short face_index)
{
    AsscEntry* assoc;
...
static FT_Error read_lwfn(FT_Memory memory, short res, FT_Byte** pfb_data,
                          FT_ULong* size)
{
...
static FT_Error open_face_from_buffer(FT_Library library, FT_Byte* base,
                                      FT_ULong size, FT_Long face_index,
...
static FT_Error FT_New_Face_From_LWFN(FT_Library library, const UInt8* pathname,
...
static FT_Error FT_New_Face_From_SFNT(FT_Library library, short sfnt_id,
                                      FT_Long face_index, FT_Face* aface)
{
    Handle sfnt = NULL;
    FT_Byte* sfnt_data;
    ...
    is_cff = sfnt_size > 4 && sfnt_data[0] == 'O' && sfnt_data[1] == 'T'
             && sfnt_data[2] == 'T' && sfnt_data[3] == 'O';

    return open_face_from_buffer(library, sfnt_data, sfnt_size, face_index,
                      ...
                      FT_Face* aface)
{
    ...
    if (lwfn_file_name[0])
    {
...

        {
            ...
            err = FSMakeFSSpec(pb.ioFCBVRefNum, pb.ioFCBParID, fond_file_name,
                               &spec);
            if (noErr != err)
                goto found_no_lwfn_file;
