...
ft_amiga_stream_io(FT_Stream stream, unsigned long offset,
                   unsigned char* buffer, unsigned long count)
{
    ...
    if (count != 0)
    {
        ...
        if (offset + count <= sysfile->iobuf_end)
        {
            ...
            CopyMem(&sysfile->iobuf[offset - sysfile->iobuf_start], buffer,
                    count);
            read_bytes = count;
        }
        else
            ...
            else
            {
                ...
                ** bytes of that to our internal buffer for later use */
                ...
                else
                {
                    ...
                    bufsize =
                        (read_bytes > IOBUF_SIZE) ? IOBUF_SIZE : read_bytes;
                    sysfile->iobuf_end = offset + read_bytes;
                    sysfile->iobuf_start = sysfile->iobuf_end - bufsize;
                    CopyMem(&buffer[read_bytes - bufsize], sysfile->iobuf,
                            bufsize);
                }
            }
        }
