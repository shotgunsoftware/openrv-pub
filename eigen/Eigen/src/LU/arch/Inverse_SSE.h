...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        struct compute_inverse_size4<Architecture::SSE, float, MatrixType,
                                     ResultType>
        {
            enum
            {
            ...
            static void run(const MatrixType& mat, ResultType& result)
            {
                ...
                // technique. The original matrix is divide into four 2x2
                // sub-matrices. Since each register holds four matrix element,
                // the smaller matrices are represented as a registers. Hence we
                // get a better locality of the calculations.
                ...
                //  iD = C*A#*B
                iD = _mm_mul_ps(_mm_shuffle_ps(C, C, 0xA0),
                                _mm_movelh_ps(AB, AB));
                iD = _mm_add_ps(iD, _mm_mul_ps(_mm_shuffle_ps(C, C, 0xF5),
                                               _mm_movehl_ps(AB, AB)));
                //  iA = B*D#*C
                iA = _mm_mul_ps(_mm_shuffle_ps(B, B, 0xA0),
                                _mm_movelh_ps(DC, DC));
                ...
                //     #ifdef ZERO_SINGULAR
                //         rd = _mm_and_ps(_mm_cmpneq_ss(det,_mm_setzero_ps()),
                //         rd);
                //     #endif
                ...
                pstoret<float, Packet4f, ResultAlignment>(
                    res + 0, _mm_shuffle_ps(iA, iB, 0x77));
                pstoret<float, Packet4f, ResultAlignment>(
                    res + res_stride, _mm_shuffle_ps(iA, iB, 0x22));
                pstoret<float, Packet4f, ResultAlignment>(
                    res + 2 * res_stride, _mm_shuffle_ps(iC, iD, 0x77));
                pstoret<float, Packet4f, ResultAlignment>(
                    res + 3 * res_stride, _mm_shuffle_ps(iC, iD, 0x22));
            }
        };
                                     ...
                                     ResultType>
        {
            ...
            static void run(const MatrixType& mat, ResultType& result)
            {
                ...
                // The inverse is calculated using "Divide and Conquer"
                // technique. The original matrix is divide into four 2x2
                // sub-matrices. Since each register of the matrix holds two
                // elements, the smaller matrices are consisted of two
                ...
                AB1 =
                    _mm_sub_pd(AB1, _mm_mul_pd(B2, _mm_shuffle_pd(A1, A1, 3)));
                AB2 =
                    _mm_sub_pd(AB2, _mm_mul_pd(B1, _mm_shuffle_pd(A2, A2, 0)));
                ...
                DC1 =
                    _mm_sub_pd(DC1, _mm_mul_pd(C2, _mm_shuffle_pd(D1, D1, 3)));
                DC2 =
                    _mm_sub_pd(DC2, _mm_mul_pd(C1, _mm_shuffle_pd(D2, D2, 0)));
                ...
                iD1 =
                    _mm_add_pd(iD1, _mm_mul_pd(AB2, _mm_shuffle_pd(C1, C1, 3)));
                iD2 =
                    _mm_add_pd(iD2, _mm_mul_pd(AB2, _mm_shuffle_pd(C2, C2, 3)));

                //  iA = B*D#*C
                iA1 = _mm_mul_pd(DC1, _mm_shuffle_pd(B1, B1, 0));
                iA2 = _mm_mul_pd(DC1, _mm_shuffle_pd(B2, B2, 0));
                iA1 =
                    _mm_add_pd(iA1, _mm_mul_pd(DC2, _mm_shuffle_pd(B1, B1, 3)));
                iA2 =
                    _mm_add_pd(iA2, _mm_mul_pd(DC2, _mm_shuffle_pd(B2, B2, 3)));
                ...
                //     #ifdef ZERO_SINGULAR
                //         rd = _mm_and_pd(_mm_cmpneq_sd(det,_mm_setzero_pd()),
                //         rd);
                //     #endif
