...
#define KILL_NON_PRINCIPAL_COL(c)          \
    ...
    template <typename IndexType> struct colamd_col
    {
        ...
        union
        {
            ...
            IndexType
                parent; /* parent in parent tree super-column structure, if */
                        /* the column is dead */
        } shared1;
        ...
        {
            IndexType
                score; /* the score used to maintain heap, if col is alive */
    ...
    template <typename IndexType> struct Colamd_Row
    {
        ...
        {
            IndexType
                mark; /* for computing set differences and marking dead rows*/
            IndexType first_column; /* first column in row (used in garbage
                                       collection) */
        } shared2;
    };
    ...
    /*
      ...
      Explicit typecast to IndexType added Sept. 23, 2002, COLAMD version 2.2,
      to avoid gcc -pedantic warning messages.
    */
    ...
    static IndexType
    init_rows_cols(IndexType n_row, IndexType n_col,
                   Colamd_Row<IndexType> Row[], colamd_col<IndexType> col[],
    ...
    static IndexType find_ordering(IndexType n_row, IndexType n_col,
                                   IndexType Alen, Colamd_Row<IndexType> Row[],
...
#define COLAMD_ASSERT(expression) ((void)0)
                       ...
                       IndexType stats[COLAMD_STATS])
    {
        ...
        if (!Eigen::internal::init_rows_cols(n_row, n_col, Row, Col, A, p,
                                             stats))
        {
        ...
        ngarbage = Eigen::internal::find_ordering(
    ...
    */
        ...
        )
    {
        ...
        stats[COLAMD_INFO3] =
            0; /* number of duplicate or unsorted row indices*/
    ...
    */
    template <typename IndexType>
    static IndexType
        find_ordering /* return the number of garbage collections */
        (
            /* === Parameters
               ======================================================= */
        ...
        )
    {
        ...
        IndexType
            set_difference;  /* set difference size of row with pivot row */
        IndexType min_score; /* smallest column score */
        IndexType
            col_thickness;  /* "thickness" (no. of columns in a supercol) */
        IndexType max_mark; /* maximum value of tag_mark */
        IndexType pivot_col_thickness; /* number of columns represented by pivot
                                          col */
        ...
        for (k = 0; k < n_col2; /* 'k' is incremented below */)
        {
            ...
            {
                pfree = Eigen::internal::garbage_collection(n_row, n_col, Row,
                                                            Col, A, &A[pfree]);
                ngarbage++;
            ...
            /* Here begins the computation of the approximate degree.  The
             * column */
            /* score is the sum of the pivot row "length", plus the size of the
             */
            /* set differences of each row in the column minus the pattern of
             * the */
             ...
             */
            /* the pivot row.  Thus, the amortized time to compute a column
             * score */
            ...
            while (rp < rp_end)
            {
                ...
                else
                {
                    ...
                    {
                        /* degree list "hash" is non-empty, use prev (shared3)
                         * of */
            ...
            if (pivot_row_degree > 0)
            {
                ...
                Row[pivot_row].length =
                    (IndexType)(new_rp - &A[pivot_row_start]);
    ...
    /*
      ...
      like it at first glance, but be assured that this routine takes time
      linear in the number of columns.  Although not immediately obvious, the
      time taken by this routine is O (n_col), that is, linear in the number of
    ...
    */
    ...
    )
    {
        ...
        for (i = 0; i < n_col; i++)
        {
            ...
            if (!COL_IS_DEAD_PRINCIPAL(i)
                && Col[i].shared2.order == COLAMD_EMPTY)
            {
                parent = i;
    ...
    /*
      ...
      buckets. Check amongst columns in the set A [row_start ... row_start +
      row_length-1]. The columns under consideration are currently *not* in the
      degree lists, and have already been placed in the hash buckets.
      ...
      for that column.  The value of Col [c].shared4.hash_next is the next
      column in the same hash bucket.
        ...
        )
    {
        ...
        for (r = 0; r < n_row; r++)
        {
            if (ROW_IS_ALIVE(r))
            {
                if (Row[r].length == 0)
                {
                    /* this row is of zero length.  cannot compact it, so kill
                     * it */
                ...
                else
                {
                    ...
                    /* flag the start of the row with the one's complement of
                     * row */
