...
namespace Eigen
{
    ...
    namespace internal
    {
         ...
         *
         * \param[in] C the input selfadjoint matrix stored in compressed column
         * major format.
         * \param[out] perm the permutation P reducing the fill-in of the input
         * matrix \a C
         *
         * Note that the input matrix \a C must be complete, that is both the
         * upper and lower parts have to be stored, as well as the diagonal
         * entries. On exit the values of C are destroyed */
            ...
            PermutationMatrix<Dynamic, Dynamic, StorageIndex>& perm)
        {
            ...
            StorageIndex d, dk, dext,
                lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln, dense,
                nzmax, mindeg = 0, nvi, nvj, nvk, mark, wnvi, ok, nel = 0, p,
                p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, t, h;

            StorageIndex n = StorageIndex(C.cols());
            dense = std::max<StorageIndex>(
                16,
                StorageIndex(10 * sqrt(double(n)))); /* find dense threshold */
            ...
            // get workspace
            ei_declare_aligned_stack_constructed_variable(StorageIndex, W,
                                                          8 * (n + 1), 0);
            StorageIndex* len = W;
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (d == 1 && has_diag) /* node i is empty */
                {
                    ...
                    w[i] = 0;
                         ...
                         || !has_diag) /* node i is dense or has no structural
                                          diagonal element */
                {
            ...
            while (nel < n) /* while (selecting pivots) do */
            {
                ...
                if (elenk > 0 && cnz + mindeg >= nzmax)
                {
                    for (j = 0; j < n; j++)
                    {
                        if ((p = Cp[j]) >= 0) /* j is a live node or element */
                        {
                            ...
                            Ci[p] = amd_flip(
                                j); /* first entry is now amd_flip(j) */
                        }
                    }
                ...
                for (pk = pk1; pk < pk2; pk++) /* scan 1: find |Le\Lk| */
                {
                    ...
                    for (p = Cp[i]; p <= Cp[i] + eln - 1; p++) /* scan Ei */
                    {
                        ...
                        {
                            w[e] = degree[e]
                                   + wnvi; /* 1st time e seen in scan 1 */
                        }
                    }
                }
                ...
                for (pk = pk1; pk < pk2; pk++) /* scan2: degree update */
                {
                    ...
                    for (h = 0, d = 0, p = p1; p <= p2; p++) /* scan Ei */
                    {
                        ...
                        if (w[e] != 0) /* e is an unabsorbed element */
                        {
                            ...
                            {
                                Cp[e] =
                                    amd_flip(k); /* aggressive absorb. e->k */
                    ...
                    {
                        degree[i] = std::min<StorageIndex>(
                            degree[i], d); /* update degree(i) */
                        Ci[pn] = Ci[p3];   /* move first node to end */
                        Ci[p3] = Ci[p1];   /* move 1st el. to end of Ei */
                        Ci[p1] = k;        /* add k as 1st element in of Ei */
                        len[i] =
                            pn - p1 + 1;    /* new len of adj. list of node i */
                ...
                for (p = pk1, pk = pk1; pk < pk2; pk++) /* finalize Lk */
                {
                    ...
                    mindeg = std::min<StorageIndex>(
                        mindeg, d); /* find new minimum degree */
                    degree[i] = d;
                    Ci[p++] = i; /* place i in Lk */
                }
                nv[k] = nvk; /* # nodes absorbed into k */
                if ((len[k] = p - pk1)
                    == 0) /* length of adj list of element k*/
                {
