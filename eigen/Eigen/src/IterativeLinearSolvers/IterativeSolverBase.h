...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        template <typename MatrixType> struct is_ref_compatible
        {
            enum
            {
                value = is_ref_compatible_impl<
                    typename remove_all<MatrixType>::type>::value
            };
        };

        template <typename MatrixType,
                  bool MatrixFree =
                      !internal::is_ref_compatible<MatrixType>::value>
        ...
        public:
            ...
            {
                typedef typename ActualMatrixType::
    ...
    public:
        ...
        typedef
            typename internal::traits<Derived>::Preconditioner Preconditioner;
    ...
    public:
         ...
         * matrix \a A for further solving \c Ax=b problems.
         *
         * Currently, this function mostly calls analyzePattern on the
         * preconditioner. In the future we might, for instance, implement
         * column reordering for faster matrix vector products.
         */
        template <typename MatrixDerived>
         ...
         *
         * Currently, this function mostly initializes/computes the
         * preconditioner. In the future we might, for instance, implement
         * column reordering for faster matrix vector products.
         *
         * \warning this class stores a reference to the matrix A as well as
        ...
        {
            return (m_maxIterations < 0) ? 2 * matrix().cols()
                                         : m_maxIterations;
        }
        ...
        solveWithGuess(const MatrixBase<Rhs>& b, const Guess& x0) const
        {
            ...
            eigen_assert(derived().rows() == b.rows()
                         && "solve(): invalid number of rows of the right hand "
                            "side matrix b");
            return SolveWithGuess<Derived, Rhs, Guess>(derived(), b.derived(),
                                                       x0);
        }
                         ...
                         SparseMatrixBase<DestDerived>& aDest) const
        {
            ...
            // We do not directly fill dest because sparse expressions have to
            // be free of aliasing issue. For non square least-square problems,
            // b and dest might not have the same size whereas they might alias
            // each-other.
