...
public:
    ...
    inline SparseSelfAdjointView(const MatrixType& matrix)
        : m_matrix(matrix)
    {
     ...
     *
     * Note that it is faster to set alpha=0 than initializing the matrix to
     * zero and then keep the default value alpha=1.
     *
...
namespace internal
{
    ...
    struct traits<SparseSelfAdjointTimeDenseProduct<Lhs, Rhs, UpLo>>
        : traits<ProductBase<SparseSelfAdjointTimeDenseProduct<Lhs, Rhs, UpLo>,
                             Lhs, Rhs>>
    {
...
namespace internal
{
    ...
    struct traits<DenseTimeSparseSelfAdjointProduct<Lhs, Rhs, UpLo>>
        : traits<ProductBase<DenseTimeSparseSelfAdjointProduct<Lhs, Rhs, UpLo>,
                             Lhs, Rhs>>
    {
    };
} // namespace internal
                         ...
                         const typename MatrixType::Index* perm)
    {
        ...
        for (Index j = 0; j < size; ++j)
        {
            ...
            for (typename MatrixType::InnerIterator it(mat, j); it; ++it)
            {
                ...
                Index ip = perm ? perm[i] : i;
                count[DstUpLo == Lower ? (std::min)(ip, jp)
                                       : (std::max)(ip, jp)]++;
            }
        }
        dest._outerIndexPtr()[0] = 0;
        ...
        for (Index j = 0; j < size; ++j)
        {
            ...
            for (typename MatrixType::InnerIterator it(mat, j); it; ++it)
            {
                ...
                Index ip = perm ? perm[i] : i;
                Index k = count[DstUpLo == Lower ? (std::min)(ip, jp)
                                                 : (std::max)(ip, jp)]++;
                dest._innerIndexPtr()[k] =
                    DstUpLo == Lower ? (std::max)(ip, jp) : (std::min)(ip, jp);

                if ((DstUpLo == Lower && ip < jp)
                    || (DstUpLo == Upper && ip > jp))
