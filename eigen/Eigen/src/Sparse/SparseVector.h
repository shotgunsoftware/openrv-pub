...
public:
    ...
    inline SparseVector(Index size)
        : m_size(0)
    {
        resize(size);
    }
    ...
    //       (OtherDerived::Flags & RowMajorBit); if (needToTranspose)
    //       {
    //         // two passes algorithm:
    //         //  1 - compute the number of coeffs per dest inner vector
    //         //  2 - do the actual copy/eval
    //         // Since each coeff of the rhs has to be evaluated twice, let's
    //         evauluate it if needed typedef typename
    //         internal::nested<OtherDerived,2>::type OtherCopy; OtherCopy
    //         otherCopy(other.derived()); typedef typename
    //         internal::remove_all<OtherCopy>::type _OtherCopy;
    //
    ...
    //         return
    //         SparseMatrixBase<SparseMatrix>::operator=(other.derived());
    //       }
    //     }
