...
// be easily approximated by a polynomial centered on m=1 for stability.
            ...
            plog<Packet16f>(const Packet16f& _x)
        {
            ...
            _EIGEN_DECLARE_CONST_Packet16f(cephes_SQRTHF,
            ...
            Packet16f emm0 =
                _mm512_cvtepi32_ps(_mm512_srli_epi32((__m512i)x, 23));
            ...
            //   } else { x = x - 1.0; }
            __mmask16 mask =
                _mm512_cmp_ps_mask(x, p16f_cephes_SQRTHF, _CMP_LT_OQ);
            Packet16f tmp = _mm512_mask_blend_ps(mask, x, _mm512_setzero_ps());
            x = psub(x, p16f_1);
            e = psub(e,
                     _mm512_mask_blend_ps(mask, p16f_1, _mm512_setzero_ps()));
            x = padd(x, tmp);
            ...
            pexp<Packet16f>(const Packet16f& _x)
        {
            ...
            // m = floor(x/ln(2) + 0.5).
            Packet16f m =
                _mm512_floor_ps(pmadd(x, p16f_cephes_LOG2EF, p16f_half));

            // Get r = x - m*ln(2). Note that we can do this without losing more
            // than one ulp precision due to the FMA instruction.
...
// further reducing its effective latency.
            ...
            psqrt<Packet16f>(const Packet16f& _x)
        {
            ...
            __mmask16 non_zero_mask =
                _mm512_cmp_ps_mask(_x, p16f_flt_min, _CMP_GE_OQ);
            Packet16f x = _mm512_mask_blend_ps(
                non_zero_mask, _mm512_rsqrt14_ps(_x), _mm512_setzero_ps());

            // Do a single step of Newton's iteration.
            x = pmul(x, pmadd(neg_half, pmul(x, x), p16f_one_point_five));

            // Multiply the original _x by it's reciprocal square root to
        ...
        template <>
        EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS
            EIGEN_UNUSED Packet8d
            psqrt<Packet8d>(const Packet8d& _x)
        {
            _EIGEN_DECLARE_CONST_Packet8d(one_point_five, 1.5);
            _EIGEN_DECLARE_CONST_Packet8d(minus_half, -0.5);
            _EIGEN_DECLARE_CONST_Packet8d_FROM_INT64(dbl_min,
            ...
            // select only the inverse sqrt of positive normal inputs (denormals
            // are flushed to zero and cause infs as well).
            __mmask8 non_zero_mask =
                _mm512_cmp_pd_mask(_x, p8d_dbl_min, _CMP_GE_OQ);
            Packet8d x = _mm512_mask_blend_pd(
                non_zero_mask, _mm512_rsqrt14_pd(_x), _mm512_setzero_pd());
...
// computing the reciprocal square root in AVX-512.
            ...
            prsqrt<Packet16f>(const Packet16f& _x)
        {
            ...
            // select only the inverse sqrt of positive normal inputs (denormals
            // are flushed to zero and cause infs as well).
            __mmask16 le_zero_mask =
                _mm512_cmp_ps_mask(_x, p16f_flt_min, _CMP_LT_OQ);
            Packet16f x = _mm512_mask_blend_ps(
                le_zero_mask, _mm512_setzero_ps(), _mm512_rsqrt14_ps(_x));

            // Fill in NaNs and Infs for the negative/zero entries.
            __mmask16 neg_mask =
                _mm512_cmp_ps_mask(_x, _mm512_setzero_ps(), _CMP_LT_OQ);
            Packet16f infs_and_nans = _mm512_mask_blend_ps(
                neg_mask, p16f_nan,
                _mm512_mask_blend_ps(le_zero_mask, p16f_inf,
                                     _mm512_setzero_ps()));
            ...
            prsqrt<Packet8d>(const Packet8d& _x)
        {
            ...
            // select only the inverse sqrt of positive normal inputs (denormals
            // are flushed to zero and cause infs as well).
            __mmask8 le_zero_mask =
                _mm512_cmp_pd_mask(_x, p8d_dbl_min, _CMP_LT_OQ);
            Packet8d x = _mm512_mask_blend_pd(le_zero_mask, _mm512_setzero_pd(),
                                              _mm512_rsqrt14_pd(_x));

            // Fill in NaNs and Infs for the negative/zero entries.
            __mmask8 neg_mask =
                _mm512_cmp_pd_mask(_x, _mm512_setzero_pd(), _CMP_LT_OQ);
            Packet8d infs_and_nans =
                _mm512_mask_blend_pd(neg_mask, p8d_nan,
                                     _mm512_mask_blend_pd(le_zero_mask, p8d_inf,
                                                          _mm512_setzero_pd()));
