...
// to leverage AVX512 instructions.
        ...
        /* Proper support for integers is only provided by AVX2. In the
        meantime, we'll use SSE instructions and packets to deal with integers.
        ...
        */
                                           ...
                                           const Packet8f& c)
        {
            ...
            // clang stupidly generates a vfmadd213ps instruction plus some
            // vmovaps on registers, and gcc stupidly generates a vfmadd132ps
            // instruction, so let's enforce it to generate a vfmadd231ps
            // instruction since the most common use case is to accumulate the
        ...
        // Loads 4 floats from memory a returns the packet {a0, a0  a1, a1, a2,
        // a2, a3, a3}
        template <>
        ...
        // Loads 2 floats from memory a returns the packet {a0, a0  a0, a0, a1,
        // a1, a1, a1}
        template <>
        ...
        // instructions are available NOTE: for the record the following seems
        // to be slower: return _mm256_i32gather_ps(from,
        // _mm256_set1_epi32(stride), 4);
        template <>
        EIGEN_DEVICE_FUNC inline Packet8f
        pgather<float, Packet8f>(const float* from, Index stride)
        {
            return _mm256_set_ps(from[7 * stride], from[6 * stride],
                                 from[5 * stride], from[4 * stride],
                                 from[3 * stride], from[2 * stride],
        ...
        {
            return _mm256_set_pd(from[3 * stride], from[2 * stride],
                                 from[1 * stride], from[0 * stride]);
        }
        ...
        template <>
        EIGEN_DEVICE_FUNC inline void
        pscatter<double, Packet4d>(double* to, const Packet4d& from,
                                   Index stride)
        {
        ...
        template <> EIGEN_STRONG_INLINE Packet4d pabs(const Packet4d& a)
        {
            const Packet4d mask = _mm256_castsi256_pd(_mm256_setr_epi32(
                0xFFFFFFFF, 0x7FFFFFFF, 0xFFFFFFFF, 0x7FFFFFFF, 0xFFFFFFFF,
        ...
        {
            return predux(Packet4f(_mm_add_ps(_mm256_castps256_ps128(a),
        ...
        {
            return predux(Packet2d(_mm_add_pd(_mm256_castpd256_pd128(a),
        ...
        {
            return _mm_add_ps(_mm256_castps256_ps128(a),
                              _mm256_extractf128_ps(a, 1));
        }
        ...
        {
            static EIGEN_STRONG_INLINE void run(Packet8f& first,
                                                const Packet8f& second)
            {
                if (Offset == 1)
                {
                    ...
                    Packet8f tmp1 =
                        _mm256_permute_ps(first, _MM_SHUFFLE(0, 3, 2, 1));
                ...
                else if (Offset == 2)
                {
                    ...
                    Packet8f tmp1 =
                        _mm256_permute_ps(first, _MM_SHUFFLE(1, 0, 3, 2));
                    Packet8f tmp2 = _mm256_permute2f128_ps(tmp1, tmp1, 1);
                ...
                else if (Offset == 4)
                {
                    ...
                    Packet8f tmp1 =
                        _mm256_permute_ps(first, _MM_SHUFFLE(3, 2, 1, 0));
                    Packet8f tmp2 = _mm256_permute2f128_ps(tmp1, tmp1, 1);
        ...
        {
            static EIGEN_STRONG_INLINE void run(Packet4d& first,
        ...
        {
            __m256d T0 =
                _mm256_shuffle_pd(kernel.packet[0], kernel.packet[1], 15);
            __m256d T1 =
                _mm256_shuffle_pd(kernel.packet[0], kernel.packet[1], 0);
            __m256d T2 =
                _mm256_shuffle_pd(kernel.packet[2], kernel.packet[3], 15);
            __m256d T3 =
                _mm256_shuffle_pd(kernel.packet[2], kernel.packet[3], 0);
                                            ...
                                            const Packet8f& elsePacket)
        {
            ...
            const __m256 select = _mm256_set_ps(
                ifPacket.select[7], ifPacket.select[6], ifPacket.select[5],
