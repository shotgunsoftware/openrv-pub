...
namespace Eigen
{
    namespace internal
    {
        ...
        // parameters on which T itself depends. For example, traits<Map<const
        // T> > != traits<Map<T> >, but
        ...
        template <typename Derived> struct accessors_level
        {
            enum
            {
                ...
                value = has_direct_access
                            ? (has_write_access ? DirectWriteAccessors
                                                : DirectAccessors)
                            : (has_write_access ? WriteAccessors
                                                : ReadOnlyAccessors)
            };
        };
    ...
    template <typename _Scalar, int _Rows, int _Cols,
              int _Options =
                  ...
                  // slightly differently and this makes gcc 3.4.6 happy, but
                  // it's ugly. The error would only show up with
    ...
    namespace internal
    {
         ...
         * Products need their own evaluator with more template arguments
         * allowing for easier partial template specializations.
         */
        template <typename T,
                  int ProductTag = internal::product_type<typename T::Lhs,
                                                          typename T::Rhs>::ret,
                  typename LhsShape =
