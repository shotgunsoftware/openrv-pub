...
// might exist for more recent versions too.
    ...
    namespace internal
    {
         ...
         * - otherwise, global_math_functions_filtering_base<T>::type is a
         * typedef for T.
         *
         * How it's used:
         * To allow to defined the global math functions (like sin...) in
         * certain cases, like the Array expressions. When you do
         * sin(array1+array2), the object array1+array2 has a complicated
         * expression type, all what you want to know is that it inherits
         * ArrayBase. So we implement a partial specialization of sin_impl for
         * ArrayBase<Derived>. So we must make sure to use
         * sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>, otherwise
         * our partial specialization won't be used. How does sin know that?
         * That's exactly what global_math_functions_filtering_base tells it.
         *
         * How it's implemented:
         * SFINAE in the style of enable_if. Highly susceptible of breaking
         * compilers. With GCC, it sure does work, but if you replace the
         * typename dummy by an integer template parameter, it doesn't work
         * anymore!
         */
        ...
        struct global_math_functions_filtering_base<
            ...
            typename always_void<
                typename T::
                    Eigen_BaseClassForSpecializationOfGlobalMathFuncImpl>::type>
        {
            typedef
...
#define EIGEN_MATHFUNC_RETVAL(func, scalar)                             \
    typename Eigen::internal::func##_retval<                            \
        ...
        template <typename Scalar> struct abs2_impl
        {
            ...
            static inline RealScalar run(const Scalar& x)
            {
                return abs2_impl_default<Scalar,
                                         NumTraits<Scalar>::IsComplex>::run(x);
            }
        };
        ...
        template <typename Scalar>
        struct norm1_impl
            : norm1_default_impl<Scalar, NumTraits<Scalar>::IsComplex>
        {
        };
        ...
        // here, for once, we're plainly returning NewType: we don't want cast
        // to do weird things.
        ...
        template <typename Scalar> struct round_impl
        {
            static inline Scalar run(const Scalar& x)
            {
                ...
                return (x > Scalar(0)) ? floor(x + Scalar(0.5))
                                       : ceil(x - Scalar(0.5));
            }
        };
#endif
        ...
        {
            // fallback log1p implementation in case there is no log1p(Scalar)
            // function in namespace of Scalar, or that there is no suitable
            // std::log1p function available
            template <typename Scalar>
        ...
        template <typename ScalarX, typename ScalarY,
                  bool IsInteger = NumTraits<ScalarX>::IsInteger
        ...
        struct pow_impl
        {
            ...
            typedef typename ScalarBinaryOpTraits<
                ScalarX, ScalarY,
                internal::scalar_pow_op<ScalarX, ScalarY>>::ReturnType
                result_type;
        ...
        template <typename Scalar>
        struct random_impl
            : random_default_impl<Scalar, NumTraits<Scalar>::IsComplex,
        ...
        struct random_default_impl<Scalar, false, false>
        {
            ...
            {
                return run(Scalar(NumTraits<Scalar>::IsSigned ? -1 : 0),
                           Scalar(1));
            }
        };
        ...
        template <unsigned int n, int lower = 0,
                  ...
                  int selector =
                      meta_floor_log2_selector<n, lower, upper>::value>
        ...
        struct meta_floor_log2<n, lower, upper, meta_floor_log2_move_up>
        {
            ...
            {
                value = meta_floor_log2<
                    n, meta_floor_log2_selector<n, lower, upper>::middle,
                    upper>::value
            };
        };
        ...
        struct meta_floor_log2<n, lower, upper, meta_floor_log2_terminate>
        {
            enum
            {
                value = (n >= ((unsigned int)(1) << (lower + 1))) ? lower + 1
                                                                  : lower
            };
        };
        ...
        struct random_default_impl<Scalar, false, true>
        {
            static inline Scalar run(const Scalar& x, const Scalar& y)
            {
                typedef typename conditional<NumTraits<Scalar>::IsSigned,
                                             std::ptrdiff_t, std::size_t>::type
                    ScalarX;
                if (y < x)
                    return x;
                // the following difference might overflow on a 32 bits system,
                // but since y>=x the result converted to an unsigned long is
                // still correct.
            ...
            static inline Scalar run()
            {
                ...
                return run(Scalar(NumTraits<Scalar>::IsSigned ? -10 : 0),
                           Scalar(10));
#else
                enum
                {
                    ...
                    offset = NumTraits<Scalar>::IsSigned
                                 ? (1 << (EIGEN_PLAIN_ENUM_MIN(rand_bits,
                                                               scalar_bits)
                                          - 1))
                                 : 0
                };
...
#undef EIGEN_TMP_NOOPT_ATTRIB
    ...
    namespace numext
    {
         ...
         *
         * It is essentially equivalent to \code using std::sqrt; return
         * sqrt(x); \endcode, but slightly faster for float/double and some
         * compilers (e.g., gcc), thanks to specializations when SSE is enabled.
         *
    ...
    namespace internal
    {
        ...
        struct scalar_fuzzy_default_impl<Scalar, true, false>
        {
                                        ...
                                        const RealScalar& prec)
            {
                return numext::abs2(x - y)
                       <= numext::mini(numext::abs2(x), numext::abs2(y)) * prec
                              * prec;
            }
        };
        ...
        {
            return scalar_fuzzy_impl<Scalar>::template isMuchSmallerThan<
        ...
        isApproxOrLessThan(const Scalar& x, const Scalar& y,
                           const typename NumTraits<Scalar>::Real& precision =
                               NumTraits<Scalar>::dummy_precision())
        {
            return scalar_fuzzy_impl<Scalar>::isApproxOrLessThan(x, y,
                                                                 precision);
        }
