...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        struct functor_traits<scalar_constant_op<Scalar>>
        {
            enum
            {
                Cost = 0 /* as the constant value should be loaded in register
                            only once for the whole expression */
                ,
        ...
        {
            linspaced_op_impl(const Scalar& low, const Scalar& high,
                              Index num_steps)
                : m_low(low)
                , m_high(high)
            ...
            packetOp(IndexType i) const
            {
                ...
                if (m_flip)
                {
                    ...
                    Packet res = padd(pset1<Packet>(m_high),
                ...
                else
                {
                    ...
                    Packet res = padd(pset1<Packet>(m_low),
                                      pmul(pset1<Packet>(m_step), pi));
        ...
        // Forward declaration (we default to random access which does not
        // really give us a speed gain when using packet access but it allows to
        // use the functor in nested expressions).
            ...
            enum
  {
    ...
    IsRepeatable = true
        ...
        template <typename Scalar, typename PacketType> struct linspaced_op
        {
            ...
            const linspaced_op_impl<Scalar, PacketType,
                                    NumTraits<Scalar>::IsInteger>
                impl;
        };

        // Linear access is automatically determined from the operator()
        // prototypes available for the given functor. If it exposes an
        // operator()(i,j), then we assume the i and j coefficients are required
        // independently and linear access is not possible. In all other cases,
        // linear access is enabled. Users should not have to deal with this
        // structure.
