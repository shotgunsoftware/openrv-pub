...
namespace Eigen
{
    ...
    public:
        ...
        // errors when compiling test_stdvector with GCC 4.4.2. This looks like
        // a GCC bug though, so feel free to re-enable it if it's useful;
        // however notice that we already have the templated operator= above and
        // e.g. in MatrixBase we didn't have to add, in addition to templated
        // operator=, such a non-templated copy operator.
        ...
        {
            return Quaternion<Scalar>(Scalar(1), Scalar(0), Scalar(0),
                                      Scalar(0));
        }
        ...
        setFromTwoVectors(const MatrixBase<Derived1>& a,
...
#include EIGEN_QUATERNIONBASE_PLUGIN
     ...
     * \tparam _Options controls the memory alignment of the coefficients. Can
     * be \# AutoAlign or \# DontAlign. Default is AutoAlign.
     *
     * This class represents a quaternion \f$ w+xi+yj+zk \f$ that is a
     * convenient representation of orientations and rotations of objects in
     * three dimensions. Compared to other representations like Euler angles or
     * 3x3 matrices, quaternions offer the following advantages:
     ...
     *
     * \warning Operations interpreting the quaternion as rotation have
     * undefined behavior if the quaternion is not normalized.
     *
    ...
    public:
        ...
        typedef
            typename internal::traits<Quaternion>::Coefficients Coefficients;
...
#include EIGEN_QUATERNION_PLUGIN
     ...
     *
     * This is a specialization of class Map for Quaternion. This class allows
     * to view a 4 scalar memory buffer as an Eigen's Quaternion object.
     *
    ...
    public:
         ...
         *
         * The pointer \a coeffs must reference the four coefficients of
         * Quaternion in the following order:
         * \code *coeffs == {x, y, z, w} \endcode
         *
         * If the template parameter _Options is set to #Aligned, then the
         * pointer coeffs must be aligned. */
    ...
    namespace internal
    {
        ...
        struct quat_product
        {
            ...
            run(const QuaternionBase<Derived1>& a,
                const QuaternionBase<Derived2>& b)
            {
                return Quaternion<Scalar>(a.w() * b.w() - a.x() * b.x()
                                              - a.y() * b.y() - a.z() * b.z(),
                                          a.w() * b.x() + a.x() * b.w()
                                              + a.y() * b.z() - a.z() * b.y(),
                                          a.w() * b.y() + a.y() * b.w()
     ...
     * matrix and \a xpr is converted to a quaternion
     */
    ...
    QuaternionBase<Derived>::toRotationMatrix(void) const
    {
        ...
        // gcc 4.3 !!) if not inlined then the cost of the return by value is
        // huge ~ +35%, however, not inlining this function is an order of
        // magnitude slower, so it has to be inlined, and so the return by value
        // is not an issue
        Matrix3 res;
    ...
    /** \returns a random unit quaternion following a uniform distribution law
     * on SO(3)
     *
     ...
     * multiplicative inverse if the quaternion is normalized. The conjugate of
     * a quaternion represents the opposite rotation.
     *
     ...
     *
     * This represents an interpolation for a constant motion between \c *this
     * and \a other, see also http://en.wikipedia.org/wiki/Slerp.
     */
    template <class Derived>
    template <class OtherDerived>
    ...
    namespace internal
    {
        ...
        template <typename Other> struct quaternionbase_assign_impl<Other, 3, 3>
        {
                                                     ...
                                                     const Other& a_mat)
            {
                ...
                EIGEN_USING_STD_MATH(sqrt)
                // This algorithm comes from  "Quaternion Calculus and Fast
                // Animation", Ken Shoemake, 1987 SIGGRAPH course notes
                Scalar t = mat.trace();
                if (t > Scalar(0))
                {
                    t = sqrt(t + Scalar(1.0));
                ...
                else
                {
                    ...
                    q.coeffs().coeffRef(j) =
                        (mat.coeff(j, i) + mat.coeff(i, j)) * t;
                    q.coeffs().coeffRef(k) =
                        (mat.coeff(k, i) + mat.coeff(i, k)) * t;
                }
            }
        };
