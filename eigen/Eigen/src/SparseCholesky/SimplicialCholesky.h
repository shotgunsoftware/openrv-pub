...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        struct simplicial_cholesky_grab_input
        {
            ...
            static void run(const InputMatrixType& input,
     ...
     * prior to the factorization such that the factorized matrix is P A P^-1.
     *
    ...
    public:
        ...
        template <typename Stream> void dumpMemory(Stream& s)
        {
            ...
            s << "  L:        "
              << ((total +=
                   (m_matrix.cols() + 1) * sizeof(int)
                   + m_matrix.nonZeros() * (sizeof(int) + sizeof(Scalar)))
                  >> 20)
              << "Mb" << "\n";
            s << "  diag:     "
              << ((total += m_diag.size() * sizeof(Scalar)) >> 20) << "Mb"
              << "\n";
            s << "  tree:     "
              << ((total += m_parent.size() * sizeof(int)) >> 20) << "Mb"
              << "\n";
            s << "  nonzeros: "
              << ((total += m_nonZerosPerCol.size() * sizeof(int)) >> 20)
              << "Mb" << "\n";
            s << "  perm:     " << ((total += m_P.size() * sizeof(int)) >> 20)
              << "Mb" << "\n";
            s << "  perm^-1:  "
              << ((total += m_Pinv.size() * sizeof(int)) >> 20) << "Mb" << "\n";
            s << "  TOTAL:    " << (total >> 20) << "Mb" << "\n";
        }
     ...
     * that are selfadjoint and positive definite. The factorization allows for
     ...
     *
     * This class provides a LDL^T Cholesky factorizations without square root
     * of sparse matrices that are selfadjoint and positive definite. The
     * factorization allows for solving A.X = B where X and B can be either
     * dense or sparse.
     *
     * In order to reduce the fill-in, a symmetric permutation P is applied
     * prior to the factorization such that the factorized matrix is P A P^-1.
    ...
    public:
        ...
        Scalar determinant() const
        {
            ...
            {
                Scalar detL =
                    Diagonal<const CholMatrixType>(Base::m_matrix).prod();
                return numext::abs2(detL);
            }
        }
