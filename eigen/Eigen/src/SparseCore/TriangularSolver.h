...
namespace Eigen
{
            ...
            static void run(const Lhs& lhs, Rhs& other)
            {
                ...
                for (Index col = 0; col < other.cols(); ++col)
                {
                    for (Index i = 0; i < lhs.cols(); ++i)
                    {
                        ...
                        if (tmp != Scalar(0)) // optimization when other is
                                              // actually sparse
                        {
                            LhsIterator it(lhsEval, i);
                            while (it && it.index() < i)
                ...
                for (Index col = 0; col < other.cols(); ++col)
                {
                    for (Index i = lhs.cols() - 1; i >= 0; --i)
                    {
                                              ...
                                              // actually sparse
                        {
                            if (!(Mode & UnitDiag))
                            {
                                // TODO replace this by a binary search. make
                                // sure the binary search is safe for partially
                                // sorted elements
                                LhsIterator it(lhsEval, i);
                                while (it && it.index() != i)
                                    ++it;
                                                       ...
                                                       ColMajor>
        {
            ...
            static void run(const Lhs& lhs, Rhs& other)
            {
                ...
                for (Index col = 0; col < other.cols(); ++col)
                {
                    ...
                    {
                        ++count;
                        //         std::cerr << "fill " << it.index() << ", " <<
                        //         col << "\n"; std::cout << it.value() << "  ";
                        // FIXME use insertBack
                        res.insert(it.index(), col) = it.value();
                    }
                    //       std::cout << "tempVector.nonZeros() == " <<
                    //       int(count) << " / " << (other.rows()) << "\n";
                }
                res.finalize();
        ...
        SparseMatrixBase<OtherDerived>& other) const
    {
        ...
        internal::sparse_solve_triangular_sparse_selector<
            ExpressionType, OtherDerived, Mode>::run(derived()
                                                         .nestedExpression(),
                                                     other.derived());
