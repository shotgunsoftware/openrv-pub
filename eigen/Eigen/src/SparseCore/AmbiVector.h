...
namespace Eigen
{
    ...
    namespace internal
    {
        ...
        protected:
            ...
            {
                // if the size of the matrix is not too large, let's allocate a
                // bit more than needed such that we can handle dense vector
                // even in sparse mode.
                delete[] m_buffer;
                if (size < 1000)
                {
                    Index allocSize =
                        (size * sizeof(ListEl) + sizeof(Scalar) - 1)
                        / sizeof(Scalar);
            ...
            else
            {
                ...
                else
                {
                    ...
                    eigen_assert(i >= llElements[m_llCurrent].index
                                 && "you must call restart() before inserting "
                                    "an element with lower or equal index");
        ...
        public:
             ...
             * In practice, all coefficients having a magnitude smaller than \a
             * epsilon are skipped.
             */
            explicit Iterator(const AmbiVector& vec,
                ...
                : m_vector(vec)
            {
                ...
                else
                {
                    ...
                    {
                        m_cachedValue =
                            0; // this is to avoid a compilation warning
                        m_cachedIndex = -1;
                    }
                    else
                    {
                ...
                if (m_isDense)
                {
                    do
                    {
                        ++m_cachedIndex;
                             ...
                             && abs(m_vector.m_buffer[m_cachedIndex])
                                    <= m_epsilon);
                ...
                else
                {
                    ...
                    do
                    {
                        m_currentEl = llElements[m_currentEl].next;
                             ...
                             && abs(llElements[m_currentEl].value)
                                    <= m_epsilon);
                    if (m_currentEl < 0)
                    {
                        m_cachedIndex = -1;
        ...
        protected:
            ...
            StorageIndex
                m_currentEl; // the current element in sparse/linked-list mode
