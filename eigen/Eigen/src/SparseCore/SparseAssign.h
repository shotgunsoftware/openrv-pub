...
namespace Eigen
{
    ...
    {
        // by default sparse evaluation do not alias, so we can safely bypass
        // the generic call_assignment routine
        internal::Assignment<
    ...
    namespace internal
    {
        ...
        void assign_sparse_to_sparse(DstXprType& dst, const SrcXprType& src)
        {
            ...
            const Index outerEvaluationSize =
                (SrcEvaluatorType::Flags & RowMajorBit) ? src.rows()
                                                        : src.cols();
            if ((!transpose) && src.isRValue())
            {
                ...
                {
                    dst.startVec(j);
                    for (typename SrcEvaluatorType::InnerIterator it(
                             srcEvaluator, j);
                         it; ++it)
                    {
                        Scalar v = it.value();
                            ...
                            const Functor& func)
            {
                ...
                const Index outerEvaluationSize =
                    (internal::evaluator<SrcXprType>::Flags & RowMajorBit)
                        ...
                        : src.cols();
                for (Index j = 0; j < outerEvaluationSize; ++j)
                    for (typename internal::evaluator<SrcXprType>::InnerIterator
                             i(srcEval, j);
                         i; ++i)
                        func.assignCoeff(dstEval.coeffRef(i.row(), i.col()),
                                         i.value());
            }
        };
