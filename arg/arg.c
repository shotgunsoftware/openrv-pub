...
static char rcsid[] = "$Header: /home/src/cvsroot/src/pub/arg/arg.c,v 1.2 "
                      "2008/01/28 20:58:29 src Exp $";
...
Arg_form* arg_to_form1(va_list ap)
{
    ...
    for (prevform = 0; (s = va_arg(ap, char*)) != 0; prevform = form)
    {
        ...
        if (checkstr(s, "format string", prevs))
            return 0;
        ...
        if (checkstr(s, "doc string", form->format))
            return 0;
        form->doc = prevs = s;
...
Arg_form* arg_find_flag(char* arg, Arg_form* form)
{
    ...
    for (f = form; f; f = f->next)
    {
        if (f->type != ARG_REGULAR && f->type != ARG_NOP
            && str_eq(f->flag, arg))
            return f;
        if (f->type == ARG_SUBLISTFLAG)
        {
...
int arg_do(int ac, char** av, Arg_form* f)
{
    ...
    {
        fprintf(stderr, "warning: %d unused arg%s to %s: ", narg - used,
                narg - used > 1 ? "s" : "", av[-1]);
        for (i = used; i < narg; i++)
            fprintf(stderr, "%s ", av[i]);
        fprintf(stderr, "\n");
...
int nargs(int ac, char** av, Arg_form* f, int* skip)
{
    ...
    for (die = 0, i = 0; i < np && i < ac && !die;)
    {
        if (voracious)
            ...
            for (j = i + 1; !(mask >> j & 1); j++)
                ; /* go until we can stop */
    ...
    if (arg_debug)
    {
        fprintf(stderr, "    %s: requested %d, got %d args: ", flag, np,
                (int)(au - av0));
        for (j = 0; j < au - av0; j++)
    ...
    for (f = form; f; f = f->next)
    {
        ...
        if (arg_debug)
            fprintf(stderr, "   %d (%s) [%s][%s]%x (%s)\n", f->type, f->format,
                    f->flag, f->code, f->parammask, f->doc);
        if (f->type == ARG_SUBLISTFLAG)
        {
            fputs(bar, stderr);
