...
*/
int unzStringFileNameCompare(fileName1, fileName2, iCaseSensitivity) const
    char* fileName1;
const char* fileName2;
...
*/
unzFile unzOpen2(path, pzlib_filefunc_def) const char* path;
zlib_filefunc_def* pzlib_filefunc_def;
{
    unz_s us;
    unz_s* s;
...
unzFile unzOpen(path) const char* path;
{
...
/*
  ...
  return UNZ_OK if there is no problem.
*/
int unzGetCurrentFileInfo(file, pfile_info, szFileName, fileNameBufferSize,
...
int unzGetFilePos(file, file_pos)
unzFile file;
...
int unzGoToFilePos(file, file_pos)
unzFile file;
unz_file_pos* file_pos;
{
    unz_s* s;
...
int unzOpenCurrentFilePassword(file, password)
unzFile file;
const char* password;
{
...
int unzOpenCurrentFile2(file, method, level, raw)
unzFile file;
int* method;
int* level;
int raw;
...
unsigned len;
{
    ...
    if (ZREAD(pfile_in_zip_read_info->z_filefunc,
              pfile_in_zip_read_info->filestream, buf, read_now)
        != read_now)
        return UNZ_ERRNO;
