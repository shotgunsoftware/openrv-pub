...
yaml_get_version_string(void) { return YAML_VERSION_STRING; }
...
{
#ifdef USE_NEDMALLOC
    return ptr ? nedrealloc(ptr, size ? size : 1) : nedmalloc(size ? size : 1);
#else
...
{
//
// Avoid the use of strdup for allocating
// and doing a str copy on Windows this seems
// to cause periodic crashes with free().
// -- arkell
    ...
    len = strlen(str);
#ifdef USE_NEDMALLOC
    dup = (yaml_char_t*)nedmalloc(len + 1);
#else
    dup = (yaml_char_t*)malloc(len + 1);
#endif
    if (dup)
    {
        memcpy(dup, str, len);
...
yaml_queue_extend(void** start, void** head, void** tail, void** end)
{
    ...
    {
        void* new_start =
            yaml_realloc(*start, ((char*)*end - (char*)*start) * 2);
...
yaml_parser_delete(yaml_parser_t* parser)
{
    ...
    while (!STACK_EMPTY(parser, parser->tag_directives))
    {
        yaml_tag_directive_t tag_directive =
            POP(parser, parser->tag_directives);
...
yaml_sequence_start_event_initialize(yaml_event_t* event, yaml_char_t* anchor,
                                     yaml_char_t* tag, int implicit,
...
yaml_mapping_start_event_initialize(yaml_event_t* event, yaml_char_t* anchor,
                                    yaml_char_t* tag, int implicit,
