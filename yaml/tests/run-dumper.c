        ...
        case YAML_SEQUENCE_NODE:
                 ...
                 item < node->data.sequence.items.top; item++)
            {
                if (!yaml_document_append_sequence_item(
                        document_to, node - document_from->nodes.start + 1,
                        *item))
                    goto error;
            }
            break;
                  ...
                  yaml_document_t* document2, int index2)
{
    ...
    case YAML_SCALAR_NODE:
        ...
        if (strncmp((char*)node1->data.scalar.value,
                    (char*)node2->data.scalar.value, node1->data.scalar.length)
            != 0)
            return 0;
        break;
    case YAML_SEQUENCE_NODE:
    ...
    case YAML_MAPPING_NODE:
        if ((node1->data.mapping.pairs.top - node1->data.mapping.pairs.start)
            != (node2->data.mapping.pairs.top
                - node2->data.mapping.pairs.start))
            return 0;
        for (k = 0; k < (node1->data.mapping.pairs.top
                         - node1->data.mapping.pairs.start);
             k++)
        {
            if (!compare_nodes(
                    document1, node1->data.mapping.pairs.start[k].key,
                    document2, node2->data.mapping.pairs.start[k].key))
                return 0;
            if (!compare_nodes(
                    document1, node1->data.mapping.pairs.start[k].value,
...
int compare_documents(yaml_document_t* document1, yaml_document_t* document2)
{
         ...
         k++)
    {
        if ((strcmp((char*)document1->tag_directives.start[k].handle,
                    (char*)document2->tag_directives.start[k].handle)
             != 0)
            || (strcmp((char*)document1->tag_directives.start[k].prefix,
                       (char*)document2->tag_directives.start[k].prefix)
                != 0))
            return 0;
    }
    ...
    for (number = 1; number < argc; number++)
    {
        ...
        while (!done)
        {
            ...
            if (!done)
            {
                ...
                assert(
                    copy_document(&(documents[document_number++]), &document));
                assert(
        ...
        if (!error)
        {
            ...
            while (!done)
            {
                ...
                {
                    assert(
                        compare_documents(documents + count, &document)
                        || print_output(argv[number], buffer, written, count));
                    count++;
                }
                yaml_document_delete(&document);
