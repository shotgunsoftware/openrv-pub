...
int copy_event(yaml_event_t* event_to, yaml_event_t* event_from)
{
    ...
    case YAML_SCALAR_EVENT:
        return yaml_scalar_event_initialize(
            event_to, event_from->data.scalar.anchor,
            event_from->data.scalar.tag, event_from->data.scalar.value,
...
int compare_events(yaml_event_t* event1, yaml_event_t* event2)
{
    ...
    case YAML_DOCUMENT_START_EVENT:
        ...
        {
            if ((strcmp(
                     (char*)event1->data.document_start.tag_directives.start[k]
                         .handle,
                     (char*)event2->data.document_start.tag_directives.start[k]
                         .handle)
                 != 0)
                || (strcmp((char*)event1->data.document_start.tag_directives
                               .start[k]
                               .prefix,
                           (char*)event2->data.document_start.tag_directives
                               .start[k]
                               .prefix)
                    != 0))
                return 0;
        ...
        /* if (event1->data.document_start.implicit !=
           event2->data.document_start.implicit) return 0; */
        return 1;
        ...
        if ((event1->data.scalar.anchor && !event2->data.scalar.anchor)
            ...
            || (event1->data.scalar.anchor && event2->data.scalar.anchor
                && strcmp((char*)event1->data.scalar.anchor,
                          (char*)event2->data.scalar.anchor)
                       != 0))
            return 0;
        ...
        if ((event1->data.sequence_start.anchor
            ...
            || (event1->data.sequence_start.anchor
                ...
                && strcmp((char*)event1->data.sequence_start.anchor,
                          (char*)event2->data.sequence_start.anchor)
                       != 0))
            return 0;
        if ((event1->data.sequence_start.tag
             && !event2->data.sequence_start.tag)
            || (!event1->data.sequence_start.tag
                && event2->data.sequence_start.tag)
            || (event1->data.sequence_start.tag
                && event2->data.sequence_start.tag
                && strcmp((char*)event1->data.sequence_start.tag,
                          (char*)event2->data.sequence_start.tag)
                       != 0))
            return 0;
...
int main(int argc, char* argv[])
{
    ...
    for (number = 1; number < argc; number++)
    {
        ...
        printf("[%d] Parsing, emitting, and parsing again '%s': ", number,
               argv[number]);
        fflush(stdout);
