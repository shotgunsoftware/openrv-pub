...
int main(int argc, char* argv[])
{
    ...
    if (!yaml_document_start_event_initialize(&output_event, NULL, NULL, NULL,
                                              0))
        goto event_error;
    if (!yaml_emitter_emit(&emitter, &output_event))
        goto emitter_error;
    ...
    if (!yaml_sequence_start_event_initialize(&output_event, NULL,
    ...
    while (!done)
    {
        ...
        if (!yaml_mapping_start_event_initialize(&output_event, NULL,
        ...
        case YAML_STREAM_START_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "type", -1, 1,
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "STREAM-START", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (input_event.data.stream_start.encoding)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str",
                        "encoding", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
        ...
        case YAML_STREAM_END_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "STREAM-END",
                    -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
        ...
        case YAML_DOCUMENT_START_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "DOCUMENT-START", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (input_event.data.document_start.version_directive)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "version",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                ...
                if (!yaml_mapping_start_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:map", 1,
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "major",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:int", number,
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "minor",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                ...
                != input_event.data.document_start.tag_directives.end)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "tags",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                ...
                if (!yaml_sequence_start_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:seq", 1,
                        YAML_BLOCK_SEQUENCE_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
                     ...
                     tag++)
                {
                    ...
                    if (!yaml_scalar_event_initialize(
                            &output_event, NULL, "tag:yaml.org,2002:str",
                            "handle", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                        goto event_error;
                    if (!yaml_emitter_emit(&emitter, &output_event))
                        goto emitter_error;
                    ...
                    if (!yaml_scalar_event_initialize(
                            &output_event, NULL, "tag:yaml.org,2002:str",
                            "prefix", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                        goto event_error;
                    if (!yaml_emitter_emit(&emitter, &output_event))
                        goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "implicit",
                    -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.document_start.implicit ? "true"
                                                              : "false"),
        ...
        case YAML_DOCUMENT_END_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "DOCUMENT-END", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.document_end.implicit ? "true" : "false"),
                    -1, 1, 0, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
        ...
        case YAML_ALIAS_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "ALIAS", -1,
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "anchor", -1,
                    1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
        ...
        case YAML_SCALAR_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "SCALAR", -1,
                    1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (input_event.data.scalar.anchor)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "anchor",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
            ...
            if (input_event.data.scalar.tag)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "tag", -1,
                        1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "value", -1,
                    1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    ...
                    input_event.data.scalar.length, 0, 1,
                    YAML_DOUBLE_QUOTED_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "plain", -1,
                    1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.scalar.plain_implicit ? "true" : "false"),
                    -1, 1, 0, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "non-plain",
                    -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.scalar.quoted_implicit ? "true"
                                                             : "false"),
                    -1, 1, 0, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
            ...
            if (input_event.data.scalar.style)
            {
                ...
                if (!yaml_scalar_event_initialize(
                        &output_event, NULL, "tag:yaml.org,2002:str", "style",
                        -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                    goto event_error;
                if (!yaml_emitter_emit(&emitter, &output_event))
                    goto emitter_error;
        ...
        case YAML_SEQUENCE_START_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "SEQUENCE-START", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.sequence_start.implicit ? "true"
                                                              : "false"),
                    -1, 1, 0, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
        ...
        case YAML_SEQUENCE_END_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "SEQUENCE-END", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
        ...
        case YAML_MAPPING_START_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str",
                    "MAPPING-START", -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:bool",
                    (input_event.data.mapping_start.implicit ? "true"
                                                             : "false"),
                    -1, 1, 0, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
        ...
        case YAML_MAPPING_END_EVENT:
            ...
            if (!yaml_scalar_event_initialize(
                    &output_event, NULL, "tag:yaml.org,2002:str", "MAPPING-END",
                    -1, 1, 1, YAML_PLAIN_SCALAR_STYLE))
                goto event_error;
            if (!yaml_emitter_emit(&emitter, &output_event))
                goto emitter_error;
