...
int main(int argc, char* argv[])
{
    ...
    while (!done)
    {
        ...
        if (!properties)
            goto document_error;
        if (!yaml_document_append_sequence_item(&output_document, root,
                                                properties))
            goto document_error;
        ...
        case YAML_STREAM_START_EVENT:
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "type", -1,
                                           YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value =
                yaml_document_add_scalar(&output_document, NULL, "STREAM-START",
            ...
            if (input_event.data.stream_start.encoding)
            {
                ...
                key =
                    yaml_document_add_scalar(&output_document, NULL, "encoding",
                                             -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
        ...
        case YAML_STREAM_END_EVENT:
            ...
            if (!key)
                goto document_error;
            value =
                yaml_document_add_scalar(&output_document, NULL, "STREAM-END",
                                         -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_DOCUMENT_START_EVENT:
            ...
            if (input_event.data.document_start.version_directive)
            {
                ...
                key =
                    yaml_document_add_scalar(&output_document, NULL, "version",
                                             -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                ...
                key = yaml_document_add_scalar(&output_document, NULL, "major",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                ...
                if (!value)
                    goto document_error;
                if (!yaml_document_append_mapping_pair(&output_document, map,
                                                       key, value))
                    goto document_error;

                /* Add 'minor': <number>. */

                key = yaml_document_add_scalar(&output_document, NULL, "minor",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                ...
                != input_event.data.document_start.tag_directives.end)
            {
                ...
                key = yaml_document_add_scalar(&output_document, NULL, "tags",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                     ...
                     tag++)
                {
                    ...
                    if (!key)
                        goto document_error;
                    value = yaml_document_add_scalar(
                        &output_document, NULL, tag->handle, -1,
                    ...
                    if (!key)
                        goto document_error;
                    value = yaml_document_add_scalar(
                        &output_document, NULL, tag->prefix, -1,
                        YAML_DOUBLE_QUOTED_SCALAR_STYLE);
                    if (!value)
                        goto document_error;
                    if (!yaml_document_append_mapping_pair(&output_document,
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "implicit",
                                           -1, YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.document_start.implicit ? "true" : "false"),
                -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_DOCUMENT_END_EVENT:
            ...
            if (!key)
                goto document_error;
            value =
                yaml_document_add_scalar(&output_document, NULL, "DOCUMENT-END",
                                         -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
            ...
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.document_end.implicit ? "true" : "false"), -1,
                YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_ALIAS_EVENT:
            ...
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(&output_document, NULL, "ALIAS",
                                             -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "anchor", -1,
                                           YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(&output_document, NULL,
        ...
        case YAML_SCALAR_EVENT:
            ...
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(&output_document, NULL, "SCALAR",
                                             -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
            ...
            {
                key = yaml_document_add_scalar(&output_document, NULL, "anchor",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                value = yaml_document_add_scalar(
                    &output_document, NULL, input_event.data.scalar.anchor, -1,
                    YAML_DOUBLE_QUOTED_SCALAR_STYLE);
                if (!value)
                    goto document_error;
                if (!yaml_document_append_mapping_pair(&output_document,
            ...
            {
                key = yaml_document_add_scalar(&output_document, NULL, "tag",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                value = yaml_document_add_scalar(
                    &output_document, NULL, input_event.data.scalar.tag, -1,
                    YAML_DOUBLE_QUOTED_SCALAR_STYLE);
                if (!value)
                    goto document_error;
                if (!yaml_document_append_mapping_pair(&output_document,
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "value", -1,
                                           YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(&output_document, NULL,
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "version",
                                           -1, YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            map = yaml_document_add_mapping(&output_document, NULL,
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "plain", -1,
                                           YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.scalar.plain_implicit ? "true" : "false"), -1,
                YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            ...
            key = yaml_document_add_scalar(&output_document, NULL, "quoted", -1,
                                           YAML_PLAIN_SCALAR_STYLE);
            if (!key)
                goto document_error;
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.scalar.quoted_implicit ? "true" : "false"),
                -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, map, key,
            ...
            if (input_event.data.scalar.style)
            {
                ...
                key = yaml_document_add_scalar(&output_document, NULL, "style",
                                               -1, YAML_PLAIN_SCALAR_STYLE);
                if (!key)
                    goto document_error;
                value = yaml_document_add_scalar(
        ...
        case YAML_SEQUENCE_START_EVENT:
            ...
            if (input_event.data.sequence_start.tag)
            {
                ...
                if (!key)
                    goto document_error;
                value = yaml_document_add_scalar(
                    &output_document, NULL, input_event.data.sequence_start.tag,
            ...
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.sequence_start.implicit ? "true" : "false"),
                -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_SEQUENCE_END_EVENT:
            ...
            if (!key)
                goto document_error;
            value =
                yaml_document_add_scalar(&output_document, NULL, "SEQUENCE-END",
                                         -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_MAPPING_START_EVENT:
            ...
            if (input_event.data.mapping_start.tag)
            {
                ...
                if (!key)
                    goto document_error;
                value = yaml_document_add_scalar(
                    &output_document, NULL, input_event.data.mapping_start.tag,
                    -1, YAML_DOUBLE_QUOTED_SCALAR_STYLE);
                if (!value)
                    goto document_error;
                if (!yaml_document_append_mapping_pair(&output_document,
            ...
            value = yaml_document_add_scalar(
                &output_document, YAML_BOOL_TAG,
                (input_event.data.mapping_start.implicit ? "true" : "false"),
                -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
        ...
        case YAML_MAPPING_END_EVENT:
            ...
            if (!key)
                goto document_error;
            value =
                yaml_document_add_scalar(&output_document, NULL, "MAPPING-END",
                                         -1, YAML_PLAIN_SCALAR_STYLE);
            if (!value)
                goto document_error;
            if (!yaml_document_append_mapping_pair(&output_document, properties,
...
document_error:
    ...
    fprintf(stderr,
            "Memory error: Not enough memory for creating a document\n");
