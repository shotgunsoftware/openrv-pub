...
#define YAML_DECLARE(type) type
     ...
     * @c "X.Y.Z", where @c X is the major version number, @c Y is a minor
     * version number, and @c Z is the patch version number.
     */
     ...
     *
     * The @a implicit argument is considered as a stylistic parameter and may
     ...
     *                                          list.
     * @param[in]       implicit                If the document start indicator
     * is implicit.
     *
     * @returns @c 1 if the function succeeded, @c 0 on error.
     */
    ...
    YAML_DECLARE(int)
    yaml_scalar_event_initialize(yaml_event_t* event, yaml_char_t* anchor,
...
#define YAML_DEFAULT_MAPPING_TAG YAML_MAP_TAG
     ...
     * @param[in]       tag_directives_end      The end of the %TAG directives
     *                                          list.
     * @param[in]       start_implicit          If the document start indicator
     * is implicit.
    ...
    YAML_DECLARE(int)
    yaml_document_add_scalar(yaml_document_t* document, yaml_char_t* tag,
    ...
    YAML_DECLARE(int)
    yaml_document_add_sequence(yaml_document_t* document, yaml_char_t* tag,
                               yaml_sequence_style_t style);
    ...
    YAML_DECLARE(int)
    yaml_document_add_mapping(yaml_document_t* document, yaml_char_t* tag,
                              yaml_mapping_style_t style);
     ...
     *
     * The read handler is called when the parser needs to read more bytes from
     * the source.  The handler should write not more than @a size bytes to the
     * @a buffer.  The number of written bytes should be set to the @a length
     * variable.
     *
     * @param[in,out]   data        A pointer to an application data specified
     * by yaml_parser_set_input().
     * @param[out]      buffer      The buffer to write the data from the
     * source.
     * @param[in]       size        The size of the buffer.
     * @param[out]      size_read   The actual number of bytes read from the
     * source.
     *
     * @returns On success, the handler should return @c 1.  If the handler
     * failed, the returned value should be @c 0.  On EOF, the handler should
     * set the
     ...
     * the calls of yaml_parser_scan() or yaml_parser_load(). Doing this will
     * break the parser.
     *
     * @param[in,out]   parser      A parser object.
     * @param[out]      event       An empty event object.
     ...
     * the calls of yaml_parser_scan() or yaml_parser_parse(). Doing this will
     * break the parser.
     *
     * @param[in,out]   parser      A parser object.
     * @param[out]      document    An empty document object.
     ...
     *
     * The write handler is called when the emitter needs to flush the
     * accumulated characters to the output.  The handler should write @a size
     * bytes of the
     * @a buffer to the output.
     *
     * @param[in,out]   data        A pointer to an application data specified
     * by yaml_emitter_set_output().
     * @param[in]       buffer      The buffer with bytes to be written.
     * @param[in]       size        The size of the buffer.
     *
     * @returns On success, the handler should return @c 1.  If the handler
     * failed, the returned value should be @c 0.
     */
     ...
     *
     * All members are internal.  Manage the structure using the @c
     * yaml_emitter_ family of functions.
     */
     ...
     *
     * This function creates a new emitter object.  An application is
     * responsible for destroying the object using the yaml_emitter_delete()
     * function.
     *
     ...
     * Set a string output.
     *
     * The emitter will write the output characters to the @a output buffer of
     * the size @a size.  The emitter will set @a size_written to the number of
     * written bytes.  If the buffer is smaller than required, the emitter
     * produces the YAML_WRITE_ERROR error.
     *
     * @param[in,out]   emitter         An emitter object.
     * @param[in]       output          An output buffer.
     * @param[in]       size            The buffer size.
     * @param[in]       size_written    The pointer to save the number of
     * written bytes.
     */
     ...
     * @param[in]       data        Any application data for passing to the
     * write handler.
     */
     ...
     * The emitter takes the responsibility for the event object and destroys
     * its content after it is emitted. The event object is destroyed even if
     * the function fails.
     *
     * @param[in,out]   emitter     An emitter object.
     * @param[in,out]   event       An event object.
     ...
     * it is emitted. The document object is destroyedeven if the function
     * fails.
     *
     * @param[in,out]   emitter     An emitter object.
     * @param[in,out]   document    A document object.
