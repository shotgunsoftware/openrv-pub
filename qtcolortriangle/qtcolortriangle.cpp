...
struct Vertex
{
    ...
    Vertex(const QColor& c, const QPointF& p)
        : color(
              DoubleColor((double)c.red(), (double)c.green(), (double)c.blue()))
        , point(p)
    {
    }
};
...
/*! \reimp
 */
QSize QtColorTriangle::sizeHint() const { return QSize(100, 100); }

/*!
    Forces the triangle widget to always be square. Returns the value
    \a w.
*/
int QtColorTriangle::heightForWidth(int w) const { return w; }
...
void QtColorTriangle::genBackground()
{
    ...
    QRectF innerRadiusRect(bg.rect().center().x() - innerRadius,
                           bg.rect().center().y() - innerRadius,
                           innerRadius * 2 + 1, innerRadius * 2 + 1);
    QRectF outerRadiusRect(bg.rect().center().x() - outerRadius,
    ...
    for (int f = 0; f <= 5760; f += 20)
    {
        ...
        color.setHsv(int((f / 5760.0) * 360.0), 128 + (255 - value) / 2,
                     255 - (255 - value) / 4);
        p.setPen(QPen(color, penThickness));
        p.drawArc(innerRadiusRect, 1440 - f, 20);

        color.setHsv(int((f / 5760.0) * 360.0), 128 + value / 2,
                     255 - value / 4);
        p.setPen(QPen(color, penThickness));
...
void QtColorTriangle::paintEvent(QPaintEvent* e)
{
    ...
    painter.drawEllipse((int)(pd.x() - ellipseSize / 2.0),
                        (int)(pd.y() - ellipseSize / 2.0), ellipseSize,
                        ellipseSize);
 ...
 */
QColor QtColorTriangle::color() const { return curColor; }
...
Returns a * a.
 */
inline double qsqr(double a) { return a * a; }

/*! \internal

Returns the length of the vector (x,y).
 */
inline double vlen(double x, double y) { return sqrt(qsqr(x) + qsqr(y)); }
...
*/
static bool pointAbovePoint(double x, double y, double px, double py, double ax,
...
if (ax,ay) to (bx,by) describes a line, and (x,y) is a point on
    ...
    and (bx,by).
*/
static int pointInLine(double x, double y, double ax, double ay, double bx,
                       double by)
{
    if (ax > bx)
    {
...
*/
QPointF QtColorTriangle::movePointToTriangle(double x, double y,
                                             ...
                                             const Vertex& c) const
{
    ...
    if (angleBetweenAngles(angleP, angleA, angleB))
    {
        ...
        double p0x = a.point.x()
                     + ((b.point.x() - a.point.x()) / vlen(v2xB, v2yB))
                           * cos(alphaA) * pdist;
        double p0y = a.point.y()
                     + ((b.point.y() - a.point.y()) / vlen(v2xB, v2yB))
                           * cos(alphaA) * pdist;

        // If (x,y) is above the a-b line, which basically means it's
        // outside the triangle, then return its projection onto a-b.
        if (pointAbovePoint(x, y, p0x, p0y, a.point.x(), a.point.y(),
                            b.point.x(), b.point.y()))
        {
            // If the projection is "outside" a, return a. If it is
            // outside b, return b. Otherwise return the projection.
            int n = pointInLine(p0x, p0y, a.point.x(), a.point.y(), b.point.x(),
                                b.point.y());
            if (n < 0)
                return a.point;
            else if (n > 0)
    ...
    else if (angleBetweenAngles(angleP, angleB, angleC))
    {
        ...
        // the length of all edges is always > 0
        double p0x = b.point.x()
                     + ((c.point.x() - b.point.x()) / vlen(v2xC, v2yC))
                           * cos(alphaB) * pdist;
        double p0y = b.point.y()
                     + ((c.point.y() - b.point.y()) / vlen(v2xC, v2yC))
                           * cos(alphaB) * pdist;

        if (pointAbovePoint(x, y, p0x, p0y, b.point.x(), b.point.y(),
                            c.point.x(), c.point.y()))
        {
            int n = pointInLine(p0x, p0y, b.point.x(), b.point.y(), c.point.x(),
                                c.point.y());
            if (n < 0)
                return b.point;
            else if (n > 0)
    ...
    else if (angleBetweenAngles(angleP, angleC, angleA))
    {
        ...
        // the length of all edges is always > 0
        double p0x = c.point.x()
                     + ((a.point.x() - c.point.x()) / vlen(v2xA, v2yA))
                           * cos(alphaC) * pdist;
        double p0y = c.point.y()
                     + ((a.point.y() - c.point.y()) / vlen(v2xA, v2yA))
                           * cos(alphaC) * pdist;

        if (pointAbovePoint(x, y, p0x, p0y, c.point.x(), c.point.y(),
                            a.point.x(), a.point.y()))
        {
            int n = pointInLine(p0x, p0y, c.point.x(), c.point.y(), a.point.x(),
                                a.point.y());
            if (n < 0)
                return c.point;
            else if (n > 0)
