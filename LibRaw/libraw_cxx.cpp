...
void scale_colors_thread_func (LIBRAWThreadData *td)
{
    int h = td->height;
    int w = td->width * 4;
    ushort *image = (ushort *) td->ptr1;
    const float* scale_mul = (const float *) td->ptr2;
    const unsigned* cblack = (const unsigned *) td->ptr3;

    if(cblack[0] || cblack[1] || cblack[2] || cblack[3])
    {
        for (int y=td->topStart; y < h; y += td->topIncr)
        {
            int rowOffset = y * w;
            for (int x=0; x < w; ++x)
            {
...
// Excluded histogram evaluation because it is not important for
// for raw to linear light conversion; we want it to fast as possible - Arkell.
void convert_to_rgb_thread_func (LIBRAWThreadData *td)
{
    int h = td->height;
    int w = td->width * 4;
    int colors = td->colors;
    ushort *image = (ushort *) td->ptr1;
    const float *out_cam = (const float *) td->ptr2;
    float pixelRGB[4];
    for (int y=td->topStart; y < h; y += td->topIncr)
    {
        int rowOffset = y * w;
        for (int x=0; x < w; x += 4)
        {
            int i = rowOffset + x;
            ushort *img = image + i;
            pixelRGB[0] = pixelRGB[1] = pixelRGB[2] = 0.0f;
           int c;
            for(c=0; c < colors; c++) {
                pixelRGB[0] += out_cam[c] * img[c];
                pixelRGB[1] += out_cam[4+c] * img[c];
                pixelRGB[2] += out_cam[8+c] * img[c];
            }
              for(c=0; c < 3; c++) {
                // Correct if channel has -ve values.
                // we set to the smallest ushort value.
                // -Arkell
               if (pixelRGB[c] < 0.0f) pixelRGB[c] = 1.0f;
               img[c] = OUT_OF_GAMUT_CLIP((int) pixelRGB[c]);
            }
        }
    }
}
...
void LibRaw::convert_to_rgb_loop(float out_cam[3][4])
{
  if (_nthreads > 1)
  {
    ...
    std::vector< LIBRAWThreadData > td(_nthreads);

    for (int i=0; i < _nthreads; ++i)
    {
        td[i].topStart    = i;
        td[i].topIncr     = _nthreads;
        td[i].width       = S.width;
        td[i].height      = S.height;
        td[i].colors      = imgdata.idata.colors;
        td[i].ptr1        = (void *) imgdata.image[0];
        td[i].ptr2        = (void *) out_cam[0];
        workerThread[i] = new boost::thread(convert_to_rgb_thread_func, &td[i]);
    }
  ...
  for (img=imgdata.image[0], row=0; row < S.height; row++)
    for (col=0; col < S.width; col++, img+=4) {
      if (!libraw_internal_data.internal_output_params.raw_color) {
        ...
        /* Tweak patch - Arkell */
        for(c=0;c<3;c++) img[c] = OUT_OF_GAMUT_CLIP((int) out[c]);
      }
...
void LibRaw::scale_colors_loop(float scale_mul[4])
{
  if (_nthreads > 1)
  {
    ...
    for (int i=0; i < _nthreads; ++i)
    {
        ...
        td[i].topIncr     = _nthreads;
        td[i].width       = S.iwidth;
        td[i].height      = S.iheight;
        td[i].ptr1        = (void *) imgdata.image[0];
        td[i].ptr2        = (void *) scale_mul;
        td[i].ptr3        = (void *) C.cblack;
        workerThread[i] = new boost::thread(scale_colors_thread_func, &td[i]);
    }
