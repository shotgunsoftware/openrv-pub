...
#if !defined(SOLARISDL) && !defined(IRIX5) && !defined(MSWIN32)   \
    && !defined(MSWINCE) && !defined(CYGWIN32)                    \
    && !(defined(ALPHA) && defined(OSF1)) && !defined(HPUX)       \
    && !(defined(LINUX) && defined(__ELF__)) && !defined(AIX)     \
    && !defined(SCO_ELF) && !defined(DGUX)                        \
...
#define ElfW(type) Elf64_##type
 ...
 STATIC struct link_map* GC_FirstDLOpenedLinkMap(void)
 {
...
#define MAPS_BUF_SIZE (32 * 1024)
 ...
 STATIC word GC_register_map_entries(char* maps)
 {
     ...
     {
         buf_ptr =
             GC_parse_map_entry(buf_ptr, &start, &end, &prot, &maj_dev, 0);
         if (buf_ptr == NULL)
             return 1;
         if (prot[1] == 'w')
         {
                    ...
                    && start < end)
             {
                 if ((char*)start < GC_our_memory[i].hs_start)
                     GC_add_roots_inner((char*)start, GC_our_memory[i].hs_start,
                                        TRUE);
                 start = GC_our_memory[i].hs_start + GC_our_memory[i].hs_bytes;
                 ++i;
             }
 ...
 GC_INNER GC_bool GC_register_main_static_data(void) { return FALSE; }
...
#define MAX_LOAD_SEGS MAX_ROOT_SETS
 ...
 STATIC int GC_register_dynlib_callback(struct dl_phdr_info* info, size_t size,
                                        void* ptr)
 {
     const ElfW(Phdr) * p;
     ptr_t start, end;
     ...
     for (i = 0; i < (int)info->dlpi_phnum; i++, p++)
     {
         ...
         case PT_GNU_RELRO:
             /* This entry is known to be constant and will eventually be
                remapped read-only.  However, the address range covered by this
                entry is typically a subset of a previously encountered `LOAD'
                segment, so we need to exclude it.  */
             {
                 int j;
                 ...
                 {
                     if (start >= load_segs[j].start
                         && start < load_segs[j].end)
                     {
                         if (load_segs[j].start2 != 0)
                         {
                             WARN("More than one GNU_RELRO segment per load "
                                  "seg\n",
                                  0);
                         }
                         else
 ...
 STATIC GC_bool GC_register_dynamic_libraries_dl_iterate_phdr(void)
 {
...
#ifdef PT_GNU_RELRO
     {
         ...
         if (!excluded_segs)
         {
             GC_exclude_static_roots_inner(
                 (ptr_t)load_segs, (ptr_t)load_segs + sizeof(load_segs));
             excluded_segs = TRUE;
         }
     }
#endif
     ...
     if (did_something)
     {
         ...
         for (i = 0; i < n_load_segs; ++i)
         {
             ...
             {
                 GC_add_roots_inner(load_segs[i].start2, load_segs[i].end2,
                                    TRUE);
             }
         }
#endif
...
#define MA_PHYS 0
     ...
     if (needed_sz >= current_sz)
     {
         ...
         addr_map =
             (prmap_t*)GC_scratch_alloc((word)current_sz * sizeof(prmap_t));
         if (addr_map == NULL)
     ...
     if (GC_n_heap_sects > 0)
     {
         ...
         if (heap_end < GC_scratch_last_end_ptr)
     ...
     for (i = 0; i < needed_sz; i++)
     {
         ...
         if ((flags
              & (MA_BREAK | MA_STACK | MA_PHYS | MA_FETCHOP | MA_NOTCACHED))
             != 0)
             goto irrelevant;
 ...
 GC_INNER void GC_register_dynamic_libraries(void)
 {
     ...
     while (p < GC_sysinfo.lpMaximumApplicationAddress)
     {
         ...
         if (result == 0)
         {
             ...
             new_limit = (char*)(((DWORD)p + GC_sysinfo.dwAllocationGranularity)
                                 & ~(GC_sysinfo.dwAllocationGranularity - 1));
         }
         else
#endif
...
#include <loader.h>
 ...
 GC_INNER void GC_register_dynamic_libraries(void)
 {
     ...
     while (TRUE)
     {
         ...
         status = ldr_inq_module(mypid, moduleid, &moduleinfo, moduleinfosize,
                                 &modulereturnsize);
         if (status != 0)
         ...
         for (region = 0; region < moduleinfo.lmi_nregion; region++)
         {
             ...
             GC_add_roots_inner(
                 ...
                 (char*)regioninfo.lri_mapaddr + regioninfo.lri_size, TRUE);
         }
     }
 }
#endif
...
#include <dl.h>
 ...
 GC_INNER void GC_register_dynamic_libraries(void)
 {
     ...
     while (TRUE)
     {
          ...
          */
         GC_add_roots_inner((char*)shl_desc->dstart, (char*)shl_desc->dend,
                            TRUE);
...
#include <mach-o/getsect.h>
 ...
 STATIC const char* GC_dyld_add_sect_fmts[] = {
     "__bss%u", "__pu_bss%u", "__zo_bss%u", "__zo_pu_bss%u", NULL};
...
#define L2_MAX_OFILE_ALIGNMENT 15
 ...
 STATIC void GC_dyld_image_add(const struct GC_MACH_HEADER* hdr, intptr_t slide)
 {
          ...
          i++)
     {
         ...
         {
#ifdef DARWIN_DEBUG
             GC_log_printf("Adding section __DATA,%s at %p-%p (%lu bytes) from "
                           "image %s\n",
...
GC_INNER void GC_register_dynamic_libraries(void)
{
 ...
 /* Do we need to separately register the main static data segment? */
 GC_INNER GC_bool GC_register_main_static_data(void) { return TRUE; }
