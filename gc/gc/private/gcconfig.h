...
#endif
#if defined(LINUX)                                                     \
    && (defined(powerpc) || defined(__powerpc__) || defined(powerpc64) \
...
#define PREFETCH_FOR_WRITE(x) __asm__("        lfetch.excl     [%0]" : : "r"(x))
...
#define GET_MEM(bytes) \
    HBLKPTR(real_malloc((size_t)(bytes) + GC_page_size) + GC_page_size - 1)
#elif defined(OS2)
void* os2_alloc(size_t bytes);
#define GET_MEM(bytes) \
    HBLKPTR((ptr_t)os2_alloc((size_t)(bytes) + GC_page_size) + GC_page_size - 1)
#elif defined(NEXT) || defined(DOS4GW) || defined(NONSTOP)        \
    || (defined(AMIGA) && !defined(GC_AMIGA_FASTALLOC))           \
    || (defined(SOLARIS) && !defined(USE_MMAP)) || defined(RTEMS) \
    || defined(__CC_ARM)
#define GET_MEM(bytes)                                                       \
    HBLKPTR((size_t)calloc(1, (size_t)(bytes) + GC_page_size) + GC_page_size \
            - 1)
...
#define GET_MEM(bytes)                                                         \
    HBLKPTR(GC_MacTemporaryNewPtr((bytes) + GC_page_size, true) + GC_page_size \
            - 1)
#else
#define GET_MEM(bytes) \
    HBLKPTR(NewPtrClear((bytes) + GC_page_size) + GC_page_size - 1)
#endif
...
#define GET_MEM(bytes)                                               \
    HBLKPTR((size_t)GC_amiga_get_mem((size_t)(bytes) + GC_page_size) \
            + GC_page_size - 1)
