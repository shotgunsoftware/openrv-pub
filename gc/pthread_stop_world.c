    ...
    for (i = 0; i < THREAD_TABLE_SZ; i++)
    {
        for (p = GC_threads[i]; p != 0; p = p->next)
        {
            ...
            GC_push_all_register_sections(
                bs_lo, bs_hi, THREAD_EQUAL(p->id, self), p->traced_stack_sect);
...
#define NACL_PARK_WAIT_NANOSECONDS (100 * 1000)
    ...
    while (1)
    {
        ...
        for (i = 0;
             i < MAX_NACL_GC_THREADS && num_used < GC_nacl_num_gc_threads; i++)
        {
