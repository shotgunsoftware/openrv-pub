...
GC_INNER GC_bool GC_is_static_root(ptr_t p)
{
    ...
    for (i = 0; i < n_root_sets; i++)
    {
        if (p >= GC_static_roots[i].r_start && p < GC_static_roots[i].r_end)
        {
            last_root_set = i;
            return (TRUE);
        }
...
STATIC void GC_remove_roots_inner(ptr_t b, ptr_t e)
{
    ...
    for (i = 0; i < n_root_sets;)
    {
        if (GC_static_roots[i].r_start >= b && GC_static_roots[i].r_end <= e)
        {
            GC_remove_root_at_pos(i);
        }
        else
...
STATIC struct exclusion* GC_next_exclusion(ptr_t start_addr)
{
    ...
    if ((word)GC_excl_table[low].e_end <= (word)start_addr)
        return 0;
...
GC_INNER void GC_exclude_static_roots_inner(void* start, void* finish)
{
    ...
    if (GC_excl_table_entries == MAX_EXCLUSIONS)
...
STATIC void GC_push_current_stack(ptr_t cold_gc_frame, void* context)
{
    ...
    /* GC_push_all_stack_partially_eager.                     */
    {
        ...
        if (GC_all_interior_pointers && cold_gc_bs_pointer > BACKING_STORE_BASE)
        {
