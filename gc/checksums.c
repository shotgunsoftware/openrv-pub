...
STATIC void GC_update_check_page(struct hblk* h, int index)
{
    ...
    if (pe->new_valid && hhdr != 0
        && hhdr->hb_descr != 0 /* may contain pointers */
        && pe->old_sum != pe->new_sum)
    {
...
#ifdef STUBBORN_ALLOC
        if (!HBLK_IS_FREE(hhdr) && hhdr->hb_obj_kind == STUBBORN
            && !GC_page_was_changed(h) && !GC_on_free_list(h))
        {
...
void GC_check_dirty(void)
{
    ...
    for (i = 0; i < GC_n_heap_sects; i++)
    {
        ...
        for (h = (struct hblk*)start;
             h < (struct hblk*)(start + GC_heap_sects[i].hs_bytes); h++)
        {
...
out:
    ...
    if (GC_n_changed_errors > 0)
    {
        ...
        GC_err_printf("These may be benign (provoked by nonpointer changes)\n");
#ifdef THREADS
        GC_err_printf(
