...
asm static void PushMacRegisters(){
    sub.w #4,
    sp // reserve space for one parameter.
        move.l a2,
    (sp)jsr GC_push_one move.l a3,
    (sp)jsr GC_push_one move.l a4,
    (sp)jsr GC_push_one
    ...
    (sp)jsr GC_push_one
        ...
        move.l d2,
    (sp)jsr GC_push_one move.l d3,
    (sp)jsr GC_push_one move.l d4,
    (sp)jsr GC_push_one move.l d5,
    (sp)jsr GC_push_one move.l d6,
    (sp)jsr GC_push_one move.l d7,
    (sp)jsr GC_push_one add.w #4,
    sp // fix stack.
        rts}
...
void GC_push_regs(void) { PushMacRegisters(); }
...
GC_INNER void GC_with_callee_saves_pushed(void (*fn)(ptr_t, void*), ptr_t arg)
{
    ...
    __asm__ __volatile__("fldcw %0" : : "m"(*&old_fcw));
    {
        ...
        mxcsr =
            (mxcsr & ~(FE_ALL_EXCEPT << 7)) | ((old_fcw & FE_ALL_EXCEPT) << 7);
...
#if defined(MSWIN32) || defined(MSWINCE) || defined(UTS4) || defined(LINUX) \
    || defined(EWS4800) || defined(RTEMS)
    (void)setjmp(regs);
#else
    (void)_setjmp(regs);
...
#ifdef LINT
/*ARGSUSED*/
ptr_t GC_clear_stack_inner(ptr_t arg, word limit) { return (arg); }
