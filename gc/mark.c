...
GC_API void GC_CALL GC_noop1(word x) { GC_noop_sink = x; }
...
GC_INNER struct obj_kind GC_obj_kinds[MAXOBJKINDS] = {
    ...
    /* UNCOLLECTABLE */
    {&GC_uobjfreelist[0], 0, 0 | GC_DS_LENGTH, TRUE /* add length to descr */,
     TRUE},
#ifdef ATOMIC_UNCOLLECTABLE
    /* AUNCOLLECTABLE */
    {&GC_auobjfreelist[0], 0, 0 | GC_DS_LENGTH, FALSE /* add length to descr */,
     FALSE},
#endif
#ifdef STUBBORN_ALLOC
    /*STUBBORN*/
    {(void**)&GC_sobjfreelist[0], 0, 0 | GC_DS_LENGTH,
     TRUE /* add length to descr */, TRUE},
#endif
};
...
#endif
{
    ...
    case MS_PUSH_UNCOLLECTABLE:
        if (GC_mark_stack_top >= GC_mark_stack + GC_mark_stack_size / 4)
        {
#ifdef PARALLEL_MARK
...
static EXCEPTION_DISPOSITION mark_ex_handler(struct _EXCEPTION_RECORD* ex_rec,
                                             void* est_frame,
...
#define SHARE_BYTES 2048
                ...
                mark_stack_top->mse_start = limit =
                    current_p + WORDS_TO_BYTES(SPLIT_RANGE_WORDS - 1);
            ...
            case GC_DS_PROC:
                mark_stack_top--;
#ifdef ENABLE_TRACE
                if (GC_trace_addr >= current_p && GC_base(current_p) != 0
...
#ifdef ENABLE_TRACE
        if (GC_trace_addr >= current_p && GC_trace_addr < limit)
        {
            GC_log_printf("GC:%u Tracing from %p len %lu\n", (int)GC_gc_no,
                          current_p, (unsigned long)descr);
        }
#endif /* ENABLE_TRACE */
...
#endif /* ENABLE_TRACE */
            PUSH_CONTENTS((ptr_t)deferred, mark_stack_top, mark_stack_limit,
                          current_p, exit4);
        next_object:;
#endif
        }
...
/* entry.                                                               */
STATIC mse* GC_steal_mark_stack(mse* low, mse* high, mse* local, unsigned max,
                                mse** next)
{
    mse* p;
    mse* top = local - 1;
...
#define N_LOCAL_ITERS 1
    ...
    for (;;)
    {
            ...
            && local_top > local_mark_stack + 1)
        {
            ...
            /* it's harder.                                             */
            mse* new_bottom =
                local_mark_stack + (local_top - local_mark_stack) / 2;
            GC_ASSERT(new_bottom > local_mark_stack && new_bottom < local_top);
...
STATIC void GC_mark_local(mse* local_mark_stack, int id)
{
    ...
    GC_ASSERT((mse*)AO_load(&GC_first_nonempty) >= GC_mark_stack
              && (mse*)AO_load(&GC_first_nonempty)
                     <= (mse*)AO_load((volatile AO_t*)(&GC_mark_stack_top))
                            + 1);
    ...
    for (;;)
    {
        ...
        {
            AO_compare_and_swap(&GC_first_nonempty, (AO_t)global_first_nonempty,
    ...
    if (GC_mark_stack_size != 0)
    {
        if (new_stack != 0)
        {
            if (recycle_old)
            {
                ...
                if (size > 0)
                {
                    GC_add_to_heap((struct hblk*)((word)GC_mark_stack + displ),
                                   (word)size);
                }
            }
...
GC_INNER void GC_mark_init(void) { alloc_mark_stack(INITIAL_MARK_STACK_SIZE); }
...
#endif
    PUSH_CONTENTS_HDR(r, GC_mark_stack_top, GC_mark_stack_limit, source,
                      mark_and_push_exit, hhdr, FALSE);
mark_and_push_exit:;
...
void GC_add_trace_entry(char* kind, word arg1, word arg2)
{
    ...
    GC_trace_buf_ptr++;
    if (GC_trace_buf_ptr >= TRACE_ENTRIES)
        GC_trace_buf_ptr = 0;
}
...
void GC_print_trace(word gc_no, GC_bool lock)
{
    ...
    for (i = GC_trace_buf_ptr - 1; i != GC_trace_buf_ptr; i--)
    {
        ...
        printf("Trace:%s (gc:%u,bytes:%lu) 0x%X, 0x%X\n", p->kind,
               (unsigned)p->gc_no, (unsigned long)p->bytes_allocd,
...
STATIC struct hblk* GC_push_next_marked_dirty(struct hblk* h)
{
    ...
    for (;;)
    {
        if (EXPECT(IS_FORWARDING_ADDR_OR_NIL(hhdr) || HBLK_IS_FREE(hhdr),
                   FALSE))
        {
            h = GC_next_used_block(h);
            if (h == 0)
