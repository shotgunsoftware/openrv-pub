...
void* GC_DebugAllocProc(size_t size, PCR_Bool ptrFree, PCR_Bool clear)
{
    if (ptrFree)
    {
        void* result = (void*)GC_debug_malloc_atomic(size, __FILE__, __LINE__);
        if (clear && result != 0)
            BZERO(result, size);
        return (result);
    }
...
{
    return (
        GC_debug_realloc(old_object, new_size_in_bytes, __FILE__, __LINE__));
}
...
#error and its precursor was clearly broken.
    ...
    descr = hhdr->hb_descr;
    sz = hhdr->hb_sz;
    if (descr != 0 && ed->ed_pointerfree || descr == 0 && !(ed->ed_pointerfree))
        return;
    lim = (ptr_t)(h + 1) - sz;
    p = (ptr_t)h;
    do
    {
        if (PCR_ERes_IsErr(ed->ed_fail_code))
            return;
        ed->ed_fail_code = (*(ed->ed_proc))(p, sz, ed->ed_client_data);
        p += sz;
    } while (p <= lim);
}
...
PCR_ERes GC_EnumerateProc(PCR_Bool ptrFree,
...
struct PCR_MM_ProcsRep GC_Rep = {
    MY_MAGIC,         GC_AllocProc,
    GC_ReallocProc,   GC_DummyFreeProc,    /* mmp_free */
    GC_FreeProc,                           /* mmp_unsafeFree */
    GC_EnumerateProc, GC_DummyShutdownProc /* mmp_shutdown */
};

struct PCR_MM_ProcsRep GC_DebugRep = {
    MY_DEBUGMAGIC,       GC_DebugAllocProc,
    GC_DebugReallocProc, GC_DummyFreeProc,    /* mmp_free */
...
PCR_ERes PCR_GC_Setup(void) { return PCR_ERes_okay; }

PCR_ERes PCR_GC_Run(void)
{
