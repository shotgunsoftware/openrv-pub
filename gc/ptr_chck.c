...
GC_API void* GC_CALL GC_same_obj(void* p, void* q)
{
    ...
    if (hhdr == 0)
    {
        if (divHBLKSZ((word)p) != divHBLKSZ((word)q) && HDR((word)q) != 0)
        {
            goto fail;
        }
        return (p);
...
GC_API void* GC_CALL GC_is_valid_displacement(void* p)
{
    ...
    if ((sz > MAXOBJBYTES && (ptr_t)p >= (ptr_t)h + sz)
        || !GC_valid_offsets[offset] || (ptr_t)p - offset + sz > (ptr_t)(h + 1))
    {
        goto fail;
    }
    return (p);
...
GC_API void* GC_CALL GC_is_visible(void* p)
{
    ...
    retry:
        ...
        {
        case GC_DS_LENGTH:
            if ((word)((ptr_t)p - (ptr_t)base) > (word)descr)
                goto fail;
            break;
        case GC_DS_BITMAP:
            if ((word)((ptr_t)p - (ptr_t)base) >= WORDS_TO_BYTES(BITMAP_BITS)
                || ((word)p & (sizeof(word) - 1)))
                goto fail;
            if (!(((word)1 << (WORDSZ - ((ptr_t)p - (ptr_t)base) - 1)) & descr))
                goto fail;
            break;
        case GC_DS_PROC:
