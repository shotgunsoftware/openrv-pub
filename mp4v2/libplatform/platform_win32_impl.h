// Note that we have a separate platform_win32_impl.h to deal with the fact that
// windows.h defines a macro called FindAtom, which mp4v2 also defines.  In
// older versions of visual studio, this actually causes some pretty seriously
// issues with naming collisions and the defined macros (think infamous min/max
// macro of windows.h vs stdc++'s min/max template functions)
#include <windows.h>
    ...
    namespace platform
    {
        namespace win32
        {
            ...
            private:
                ...
                static const UINT8* Utf8DecodeChar(const UINT8* utf8_char,
                                                   size_t num_bytes,
                                                   wchar_t* utf16,
                                                   int* invalid);
