    ...
    namespace platform
    {
        namespace win32
        {
            ...
            static struct utf8_len_info s_len_info[] = {
                {0x00000000, 0x0000007F, 1}, {0x00000080, 0x000007FF, 2},
                {0x00000800, 0x0000FFFF, 3}, {0x00010000, 0x001FFFFF, 4},
                {0x00200000, 0x03FFFFFF, 5}, {0x04000000, 0x7FFFFFFF, 6}};
             ...
             */
            wchar_t* Utf8ToFilename::ConvertToUTF16(const string& utf8string)
            {
                int num_bytes;
                size_t num_chars;
                wchar_t* retval;
                ...
                LOG_PRINTF((MP4_LOG_VERBOSE4,
                            "%s: entry point (%d character string)",
                ...
                */
                ...
                if (!retval)
                {
                    log.errorf("%s: error allocating memory for %d byte(s)",
             ...
             */
            int Utf8ToFilename::ConvertToUTF16Buf(const char* utf8,
                                                  wchar_t* utf16_buf,
                                                  size_t num_bytes)
            {
                size_t i;
                ...
                LOG_PRINTF((MP4_LOG_VERBOSE4, "%s: converting \"%s\"",
                            __FUNCTION__, utf8));
                ...
                {
                    log.errorf("%s: %u byte(s) is not enough to transform a %u "
                               "byte UTF-8 string "
                               "to NUL-terminated UTF-16",
                               __FUNCTION__, num_bytes, num_input_bytes);
                    return 0;
                }
                ...
                {
                    LOG_PRINTF((MP4_LOG_VERBOSE4,
                                "%s: decoding first UTF-8 byte 0x%02X (UTF-16 "
                                "character %d of at most %d)",
                                __FUNCTION__, *p, (i + 1), num_utf16_chars));
                    ...
                    // characters here.
                    next_char =
                        Utf8DecodeChar(p, num_input_bytes, this_utf16, NULL);
                    ...
                    if (this_utf16[1])
                    {
                        ...
                        {
                            log.errorf("%s: out of space in %u  byte output "
                                       "string to store surrogate "
                                       "pair low half (0x%04X)",
                                       __FUNCTION__, num_bytes, this_utf16[1]);
                            return 0;
                        }
                ...
                if (*p)
                {
                    ...
                    log.errorf("%s: %u byte(s) of input string remain(s) "
                               "undecoded (%s): out of space in "
                               "%u byte output string",
                               __FUNCTION__, (num_input_bytes - 1), p,
                               num_bytes);
                    return 0;
                }
             ...
             * characters
             */
            int Utf8ToFilename::GetPrefixLen(const string& utf8string)
            {
             ...
             */
            int Utf8ToFilename::IsAbsolute(const string& utf8string)
            {
                ...
                if (utf8string[1] == ':')
                {
                    ...
                    if ((utf8string.length() >= 2)
                        && IsPathSeparator(utf8string[2]))
                    {
                        // Yup -- it's absolute
                        return 1;
                    }
             ...
             */
            int Utf8ToFilename::IsPathSeparator(char c)
            {
                return ((c == '\\') || (c == '/'));
            }
             ...
             */
            int Utf8ToFilename::IsUncPath(const string& utf8string)
            {
                const char* host;
                int num_slashes;
                const char* p;
                ...
                if ((num_slashes > 2) || !(*p))
                {
                    ...
                    log.errorf("%s: don't understand path(%s)", __FUNCTION__,
                               utf8string.c_str());
                    return 0;
                }
                ...
                if (!(*p))
                {
                    ...
                    log.warningf("%s: incomplete UNC path: host only(%s)",
                ...
                if (!(*p))
                {
                    ...
                    // part after the host
                    log.warningf(
                        "%s: incomplete UNC path: no path after host(%s)",
                        __FUNCTION__, utf8string.c_str());
                    return 0;
                }
                ...
                if (num_slashes > 1)
                {
                    ...
                    // //host///blah, etc.
                    log.warningf(
                        "%s: invalid UNC path: too many slashes after host(%s)",
                        __FUNCTION__, utf8string.c_str());
                    return 0;
                }
             ...
             */
            bool Utf8ToFilename::IsUTF16Valid() const
            {
             ...
             * num_bytes.
             */
            const UINT8* Utf8ToFilename::Utf8DecodeChar(const UINT8* utf8_char,
                                                        size_t num_bytes,
                                                        wchar_t* utf16,
                                                        int* invalid)

            {
                ...
                LOG_PRINTF((MP4_LOG_VERBOSE4,
                            "%s: decoding UTF-8 string at address 0x%p",
                ...
                */
                ...
                if (len == 0)
                {
                    log.errorf("%s: 0x%02X is not a valid first byte of a "
                ...
                */
                if (len == 1)
                {
                    ...
                    */
                    LOG_PRINTF((MP4_LOG_VERBOSE4,
                                "%s: one byte UTF-16 encoding: 0x%02X",
                                __FUNCTION__, *p));
                    *utf16 = *p;
                    if (invalid)
                    {
                ...
                */
                ...
                {
                    log.errorf(
                        "%s: first byte 0x%02X indicates a %d byte "
                ...
                ** overall result.
                **
                ** 1 byte encoding [U+00000000,U+0000007F]: 7 bits (7 bits
                *total) (handled above)
                ** 2 byte encoding [U+00000080,U+000007FF]: 5 bits, 6 bits (11
                *bits total)
                ** 3 byte encoding [U+00000800,U+0000FFFF]: 4 bits, 6 bits, 6
                *bits (16 bits total)
                ** 4 byte encoding [U+00010000,U+001FFFFF]: 3 bits, 6 bits, 6
                *bits, 6 bits (21 bits total)
                ** 5 byte encoding [U+00200000,U+03FFFFFF]: 2 bits, 6 bits, 6
                *bits, 6 bits, 6 bits (26 bits total)
                ** 6 byte encoding [U+04000000,U+7FFFFFFF]: 1 bit, 6 bits, 6
                *bits, 6 bits, 6 bits, 6 bits (31 bits total)
                **
                ...
                */
                for (i = 1; (i < len); i++)
                {
                    if ((*p < 0x80) || (*p > 0xBF))
                    {
                        log.errorf("%s: 0x%02X is not a valid continuation "
                                   "character in a UTF-8 encoding",
                                   __FUNCTION__, *p);
                ...
                */
                ...
                if (valid_len == 0)
                {
                    ...
                    */
                    log.errorf("%s: decoded a character that we can't encode "
                               "again (0x%08X)",
                               __FUNCTION__, ucs4);
                    ASSERT(0);
                ...
                if (len != valid_len)
                {
                    ...
                    log.errorf("%s: overlong encoding(%s)...should be %d "
                               "byte(s), not %d",
                               __FUNCTION__, utf8_char, valid_len, len);
                    *utf16 = REPLACEMENT_CHAR;
                    return p;
                }
                ...
                */
                if (ucs4 > 0x0010FFFF)
                {
                    log.errorf("%s: code point 0x%08X is too big", __FUNCTION__,
                               ucs4);
                    *utf16 = REPLACEMENT_CHAR;
                    return p;
                }
                ...
                */
                ...
                {
                    log.errorf("%s: code point 0x%08X is in the range used to "
                               "indicate surrogate pairs",
                               __FUNCTION__, ucs4);
                    *utf16 = REPLACEMENT_CHAR;
                    return p;
                }
                ...
                */
                ...
                {
                    log.errorf("%s: invalid code point (0x%08X)", __FUNCTION__,
                               ucs4);
                    *utf16 = REPLACEMENT_CHAR;
                    return p;
                }
                ...
                */
                if (ucs4 <= 0x0000FFFF)
                {
                    *utf16 = ucs4 & 0xFFFF;
                    LOG_PRINTF((MP4_LOG_VERBOSE4,
                                "%s: UTF-16 encoding of 0x%08X is 0x%04X",
                ...
                */
                ...
                LOG_PRINTF((MP4_LOG_VERBOSE4,
                            "%s: UTF-16 encoding of 0x%08X is 0x%04X:0x%04X",
             ...
             */
            size_t Utf8ToFilename::Utf8LenFromUcs4(UINT32 ucs4)
            {
                size_t table_idx;

                LOG_PRINTF((MP4_LOG_VERBOSE4,
                            "%s: processing UCS-4 code point 0x%08X",
                            __FUNCTION__, ucs4));

                for (table_idx = 0;
                     (table_idx
                      < (sizeof(s_len_info) / sizeof(struct utf8_len_info)));
                     table_idx++)
                {
                ...
                log.errorf("%s: 0x%08X is an invalid code point", __FUNCTION__,
                           ucs4);
            ...
            Utf8ToFilename::Utf8NumOctets(UINT8 utf8_first_byte)
            {
                 ...
                 * [0xC0,0xDF]: 2       (192-223)   (32 possible values) (see
                 * below) [0xE0,0xEF]: 3       (224-239)   (16 possible values)
