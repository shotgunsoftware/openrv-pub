...
#define MP4V2_BYTESWAP_16(x)                            \
    mp4v2::platform::uint16_t(                          \
        ((mp4v2::platform::uint16_t(x) & 0xff00U) >> 8) \
        | ((mp4v2::platform::uint16_t(x) & 0x00ffU) << 8))

#define MP4V2_BYTESWAP_32(x)                                  \
    mp4v2::platform::uint32_t(                                \
        ((mp4v2::platform::uint32_t(x) & 0xff000000U) >> 24)  \
...
#define MP4V2_BYTESWAP_64(x)                                             \
    mp4v2::platform::uint64_t(                                           \
        ((mp4v2::platform::uint64_t(x) & 0xff00000000000000ULL) >> 56)   \
