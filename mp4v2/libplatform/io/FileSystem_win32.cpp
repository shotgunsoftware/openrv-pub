        ...
        namespace io
        {
            ...
            static DWORD getAttributes(string path_)
            {
                ...
                if (attributes == INVALID_FILE_ATTRIBUTES)
                {
                    ...
                    ostringstream msg;
                    msg << "GetFileAttributes(" << filename.utf8 << ") failed ("
                        << last_err << ")";
                    throw new Exception(msg.str(), __FILE__, __LINE__,
                                        __FUNCTION__);
                }
            ...
            bool FileSystem::exists(string path_)
            {
            ...
            bool FileSystem::isDirectory(string path_)
            {
            ...
            bool FileSystem::isFile(string path_)
            {
                DWORD attributes = getAttributes(path_);
                if (attributes == INVALID_FILE_ATTRIBUTES)
                    return false;
            ...
            bool FileSystem::getFileSize(string path_, File::Size& size_)
            {
                win32::Utf8ToFilename filename(path_);
                                          ...
                                          (LPVOID)&data))
                {
                    log.errorf("%s: GetFileAttributesExW(%s) failed (%d)",
                               __FUNCTION__, filename.utf8.c_str(),
                               GetLastError());
                    return true;
                }
            ...
            bool FileSystem::rename(string from, string to)
            {
                                       ...
                                       | MOVEFILE_WRITE_THROUGH))
                {
                    log.errorf("%s: MoveFileExW(%s,%s) failed (%d)",
                               __FUNCTION__, from_file.utf8.c_str(),
                               to_file.utf8.c_str(), GetLastError());
                    return true;
                }
