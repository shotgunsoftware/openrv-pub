...
namespace mp4v2
{
    namespace util
    {
        ...
        uint64_t Timecode::convertDuration(const Timecode& obj) const
        {
        ...
        Timecode& Timecode::operator=(const Timecode& rhs)
        {
            _scale = rhs._scale;
        ...
        bool Timecode::operator<(const Timecode& obj) const
        {
        ...
        Timecode Timecode::operator+(const Timecode& obj) const
        {
            Timecode t(*this);
            t += obj;
            return t;
        ...
        bool Timecode::parse(const string& time, string* outError)
        {
            ...
            {
                double div =
                    std::pow(10.0, static_cast<double>(tbuffer.length()));
                if (div < 1.0)
                    div = 1.0;
                *tvalue = static_cast<uint64_t>(static_cast<double>(*tvalue)
                                                / div * std::ceil(_scale));
                break;
            }
            }
        ...
        void Timecode::recompute()
        {
            ...
            _duration = _subseconds + (iscale * _seconds)
                        + (iscale * _minutes * 60) + (iscale * _hours * 3600);

            ostringstream oss;
            oss << setfill('0') << right << setw(2) << _hours << ':' << setw(2)
                << _minutes << ':' << setw(2) << _seconds;
            ...
            case DECIMAL:
            {
                oss << '.' << setw(3) << setfill('0')
                    << static_cast<uint64_t>(_subseconds / _scale * 1000.0
                                             + 0.5);
                break;
            }
            }
        ...
        void Timecode::reset() { setDuration(0); }

        ///////////////////////////////////////////////////////////////////////////////

        void Timecode::setDuration(uint64_t duration_, double scale_)
        {
            if (scale_ != 0.0)
            {
                _scale = scale_;
        ...
        void Timecode::setFormat(Format format_)
        {
            _format = format_;
            recompute();
        }
        ...
        void Timecode::setHours(uint64_t hours_)
        {
            _hours = hours_;
            recompute();
        }
        ...
        void Timecode::setMinutes(uint64_t minutes_)
        {
            _minutes = minutes_;
            recompute();
        }
        ...
        void Timecode::setScale(double scale_)
        {
            ...
            _subseconds =
                static_cast<uint64_t>((_scale / oldscale) * _subseconds);
            recompute();
        }

        ///////////////////////////////////////////////////////////////////////////////

        void Timecode::setSeconds(uint64_t seconds_)
        {
            _seconds = seconds_;
            recompute();
        }
        ...
        void Timecode::setSubseconds(uint64_t subseconds_)
        {
