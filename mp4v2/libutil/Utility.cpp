...
namespace mp4v2
{
    namespace util
    {
        ...
        Utility::Utility(string name_, int argc_, char** argv_)
            ...
            , STD_OPTIMIZE('z', false, "optimize", false, LC_NONE,
                           "optimize mp4 file after modification")
            , STD_DRYRUN('y', false, "dryrun", false, LC_NONE,
                         "do not actually create or modify any files")
            , STD_KEEPGOING('k', false, "keepgoing", false, LC_NONE,
                            "continue batch processing even after errors")
            , STD_OVERWRITE('o', false, "overwrite", false, LC_NONE,
                            "overwrite existing files when creating")
            , STD_FORCE('f', false, "force", false, LC_NONE,
                        "force overwrite even if file is read-only")
            , STD_QUIET('q', false, "quiet", false, LC_NONE,
                        "equivalent to --verbose 0")
            , STD_DEBUG(
                  'd', false, "debug", true, LC_DEBUG,
                  ...
                  "\n  4  everything")
            , STD_VERBOSE(
                  'v', false, "verbose", true, LC_VERBOSE,
                  ...
                  "\n  3  everything")
            , STD_HELP('h', false, "help", false, LC_HELP,
                       "print brief help or long-option for extended help")
            , STD_VERSION(0, false, "version", false, LC_VERSION,
                          "print version information and exit")
            , STD_VERSIONX(0, false, "versionx", false, LC_VERSIONX,
                           "print extended version information", "ARG", "",
                           true)
        ...
        Utility::~Utility() { delete[] _longOptions; }

        ///////////////////////////////////////////////////////////////////////////////

        bool Utility::batch(int argi)
        {
            _jobCount = 0;
        ...
        void Utility::debugUpdate(uint32_t debug)
        {
            MP4LogLevel level;
        ...
        void Utility::errf(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
                 ...
                 it++, groupCount++)
            {
                     ...
                     ito != ieo; ito++, optionCount++)
                {
                    ...
                    if (option.lhasarg)
                    {
                        ...
                        oss << setw(longMax - option.lname.length() - 1
        ...
        bool Utility::herrf(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
        ...
        bool Utility::hwarnf(const char* format, ...)
        {
            fprintf(stdout, "WARNING: ");
            va_list ap;
            va_start(ap, format);
        ...
        void Utility::outf(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
            vfprintf(stdout, format, ap);
        ...
        void Utility::printHelp(bool extended, bool toerr)
        {
            ostringstream oss;
            ...
            if (extended)
            {
                ...
                for (list<Group*>::reverse_iterator it = _groups.rbegin();
                     it != ie; it++)
                {
                    Group& group = **it;
                    const Group::List::const_iterator ieo = group.options.end();
                    for (Group::List::const_iterator ito =
                             group.options.begin();
                         ito != ieo; ito++)
                    {
                        const Option& option = **ito;
        ...
        void Utility::printUsage(bool toerr)
        {
            ostringstream oss;
            oss << "Usage: " << _name << " " << _usage
        ...
        void Utility::printVersion(bool extended)
        {
            ostringstream oss;
            oss << left;
            ...
            if (extended)
            {
                oss << setw(13) << "utility:" << _name << '\n'
                    ...
                    << '\n'
                    << setw(18)
                    << "repository type:" << MP4V2_PROJECT_repo_type;
            }
            else
            {
        ...
        bool Utility::process_impl()
        {
            ...
            for (Group::List::const_iterator it = _group.options.begin();
                 it != ie; it++)
            {
            ...
            {
#if _MSC_VER
                const int code = prog::getOption(
                    _argc, _argv, _shortOptions.c_str(), _longOptions, NULL);
#else
                const ssize_t code = prog::getOption(
                    _argc, _argv, _shortOptions.c_str(), _longOptions, NULL);
#endif
                if (code == -1)
                    break;
        ...
        bool Utility::openFileForWriting(io::File& file)
        {
        ...
        void Utility::verbose(uint32_t level, const char* format, va_list ap)
        {
            if (level > _verbosity)
                return;
            vfprintf(stdout, format, ap);
        ...
        void Utility::verbose1f(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
            verbose(1, format, ap);
        ...
        void Utility::verbose2f(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
            verbose(2, format, ap);
        ...
        void Utility::verbose3f(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
            verbose(3, format, ap);
        ...
        void Utility::Group::add(const Option& option)
        {
        ...
        void Utility::Group::add(char scode, bool shasarg, string lname,
                                 bool lhasarg, uint32_t lcode, string descr,
                                 string argname, string help, bool hidden)
        {
            Option* o = new Option(scode, shasarg, lname, lhasarg, lcode, descr,
                                   argname, help, hidden);
        ...
        void Utility::Group::add(string lname, bool lhasarg, uint32_t lcode,
                                 string descr, string argname, string help,
                                 bool hidden)
        {
        ...
        Utility::Option::Option(char scode_, bool shasarg_, string lname_,
                                bool lhasarg_, uint32_t lcode_, string descr_,
                                string argname_, string help_, bool hidden_)
            : scode(scode_)
            , shasarg(shasarg_)
            , lname(lname_)
            , lhasarg(lhasarg_)
