    ...
    namespace impl
    {
            ...
            : MP4Atom(file, "stsc")
        {
            ...
            pTable->AddProperty(new MP4Integer32Property(
                pTable->GetParentAtom(), "firstChunk"));
            pTable->AddProperty(new MP4Integer32Property(
                pTable->GetParentAtom(), "samplesPerChunk"));
            pTable->AddProperty(new MP4Integer32Property(
                pTable->GetParentAtom(), "sampleDescriptionIndex"));

            // As an optimization we add an implicit property to this table,
            // "firstSample" that corresponds to the first sample of the
            // firstChunk
        ...
        void MP4StscAtom::Read()
        {
            ...
            MP4Integer32Property* pFirstChunk =
                (MP4Integer32Property*)((MP4TableProperty*)m_pProperties[3])
                    ->GetProperty(0);
            MP4Integer32Property* pSamplesPerChunk =
                (MP4Integer32Property*)((MP4TableProperty*)m_pProperties[3])
                    ->GetProperty(1);
            MP4Integer32Property* pFirstSample =
                (MP4Integer32Property*)((MP4TableProperty*)m_pProperties[3])
                    ->GetProperty(3);
            ...
            for (uint32_t i = 0; i < count; i++)
            {
                ...
                if (i < count - 1)
                {
                    sampleId += (pFirstChunk->GetValue(i + 1)
                                 - pFirstChunk->GetValue(i))
