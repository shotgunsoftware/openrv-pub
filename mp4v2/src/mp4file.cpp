...
namespace mp4v2
{
    namespace impl
    {
        ...
        const std::string& MP4File::GetFilename() const
        {
        ...
        void MP4File::Create(const char* fileName, uint32_t flags, int add_ftyp,
                             int add_iods, char* majorBrand,
                             uint32_t minorVersion, char** supportedBrands,
                             uint32_t supportedBrandsCount)
        {
            ...
            if (add_ftyp != 0)
            {
                MakeFtypAtom(majorBrand, minorVersion, supportedBrands,
            ...
            (void)InsertChildAtom(m_pRootAtom, "mdat", add_ftyp != 0 ? 1 : 0);
        ...
        bool MP4File::Use64Bits(const char* atomName)
        {
            ...
            {
                return (m_createFlags & MP4_CREATE_64BIT_DATA)
                       == MP4_CREATE_64BIT_DATA;
            }
            if (atomid == ATOMID("mvhd") || atomid == ATOMID("tkhd")
                || atomid == ATOMID("mdhd"))
            {
                return (m_createFlags & MP4_CREATE_64BIT_TIME)
        ...
        void MP4File::Check64BitStatus(const char* atomName)
        {
            ...
            else if (atomid == ATOMID("mvhd") || atomid == ATOMID("tkhd")
                     || atomid == ATOMID("mdhd"))
            {
        ...
        bool MP4File::Modify(const char* fileName)
        {
            ...
            else
            {
                ...
                for (i = numAtoms - 1; i >= 0; i--)
                {
                    ...
                    { // last atom isn't moov
                        ...
                        MP4Atom* pFreeAtom =
                            MP4Atom::CreateAtom(*this, NULL, "free");
                        ...
                        // finally set our file position to the end of the last
                        // atom
            ...
            MP4Atom* pMdatAtom =
                InsertChildAtom(m_pRootAtom, "mdat", numAtoms - 1);
        ...
        void MP4File::Optimize(const char* srcFileName, const char* dstFileName)
        {
            ...
            {
                // No destination given, so let's kludge together a temporary
                // file. We'll try to create it in the same directory as the
                // srcFileName, since it's more likely that directory is
                // writable.  In the absence of that, we'll create it in "./",
                // which is the default pathnameTemp() provides.
            ...
            {
                // cleanup and rethrow.  Without this, we'd leak memory and an
                // open file handle(s).
                if (src == NULL && dst == NULL)
                    delete m_file; // We didn't make it far enough to have
                                   // m_file go to src or dst.
            ...
            for (;;)
            {
                ...
                for (uint32_t i = 0; i < numTracks; i++)
                {
                    ...
                    if (nextChunkTimes[i] == MP4_INVALID_TIMESTAMP)
                    {
                        ...
                        nextChunkTimes[i] = MP4ConvertTime(
                            chunkTime, m_pTracks[i]->GetTimeScale(),
                            GetTimeScale());
                    }
                ...
                m_file = &src;
                m_pTracks[nextTrackIndex]->ReadChunk(chunkIds[nextTrackIndex],
                                                     &pChunk, &chunkSize);

                // point back at the new mp4 file for write chunk
                m_file = &dst;
                m_pTracks[nextTrackIndex]->RewriteChunk(
                    chunkIds[nextTrackIndex], pChunk, chunkSize);
        ...
        void MP4File::Open(const char* name, File::Mode mode,
                           const MP4FileProvider* provider)
        {
            ASSERT(!m_file);

            m_file = new File(name, mode,
                              provider ? new io::CustomFileProvider(*provider)
                                       : NULL);
            if (m_file->open())
            {
                ostringstream msg;
                msg << "open(" << name << ") failed";
                throw new Exception(msg.str(), __FILE__, __LINE__,
                                    __FUNCTION__);
            }
        ...
        void MP4File::GenerateTracks()
        {
            ...
            {
                char trackName[32];
                snprintf(trackName, sizeof(trackName), "moov.trak[%u]",
                         trackIndex);
                ...
                if (pTrackIdProperty && pTypeProperty)
                {
                    ...
                    try
                    {
                        if (!strcmp(pTypeProperty->GetValue(),
                                    MP4_HINT_TRACK_TYPE))
                        {
                    ...
                    if (pTrack && !strcmp(pTrack->GetType(), MP4_OD_TRACK_TYPE))
                    {
                        ...
                        {
                            log.warningf(
                                "%s: \"%s\": multiple OD tracks present",
                                __FUNCTION__, GetFilename().c_str());
                        }
                    }
                }
        ...
        void MP4File::BeginWrite() { m_pRootAtom->BeginWrite(); }
        ...
        void MP4File::FinishWrite(uint32_t options)
        {
            ...
            // remove empty moov.udta.meta
            {
                ...
                if (meta)
                {
                    ...
                    {
                        if (ATOMID(meta->GetChildAtom(0)->GetType())
                            == ATOMID("hdlr"))
                        {
                            meta->GetParentAtom()->DeleteChildAtom(meta);
                            delete meta;
            ...
            // remove empty moov.udta.name
            {
                ...
                if (name)
                {
                    ...
                    uint32_t valSize = 0;
                    GetBytesProperty("moov.udta.name.value", (uint8_t**)&val,
                                     &valSize);
                    if (valSize == 0)
                    {
            ...
            // we mark remaining bytes as free atom; otherwise trailing garbage
            // remains.
            if (GetPosition() < GetSize())
            {
                ...
                MP4FreeAtom* freeAtom =
                    (MP4FreeAtom*)MP4Atom::CreateAtom(*this, NULL, "free");
                ASSERT(freeAtom);
        ...
        {
            log.dump(0, MP4_LOG_VERBOSE1, "\"%s\": Dumping meta-information...",
                     m_file->name.c_str());
        ...
        void MP4File::Rename(const char* oldFileName, const char* newFileName)
        {
            if (FileSystem::rename(oldFileName, newFileName))
                throw new PlatformException(sys::getLastErrorStr(),
                                            sys::getLastError(), __FILE__,
                                            __LINE__, __FUNCTION__);
        }

        void MP4File::ProtectWriteOperation(const char* file, int line,
                                            const char* func)
        {
            if (!IsWriteMode())
        ...
        MP4Atom* MP4File::AddChildAtom(const char* parentName,
                                       const char* childName)
        {
            return AddChildAtom(FindAtom(parentName), childName);
        }

        MP4Atom* MP4File::AddChildAtom(MP4Atom* pParentAtom,
                                       const char* childName)
        {
        ...
        MP4Atom* MP4File::InsertChildAtom(const char* parentName,
                                          const char* childName, uint32_t index)
        {
            return InsertChildAtom(FindAtom(parentName), childName, index);
        }

        MP4Atom* MP4File::InsertChildAtom(MP4Atom* pParentAtom,
                                          const char* childName, uint32_t index)
        {
            MP4Atom* pChildAtom =
                MP4Atom::CreateAtom(*this, pParentAtom, childName);
        ...
        MP4Atom* MP4File::AddDescendantAtoms(const char* ancestorName,
                                             const char* descendantNames)
        {
            return AddDescendantAtoms(FindAtom(ancestorName), descendantNames);
        }

        MP4Atom* MP4File::AddDescendantAtoms(MP4Atom* pAncestorAtom,
                                             const char* descendantNames)
        {
        ...
        bool MP4File::FindProperty(const char* name, MP4Property** ppProperty,
                                   uint32_t* pIndex)
        {
            if (pIndex)
                                          ...
                                          uint32_t* pIndex)
        {
            ...
            default:
                ostringstream msg;
                msg << "type mismatch - property " << name << " type "
                    << (*ppProperty)->GetType();
                throw new Exception(msg.str(), __FILE__, __LINE__,
                                    __FUNCTION__);
            }
        ...
        void MP4File::GetBytesProperty(const char* name, uint8_t** ppValue,
                                       uint32_t* pValueSize)
        {
            MP4Property* pProperty;
            uint32_t index;
            ...
            ((MP4BytesProperty*)pProperty)
                ->GetValue(ppValue, pValueSize, index);
        }

        void MP4File::SetBytesProperty(const char* name, const uint8_t* pValue,
                                       uint32_t valueSize)
        {
            ProtectWriteOperation(__FILE__, __LINE__, __FUNCTION__);
        ...
        void MP4File::AddTrackToIod(MP4TrackId trackId)
        {
            ...
            (void)m_pRootAtom->FindProperty(
                "moov.iods.esIds", (MP4Property**)&pDescriptorProperty);
            ...
            (void)pDescriptor->FindProperty("id", (MP4Property**)&pIdProperty);
            ASSERT(pIdProperty);
        ...
        void MP4File::RemoveTrackFromIod(MP4TrackId trackId, bool shallHaveIods)
        {
            ...
            for (uint32_t i = 0; i < pDescriptorProperty->GetCount(); i++)
            {
                ...
                MP4Integer32Property* pIdProperty = NULL;
                (void)pDescriptorProperty->FindProperty(
                    name, (MP4Property**)&pIdProperty);
                // wmay ASSERT(pIdProperty);

                if (pIdProperty != NULL && pIdProperty->GetValue() == trackId)
                {
        ...
        void MP4File::RemoveTrackFromOd(MP4TrackId trackId)
        {
            ...
            RemoveTrackReference(MakeTrackName(m_odTrackId, "tref.mpod"),
                                 trackId);
        }

        /*
         * Try to obtain the properties of this reference track, if not found
         * then return NULL in *ppCountProperty and *ppTrackIdProperty.
         */
        void
        MP4File::GetTrackReferenceProperties(const char* trefName,
                                             MP4Property** ppCountProperty,
                                             MP4Property** ppTrackIdProperty)
        {
            char propName[1024];

            snprintf(propName, sizeof(propName), "%s.%s", trefName,
                     "entryCount");
        ...
        void MP4File::AddTrackReference(const char* trefName,
        ...
        void MP4File::RemoveTrackReference(const char* trefName,
                                           MP4TrackId refTrackId)
        {
            MP4Integer32Property* pCountProperty = NULL;
            MP4Integer32Property* pTrackIdProperty = NULL;
                                            ...
                                            uint32_t timeScale)
        {
            ...
            (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "nmhd",
                                  0);
            ...
            // of child atoms that needs to be incremented after we add the mp4s
            // atom
            ...
            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.mp4s.esds.ESID", 0);
        ...
        MP4TrackId MP4File::AddODTrack()
        {
            ...
            (void)AddDescendantAtoms(MakeTrackName(m_odTrackId, NULL),
                                     "tref.mpod");
        ...
        bool MP4File::ShallHaveIods()
        {
            ...
            const char* brandsWithIods[] = {"mp42", "isom", NULL};
        ...
        void MP4File::SetAmrVendor(MP4TrackId trackId, uint32_t vendor)
        {
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.damr.vendor", vendor);
        }

        void MP4File::SetAmrDecoderVersion(MP4TrackId trackId,
        ...
        void MP4File::SetAmrModeSet(MP4TrackId trackId, uint16_t modeSet)
        {
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.damr.modeSet", modeSet);
        }

        uint16_t MP4File::GetAmrModeSet(MP4TrackId trackId)
        {
            return GetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.damr.modeSet");
        }

        MP4TrackId MP4File::AddAmrAudioTrack(uint32_t timeScale,
                                             uint16_t modeSet,
                                             ...
                                             bool isAmrWB)
        {
            ...
            uint32_t fixedSampleDuration =
                (timeScale * 20) / 1000; // 20mSec/Sample
            ...
            (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "smhd",
                                  0);
            ...
            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the mp4a
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
            ...
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*.timeScale",
                                    timeScale);
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.damr.modeChangePeriod",
                modeChangePeriod);

            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.damr.framesPerSample",
                framesPerSample);

            m_pTracks[FindTrackIndex(trackId)]->SetFixedSampleDuration(
                fixedSampleDuration);
        ...
        MP4TrackId MP4File::AddULawAudioTrack(uint32_t timeScale)
        {
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.ulaw.timeScale", timeScale << 16);
        ...
        MP4TrackId MP4File::AddALawAudioTrack(uint32_t timeScale)
        {
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.alaw.timeScale", timeScale << 16);
        ...
        MP4TrackId MP4File::AddAudioTrack(uint32_t timeScale,
                                          ...
                                          uint8_t audioType)
        {
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.mp4a.timeScale", timeScale << 16);

            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.mp4a.esds.ESID", 0);
        ...
        MP4TrackId MP4File::AddAC3AudioTrack(uint32_t samplingRate,
                                             uint8_t fscod, uint8_t bsid,
                                             uint8_t bsmod, uint8_t acmod,
                                             uint8_t lfeon,
                                             uint8_t bit_rate_code)
        {
            ...
            {
                throw new Exception("no ac-3.samplingRate property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }
            ...
            FindProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.ac-3.dac3.fscod"),
            ...
            {
                throw new Exception("no dac3.fscod property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }

            FindProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.ac-3.dac3.bsid"),
                (MP4Property**)&pBitfieldProperty);
            if (pBitfieldProperty)
            {
            ...
            {
                throw new Exception("no dac3.bsid property", __FILE__, __LINE__,
                                    __FUNCTION__);
            }

            FindProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.ac-3.dac3.bsmod"),
                (MP4Property**)&pBitfieldProperty);
            if (pBitfieldProperty)
            {
            ...
            {
                throw new Exception("no dac3.bsmod property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }

            FindProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.ac-3.dac3.acmod"),
                (MP4Property**)&pBitfieldProperty);
            if (pBitfieldProperty)
            {
            ...
            {
                throw new Exception("no dac3.acmod property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }

            FindProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.ac-3.dac3.lfeon"),
                (MP4Property**)&pBitfieldProperty);
            if (pBitfieldProperty)
            {
            ...
            {
                throw new Exception("no dac3.lfeon property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }

            FindProperty(
                MakeTrackName(trackId,
                              "mdia.minf.stbl.stsd.ac-3.dac3.bit_rate_code"),
                (MP4Property**)&pBitfieldProperty);
            if (pBitfieldProperty)
            {
            ...
            {
                throw new Exception("no dac3.bit_rate_code property", __FILE__,
                                    __LINE__, __FUNCTION__);
            }
            ...
            m_pTracks[FindTrackIndex(trackId)]->SetFixedSampleDuration(1536);

            return trackId;
        }

        MP4TrackId MP4File::AddEncAudioTrack(
            uint32_t timeScale, MP4Duration sampleDuration, uint8_t audioType,
            uint32_t scheme_type, uint16_t scheme_version, uint8_t key_ind_len,
            uint8_t iv_len, bool selective_enc, const char* kms_uri,
            bool use_ismacryp)
        {
            ...
            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the enca
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
            ...
            if (use_ismacryp)
            {
                ...
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.enca.sinf.frma.data-format",
                    original_fmt);

                (void)AddChildAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.enca.sinf"),
                    "schm");
                (void)AddChildAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.enca.sinf"),
                    "schi");
                (void)AddChildAtom(
                    MakeTrackName(trackId,
                                  "mdia.minf.stbl.stsd.enca.sinf.schi"),
                    "iKMS");
                (void)AddChildAtom(
                    MakeTrackName(trackId,
                                  "mdia.minf.stbl.stsd.enca.sinf.schi"),
                    "iSFM");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.enca.sinf.schm.scheme_type",
                    scheme_type);
                ...
                SetTrackStringProperty(
                    trackId, "mdia.minf.stbl.stsd.enca.sinf.schi.iKMS.kms_URI",
                    kms_uri);

                SetTrackIntegerProperty(trackId,
                                        "mdia.minf.stbl.stsd.enca.sinf.schi."
                                        "iSFM.selective-encryption",
                                        selective_enc);

                SetTrackIntegerProperty(trackId,
                                        "mdia.minf.stbl.stsd.enca.sinf.schi."
                                        "iSFM.key-indicator-length",
                                        key_ind_len);
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.enca.timeScale", timeScale);

            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.enca.esds.ESID", 0);
                                                ...
                                                const char* type)
        {
            ...
            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the mp4v
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
            ...
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsz.sampleSize",
                                    sampleDuration);
        ...
        {
            MP4TrackId trackId =
                AddCntlTrackDefault(timeScale, sampleDuration, "href");

            if (base_url != NULL)
            {
                (void)AddChildAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.href"), "burl");
        ...
        MP4TrackId MP4File::AddVideoTrackDefault(uint32_t timeScale,
                                                 MP4Duration sampleDuration,
                                                 uint16_t width,
                                                 uint16_t height,
                                                 const char* videoType)
        {
            ...
            (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "vmhd",
                                  0);
        ...
        MP4TrackId MP4File::AddMP4VideoTrack(uint32_t timeScale,
                                             MP4Duration sampleDuration,
                                             uint16_t width, uint16_t height,
                                             uint8_t videoType)
        {
            MP4TrackId trackId = AddVideoTrackDefault(timeScale, sampleDuration,
                                                      width, height, "mp4v");

            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.mp4v.width",
                                    width);
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.mp4v.height",
                                    height);

            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.mp4v.esds.ESID", 0);
                                             ...
                                             const char* oFormat)
        {
            ...
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.encv.width",
                                    width);
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.encv.height",
                                    height);
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.encv.sinf.frma.data-format",
                original_fmt);

            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.encv.sinf"),
                "schm");
            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.encv.sinf"),
                "schi");
            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.encv.sinf.schi"),
                "iKMS");
            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.encv.sinf.schi"),
                "iSFM");
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.encv.sinf.schm.scheme_type",
                icPp->scheme_type);

            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.encv.sinf.schm.scheme_version",
                icPp->scheme_version);

            SetTrackStringProperty(
                trackId, "mdia.minf.stbl.stsd.encv.sinf.schi.iKMS.kms_URI",
                icPp->kms_uri);
            ...
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.encv.sinf.schi.iSFM.IV-length",
                icPp->iv_len);

            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.encv.esds.ESID", 0);
        ...
        MP4TrackId MP4File::AddH264VideoTrack(
            uint32_t timeScale, MP4Duration sampleDuration, uint16_t width,
            uint16_t height, uint8_t AVCProfileIndication,
            uint8_t profile_compat, uint8_t AVCLevelIndication,
            uint8_t sampleLenFieldSizeMinusOne)
        {
            MP4TrackId trackId = AddVideoTrackDefault(timeScale, sampleDuration,
                                                      width, height, "avc1");

            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.avc1.width",
                                    width);
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.avc1.height",
                                    height);

            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.avc1.avcC.AVCProfileIndication",
                AVCProfileIndication);
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.avc1.avcC.profile_compatibility",
                profile_compat);
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.avc1.avcC.AVCLevelIndication",
                AVCLevelIndication);
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.avc1.avcC.lengthSizeMinusOne",
        ...
        MP4TrackId MP4File::AddEncH264VideoTrack(
            uint32_t timeScale, MP4Duration sampleDuration, uint16_t width,
            uint16_t height, MP4Atom* srcAtom, mp4v2_ismacrypParams* icPp)

        {
            ...
            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.encv"), "avcC");
            ...
            // avc1 must not appear as a sample entry name or original format
            // name
                                                  ...
                                                  uint16_t sequenceLen)
        {
            ...
            if (!strcasecmp(format, "avc1"))
                avcCAtom = FindAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1.avcC"));
            ...
            if ((avcCAtom->FindProperty("avcC.numOfSequenceParameterSets",
                                        (MP4Property**)&pCount)
                 == false)
                || (avcCAtom->FindProperty(
                        "avcC.sequenceEntries.sequenceParameterSetLength",
                        (MP4Property**)&pLength)
                    == false)
                || (avcCAtom->FindProperty(
                        "avcC.sequenceEntries.sequenceParameterSetNALUnit",
                        (MP4Property**)&pUnit)
                    == false))
            {
                                                 ...
                                                 uint16_t pictLen)
        {
            ...
            if ((avcCAtom->FindProperty("avcC.numOfPictureParameterSets",
                 ...
                 == false)
                || (avcCAtom->FindProperty(
                        "avcC.pictureEntries.pictureParameterSetLength",
                        (MP4Property**)&pLength)
                    == false)
                || (avcCAtom->FindProperty(
                        "avcC.pictureEntries.pictureParameterSetNALUnit",
                        (MP4Property**)&pUnit)
                    == false))
            {
                log.errorf(
                    "%s: \"%s\": Could not find avcC picture table properties",
                    __FUNCTION__, GetFilename().c_str());
                return;
            }
        ...
        void MP4File::SetH263Vendor(MP4TrackId trackId, uint32_t vendor)
        {
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.s263.d263.vendor", vendor);
        }

        void MP4File::SetH263DecoderVersion(MP4TrackId trackId,
                                            uint8_t decoderVersion)
        {
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.s263.d263.decoderVersion",
                decoderVersion);
        }

        void MP4File::SetH263Bitrates(MP4TrackId trackId, uint32_t avgBitrate,
                                      uint32_t maxBitrate)
        {
            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.s263.d263.bitr.avgBitrate",
                avgBitrate);

            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.s263.d263.bitr.maxBitrate",
                maxBitrate);
        }

        MP4TrackId MP4File::AddH263VideoTrack(
            uint32_t timeScale, MP4Duration sampleDuration, uint16_t width,
            uint16_t height, uint8_t h263Level, uint8_t h263Profile,
...

        {
            ...
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.s263.width",
                                    width);
            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.s263.height",
                                    height);

            SetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.s263.d263.h263Level", h263Level);

            SetTrackIntegerProperty(trackId,
                                    "mdia.minf.stbl.stsd.s263.d263.h263Profile",
                                    h263Profile);

            // Add the bitr atom
            (void)AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.s263.d263"),
                "bitr");
        ...
        MP4TrackId MP4File::AddHintTrack(MP4TrackId refTrackId)
        {
            ...
            (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "hmhd",
                                  0);
            ...
            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the rtp
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
            ...
            (void)AddDescendantAtoms(MakeTrackName(trackId, NULL),
                                     "udta.hnti.sdp ");
        ...
        MP4TrackId MP4File::AddTextTrack(MP4TrackId refTrackId)
        {
            ...
            (void)InsertChildAtom(MakeTrackName(trackId, "mdia.minf"), "gmhd",
                                  0);
            ...
            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the text
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
        ...
        MP4TrackId MP4File::AddSubtitleTrack(uint32_t timescale, uint16_t width,
                                             uint16_t height)
        {
            MP4TrackId trackId = AddTrack(MP4_SUBTITLE_TRACK_TYPE, timescale);
            ...
            MP4Atom* pFtabAtom = AddChildAtom(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.tx3g"), "ftab");

            ((MP4Integer16Property*)pFtabAtom->GetProperty(0))
                ->IncrementValue();

            MP4Integer16Property* pfontID =
                (MP4Integer16Property*)((MP4TableProperty*)
                                            pFtabAtom->GetProperty(1))
                    ->GetProperty(0);
            pfontID->AddValue(1);

            MP4StringProperty* pName =
                (MP4StringProperty*)((MP4TableProperty*)pFtabAtom->GetProperty(
                                         1))
                    ->GetProperty(1);
            pName->AddValue("Arial");

            SetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.tx3g.fontID",
                                    1);

            // stsd is a unique beast in that it has a count of the number
            // of child atoms that needs to be incremented after we add the tx3g
            // atom
            MP4Integer32Property* pStsdCountProperty;
            FindIntegerProperty(
                MakeTrackName(trackId, "mdia.minf.stbl.stsd.entryCount"),
        ...
        MP4TrackId MP4File::AddSubpicTrack(uint32_t timescale, uint16_t width,
                                           uint16_t height)
        {
            MP4TrackId trackId = AddTrack(MP4_SUBPIC_TRACK_TYPE, timescale);
                                                ...
                                                uint32_t timescale)
        {
            ...
            // as do all movies with chapter tracks made by hand in QuickTime
            // Pro
            (void)AddChildAtom(MakeTrackName(trackId, "mdia.minf.gmhd"),
                               "text");
            ...
            (void)AddDescendantAtoms(MakeTrackName(refTrackId, NULL),
                                     "tref.chap");
        ...
        MP4TrackId MP4File::AddPixelAspectRatio(MP4TrackId trackId,
                                                uint32_t hSpacing,
                                                uint32_t vSpacing)
        {
            // validate reference track id
            ...
            {
                (void)AddChildAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1"), "pasp");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.avc1.pasp.hSpacing",
                    hSpacing);
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.avc1.pasp.vSpacing",
                    vSpacing);
            }
            else if (!strcasecmp(format, "mp4v"))
            {
                (void)AddChildAtom(
                    MakeTrackName(trackId, "mdia.minf.stbl.stsd.mp4v"), "pasp");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.mp4v.pasp.hSpacing",
                    hSpacing);
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.mp4v.pasp.vSpacing",
                    vSpacing);
            }
        ...
        MP4TrackId MP4File::AddColr(MP4TrackId trackId, uint16_t primariesIndex,
                                    ...
                                    uint16_t matrixIndex)
        {
            ...
            if (!strcasecmp(format, "avc1"))
            {
                AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.avc1"),
                             "colr");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.avc1.colr.primariesIndex",
                    primariesIndex);
                SetTrackIntegerProperty(
                    trackId,
                    "mdia.minf.stbl.stsd.avc1.colr.transferFunctionIndex",
                    transferFunctionIndex);
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.avc1.colr.matrixIndex",
                    matrixIndex);
            }
            else if (!strcasecmp(format, "mp4v"))
            {
                AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.mp4v"),
                             "colr");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.mp4v.colr.primariesIndex",
                    primariesIndex);
                SetTrackIntegerProperty(
                    trackId,
                    "mdia.minf.stbl.stsd.mp4v.colr.transferFunctionIndex",
                    transferFunctionIndex);
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.mp4v.colr.matrixIndex",
                    matrixIndex);
            }
            else if (!strcasecmp(format, "jpeg"))
            {
                AddChildAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd.jpeg"),
                             "colr");
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.jpeg.colr.primariesIndex",
                    primariesIndex);
                SetTrackIntegerProperty(
                    trackId,
                    "mdia.minf.stbl.stsd.jpeg.colr.transferFunctionIndex",
                    transferFunctionIndex);
                SetTrackIntegerProperty(
                    trackId, "mdia.minf.stbl.stsd.jpeg.colr.matrixIndex",
                    matrixIndex);
            }
        ...
        void MP4File::AddChapter(MP4TrackId chapterTrackId,
                                 MP4Duration chapterDuration,
                                 const char* chapterTitle)
        {
            if (MP4_INVALID_TRACK_ID == chapterTrackId)
            {
                throw new Exception("No chapter track given", __FILE__,
                                    __LINE__, __FUNCTION__);
            }
            ...
            {
                textLen = min((uint32_t)strlen(chapterTitle),
                              (uint32_t)MP4V2_CHAPTER_TITLE_MAX);
                if (0 < textLen)
                {
            ...
            else
            {
                ...
                snprintf(text, 1023, "Chapter %03d",
                         pChapterTrack->GetNumberOfSamples() + 1);
                textLen = (uint32_t)strlen(text);
            }

            sampleLength =
                textLen + 2
                + 12; // Account for text length code and other marker
        ...
        void MP4File::AddNeroChapter(MP4Timestamp chapterStart,
                                     const char* chapterTitle)
        {
            ...
            MP4Integer32Property* pCount =
                (MP4Integer32Property*)pChpl->GetProperty(3);
            pCount->IncrementValue();
            ...
            if (pChpl->FindProperty("chpl.chapters", (MP4Property**)&pTable))
            {
                MP4Integer64Property* pStartTime =
                    (MP4Integer64Property*)pTable->GetProperty(0);
                MP4StringProperty* pName =
                    (MP4StringProperty*)pTable->GetProperty(1);
        ...
        MP4TrackId MP4File::FindChapterReferenceTrack(MP4TrackId chapterTrackId,
                                                      char* trackName,
                                                      int trackNameSize)
        {
            for (uint32_t i = 0; i < m_pTracks.Size(); i++)
            {
                    ...
                    || MP4_IS_AUDIO_TRACK_TYPE(m_pTracks[i]->GetType()))
                {
                    ...
                    if (FindTrackReference(name, chapterTrackId))
                    {
                        if (0 != trackName)
                        {
                            int nameLen = min((uint32_t)strlen(name),
                                              (uint32_t)trackNameSize);
        ...
        MP4TrackId MP4File::FindChapterTrack(char* trackName, int trackNameSize)
        {
            for (uint32_t i = 0; i < m_pTracks.Size(); i++)
            {
                ...
                {
                    MP4TrackId refTrackId = FindChapterReferenceTrack(
                        m_pTracks[i]->GetId(), trackName, trackNameSize);
                                               ...
                                               MP4TrackId chapterTrackId)
        {
                ...
                || MP4ChapterTypeQt == chapterType)
            {
                ...
                if (MP4_INVALID_TRACK_ID != chapterTrackId && 0 != trackName[0])
                {
                    ...
                    deletedType = MP4ChapterTypeNone == deletedType
                                      ? MP4ChapterTypeQt
                                      : MP4ChapterTypeAny;
                }
            }
        ...
        MP4ChapterType MP4File::GetChapters(MP4Chapter_t** chapterList,
                                            uint32_t* chapterCount,
                                            MP4ChapterType fromChapterType)
        {
            *chapterList = 0;
            *chapterCount = 0;

            if (MP4ChapterTypeAny == fromChapterType
                || MP4ChapterTypeQt == fromChapterType)
            {
                uint8_t* sample = 0;
                ...
                else
                {
                    ...
                    if (0 < counter)
                    {
                        ...
                        MP4Chapter_t* chapters = (MP4Chapter_t*)MP4Malloc(
                            sizeof(MP4Chapter_t) * counter);
                        ...
                        for (uint32_t i = 0; i < counter; ++i)
                        {
                            ...
                            MP4SampleId sampleId =
                                pChapterTrack->GetSampleIdFromTime(
                                    startTime + duration, true);
                            pChapterTrack->ReadSample(sampleId, &sample,
                                                      &sampleSize);

                            // get the starttime and duration
                            pChapterTrack->GetSampleTimes(sampleId, &startTime,
                                                          &duration);

                            // we know that sample+2 contains the title
                            // (sample[0] and sample[1] is the length)
                            const char* title = (const char*)&(sample[2]);
                            int titleLen =
                                min((uint32_t)((sample[0] << 8) | sample[1]),
                                    (uint32_t)MP4V2_CHAPTER_TITLE_MAX);
                            ...
                            chapters[i].duration =
                                MP4ConvertTime(duration, timescale,
            ...
            if (MP4ChapterTypeAny == fromChapterType
                || MP4ChapterTypeNero == fromChapterType)
            {
                MP4Atom* pChpl = FindAtom("moov.udta.chpl");
                if (!pChpl)
                {
                ...
                if (!pChpl->FindProperty("chpl.chaptercount",
                                         (MP4Property**)&pCounter))
                {
                    log.warningf(
                        "%s: \"%s\": Nero chapter count does not exist",
                        __FUNCTION__, GetFilename().c_str());
                    return MP4ChapterTypeNone;
                }
                ...
                if (!pChpl->FindProperty("chpl.chapters",
                                         (MP4Property**)&pTable))
                {
                ...
                {
                    log.warningf(
                        "%s: \"%s\": List of Chapter starttimes does not exist",
                        __FUNCTION__, GetFilename().c_str());
                    return MP4ChapterTypeNone;
                }
                ...
                {
                    log.warningf(
                        "%s: \"%s\": List of Chapter titles does not exist",
                        __FUNCTION__, GetFilename().c_str());
                    return MP4ChapterTypeNone;
                }

                MP4Chapter_t* chapters =
                    (MP4Chapter_t*)MP4Malloc(sizeof(MP4Chapter_t) * counter);
                ...
                for (i = 0, j = 1; i < counter; ++i, ++j)
                {
                    ...
                    uint32_t len = min((uint32_t)strlen(name),
                                       (uint32_t)MP4V2_CHAPTER_TITLE_MAX);
                    ...
                    {
                        duration =
                            MP4ConvertTime(pStartTime->GetValue(j),
                    ...
                    {
                        // last chapter
                        duration = MP4ConvertTime(GetDuration(), GetTimeScale(),
                                                  MP4_MILLISECONDS_TIME_SCALE)
                                   - chapterDurationSum;
                    }
        ...
        MP4ChapterType MP4File::SetChapters(MP4Chapter_t* chapterList,
                                            uint32_t chapterCount,
                                            MP4ChapterType toChapterType)
        {
            ...
            if (MP4ChapterTypeAny == toChapterType
                || MP4ChapterTypeNero == toChapterType)
            {
                MP4Duration duration = 0;
                for (uint32_t i = 0; i < chapterCount; ++i)
                {
                    AddNeroChapter(duration, chapterList[i].title);
                    duration += 10 * MP4_MILLISECONDS_TIME_SCALE
                                * chapterList[i].duration;
                }
            ...
            if (MP4ChapterTypeAny == toChapterType
                || MP4ChapterTypeQt == toChapterType)
            {
                ...
                MP4TrackId chapterTrack =
                    AddChapterTextTrack(refTrack, MP4_MILLISECONDS_TIME_SCALE);

                for (uint32_t i = 0; i < chapterCount; ++i)
                {
                    // create and write the chapter track sample
                    AddChapter(chapterTrack, chapterList[i].duration,
                               chapterList[i].title);
                }

                setType = MP4ChapterTypeNone == setType ? MP4ChapterTypeQt
                                                        : MP4ChapterTypeAny;
            }
        ...
        MP4ChapterType MP4File::ConvertChapters(MP4ChapterType toChapterType)
        {
            ...
            if (0 == chapterCount)
            {
                log.warningf("%s: \"%s\": %s", __FUNCTION__,
                             GetFilename().c_str(), errMsg);
                return MP4ChapterTypeNone;
            }
        ...
        void MP4File::ChangeMovieTimeScale(uint32_t timescale)
        {
            ...
            SetDuration(
                MP4ConvertTime(movieDuration, origTimeScale, timescale));

            // set track header duration (calculated with movie header
            // timescale)
            ...
            for (uint32_t i = 0; i < trackCount; ++i)
            {
                ...
                if (trackAtom.FindProperty("trak.tkhd.duration",
                                           (MP4Property**)&duration))
                {
                    duration->SetValue(MP4ConvertTime(
                        duration->GetValue(), origTimeScale, timescale));
                }
            }
        }
        ...
        MP4TrackId MP4File::AllocTrackId()
        {
            ...
            throw new Exception("too many existing tracks", __FILE__, __LINE__,
                                __FUNCTION__);
            return MP4_INVALID_TRACK_ID; // to keep MSVC happy
        }

        MP4TrackId MP4File::FindTrackId(uint16_t trackIndex, const char* type,
                                        uint8_t subType)
        {
            if (type == NULL)
            {
            ...
            ostringstream msg;
            msg << "Track index doesn't exist - track " << trackIndex
                << " type " << type;
        ...
        uint32_t MP4File::GetSampleSize(MP4TrackId trackId,
                                        MP4SampleId sampleId)
        {
        ...
        MP4SampleId MP4File::GetSampleIdFromTime(MP4TrackId trackId,
                                                 ...
                                                 bool wantSyncSample)
        {
            return m_pTracks[FindTrackIndex(trackId)]->GetSampleIdFromTime(
                when, wantSyncSample);
        }

        MP4Timestamp MP4File::GetSampleTime(MP4TrackId trackId,
                                            MP4SampleId sampleId)
        {
            MP4Timestamp timestamp;
            m_pTracks[FindTrackIndex(trackId)]->GetSampleTimes(
                sampleId, &timestamp, NULL);
            return timestamp;
        }

        MP4Duration MP4File::GetSampleDuration(MP4TrackId trackId,
                                               MP4SampleId sampleId)
        {
            MP4Duration duration;
            m_pTracks[FindTrackIndex(trackId)]->GetSampleTimes(sampleId, NULL,
                                                               &duration);
            return duration;
        }

        MP4Duration MP4File::GetSampleRenderingOffset(MP4TrackId trackId,
                                                      MP4SampleId sampleId)
        {
            return m_pTracks[FindTrackIndex(trackId)]->GetSampleRenderingOffset(
                sampleId);
        }
        ...
        void MP4File::ReadSample(MP4TrackId trackId, MP4SampleId sampleId,
                                 ...
                                 bool* pIsSyncSample, bool* hasDependencyFlags,
                                 uint32_t* dependencyFlags)
        {
            m_pTracks[FindTrackIndex(trackId)]->ReadSample(
                sampleId, ppBytes, pNumBytes, pStartTime, pDuration,
                pRenderingOffset, pIsSyncSample, hasDependencyFlags,
                dependencyFlags);
        }

        void MP4File::WriteSample(MP4TrackId trackId, const uint8_t* pBytes,
                                  uint32_t numBytes, MP4Duration duration,
        ...
        void MP4File::WriteSampleDependency(
            MP4TrackId trackId, const uint8_t* pBytes, uint32_t numBytes,
            MP4Duration duration, MP4Duration renderingOffset,
            bool isSyncSample, uint32_t dependencyFlags)
        {
            ProtectWriteOperation(__FILE__, __LINE__, __FUNCTION__);
        ...
        {
            ProtectWriteOperation(__FILE__, __LINE__, __FUNCTION__);
            m_pTracks[FindTrackIndex(trackId)]->SetSampleRenderingOffset(
                sampleId, renderingOffset);

            m_pModificationProperty->SetValue(MP4GetAbsTimestamp());
        }

        void MP4File::MakeFtypAtom(char* majorBrand, uint32_t minorVersion,
        ...
        char* MP4File::MakeTrackName(MP4TrackId trackId, const char* name)
        {
            ...
            {
                snprintf(m_trakName, sizeof(m_trakName), "moov.trak[%u]",
                         trakIndex);
            }
            else
            {
                snprintf(m_trakName, sizeof(m_trakName), "moov.trak[%u].%s",
                         trakIndex, name);
            }
            return m_trakName;
        }
        ...
        uint64_t MP4File::GetTrackIntegerProperty(MP4TrackId trackId,
                                                  const char* name)
        {
            return GetIntegerProperty(MakeTrackName(trackId, name));
        }

        void MP4File::SetTrackIntegerProperty(MP4TrackId trackId,
                                              const char* name, int64_t value)
        {
            SetIntegerProperty(MakeTrackName(trackId, name), value);
        }

        float MP4File::GetTrackFloatProperty(MP4TrackId trackId,
                                             const char* name)
        {
            return GetFloatProperty(MakeTrackName(trackId, name));
        }

        void MP4File::SetTrackFloatProperty(MP4TrackId trackId,
                                            const char* name, float value)
        {
            SetFloatProperty(MakeTrackName(trackId, name), value);
        }

        const char* MP4File::GetTrackStringProperty(MP4TrackId trackId,
                                                    const char* name)
        {
            return GetStringProperty(MakeTrackName(trackId, name));
        }

        void MP4File::SetTrackStringProperty(MP4TrackId trackId,
                                             const char* name,
                                             const char* value)
        {
        ...
        void MP4File::GetTrackBytesProperty(MP4TrackId trackId,
                                            const char* name, uint8_t** ppValue,
                                            uint32_t* pValueSize)
        {
            GetBytesProperty(MakeTrackName(trackId, name), ppValue, pValueSize);
        }

        void MP4File::SetTrackBytesProperty(MP4TrackId trackId,
                                            const char* name,
        ...
        {
            ostringstream oss;
            oss << "moov.trak[" << FindTrakAtomIndex(trackId)
                << "].mdia.mdhd.language";
            ...
            MP4LanguageCodeProperty& lang =
                *static_cast<MP4LanguageCodeProperty*>(prop);
            string slang;
        ...
        bool MP4File::GetTrackName(MP4TrackId trackId, char** name)
        {
            ...
            pMetaAtom =
                m_pRootAtom->FindAtom(MakeTrackName(trackId, "udta.name"));

            if (pMetaAtom)
            {
                GetBytesProperty(MakeTrackName(trackId, "udta.name.value"),
                                 (uint8_t**)&val, &valSize);
            }
            if (valSize > 0)
            {
        ...
        void MP4File::SetTimeScale(uint32_t value)
        {
            if (value == 0)
            {
                throw new Exception("invalid value", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
        ...
        void MP4File::AppendSessionSdp(const char* sdpFragment)
        {
            ...
            char* newSdpString = (char*)MP4Malloc(strlen(oldSdpString)
                                                  + strlen(sdpFragment) + 1);
                                                      ...
                                                      uint32_t buflen)
        {
            ...
            format = GetTrackIntegerProperty(
                trackId, "mdia.minf.stbl.stsd.*.sinf.frma.data-format");
        ...
        const char* MP4File::GetTrackMediaDataName(MP4TrackId trackId)
        {
            MP4Atom* pChild;
            MP4Atom* pAtom =
                FindAtom(MakeTrackName(trackId, "mdia.minf.stbl.stsd"));
            if (pAtom->GetNumberOfChildAtoms() != 1)
            {
                log.errorf(
                    "%s: \"%s\": track %d has more than 1 child atoms in stsd",
        ...
        double MP4File::GetTrackVideoFrameRate(MP4TrackId trackId)
        {
            ...
            uint64_t msDuration = ConvertFromTrackDuration(
                trackId, GetTrackDuration(trackId), MP4_MSECS_TIME_SCALE);
            ...
            return ((double)numSamples / double(msDuration))
                                              ...
                                              uint32_t* pConfigSize)
        {
            try
            {
                GetTrackBytesProperty(trackId,
                                      "mdia.minf.stbl.stsd.*[0].esds."
                                      "decConfigDescr.decSpecificInfo[0].info",
                                              ...
                                              uint32_t configSize)
        {
                ...
                || pConfigDescrProperty == NULL)
            {
                ...
                throw new Exception("no such property", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            (void)pConfigDescrProperty->FindProperty(
                "decSpecificInfo[0].info", (MP4Property**)&pInfoProperty);
                                                 ...
                                                 uint32_t** ppPictHeaderSize)
        {
            ...
            *ppPictHeaderSize = NULL;
            ...
            uint8_t** ppSeqHeader = (uint8_t**)malloc(
                (pSeqCount->GetValue() + 1) * sizeof(uint8_t*));
            if (ppSeqHeader == NULL)
                return;
            *pppSeqHeader = ppSeqHeader;

            uint32_t* pSeqHeaderSize = (uint32_t*)malloc(
                (pSeqCount->GetValue() + 1) * sizeof(uint32_t*));
            ...
            *ppSeqHeaderSize = pSeqHeaderSize;
            ...
            {
                pSeqVal->GetValue(&(ppSeqHeader[count]),
                                  &(pSeqHeaderSize[count]), count);
            }
            ppSeqHeader[count] = NULL;
            pSeqHeaderSize[count] = 0;

            if ((avcCAtom->FindProperty("avcC.numOfPictureParameterSets",
                                        (MP4Property**)&pPictCount)
                 == false)
                || (avcCAtom->FindProperty(
                        "avcC.pictureEntries.pictureParameterSetLength",
                        (MP4Property**)&pPictLen)
                    == false)
                || (avcCAtom->FindProperty(
                        "avcC.pictureEntries.pictureParameterSetNALUnit",
                        (MP4Property**)&pPictVal)
                    == false))
            {
                log.errorf(
                    "%s: \"%s\": Could not find avcC picture table properties",
            ...
            uint8_t** ppPictHeader = (uint8_t**)malloc(
                (pPictCount->GetValue() + 1) * sizeof(uint8_t*));
            if (ppPictHeader == NULL)
                return;
            uint32_t* pPictHeaderSize = (uint32_t*)malloc(
                (pPictCount->GetValue() + 1) * sizeof(uint32_t*));
            ...
            *ppPictHeaderSize = pPictHeaderSize;
            ...
            {
                pPictVal->GetValue(&(ppPictHeader[count]),
                                   &(pPictHeaderSize[count]), count);
            }
        ...
        {
            return GetTrackStringProperty(hintTrackId,
                                          "udta.hnti.sdp .sdpText");
        }

        void MP4File::SetHintTrackSdp(MP4TrackId hintTrackId,
                                      const char* sdpString)
        {
            ...
            {
                throw new Exception("track is not a hint track", __FILE__,
                                    __LINE__, __FUNCTION__);
            }

            (void)AddDescendantAtoms(MakeTrackName(hintTrackId, NULL),
                                     "udta.hnti.sdp ");

            SetTrackStringProperty(hintTrackId, "udta.hnti.sdp .sdpText",
                                   sdpString);
        }
                                             ...
                                             char** ppEncodingParams)
        {
            ...
            ((MP4RtpHintTrack*)pTrack)
                ->GetPayload(ppPayloadName, pPayloadNumber, pMaxPayloadSize,
                             ppEncodingParams);
        }
                                             ...
                                             bool include_mpeg4_esid)
        {
            ...
            ((MP4RtpHintTrack*)pTrack)
                ->SetPayload(payloadName, payloadNumber, maxPayloadSize,
                             encoding_params, include_rtp_map,
                             include_mpeg4_esid);
        }
        ...
        void MP4File::ReadRtpHint(MP4TrackId hintTrackId,
        ...
        uint16_t MP4File::GetRtpHintNumberOfPackets(MP4TrackId hintTrackId)
        {
            MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
        ...
        int8_t MP4File::GetRtpPacketBFrame(MP4TrackId hintTrackId,
        ...
        int32_t MP4File::GetRtpPacketTransmitOffset(MP4TrackId hintTrackId,
                                                    uint16_t packetIndex)
        {
            MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
            ...
            return ((MP4RtpHintTrack*)pTrack)
                ->GetPacketTransmitOffset(packetIndex);
        }

        void MP4File::ReadRtpPacket(MP4TrackId hintTrackId,
                                    uint16_t packetIndex, uint8_t** ppBytes,
                                    uint32_t* pNumBytes, uint32_t ssrc,
                                    bool includeHeader, bool includePayload)
        {
            MP4Track* pTrack = m_pTracks[FindTrackIndex(hintTrackId)];
            ...
            ((MP4RtpHintTrack*)pTrack)
                ->ReadPacket(packetIndex, ppBytes, pNumBytes, ssrc,
                             includeHeader, includePayload);
        }
        ...
        void MP4File::SetRtpTimestampStart(MP4TrackId hintTrackId,
        ...
        void MP4File::AddRtpHint(MP4TrackId hintTrackId, bool isBframe,
                                       ...
                                       uint32_t dataOffset, uint32_t dataLength)
        {
            ...
            ((MP4RtpHintTrack*)pTrack)
                ->AddSampleData(sampleId, dataOffset, dataLength);
        }
        ...
        void MP4File::WriteRtpHint(MP4TrackId hintTrackId, MP4Duration duration,
                                   bool isSyncSample)
        {
            ProtectWriteOperation(__FILE__, __LINE__, __FUNCTION__);
        ...
        uint64_t MP4File::ConvertFromMovieDuration(MP4Duration duration,
                                                   uint32_t timeScale)
        {
            return MP4ConvertTime((uint64_t)duration, GetTimeScale(),
                                  timeScale);
        }

        uint64_t MP4File::ConvertFromTrackTimestamp(MP4TrackId trackId,
                                                    MP4Timestamp timeStamp,
                                                    uint32_t timeScale)
        {
            return MP4ConvertTime(timeStamp, GetTrackTimeScale(trackId),
                                  timeScale);
        }

        MP4Timestamp MP4File::ConvertToTrackTimestamp(MP4TrackId trackId,
                                                      uint64_t timeStamp,
                                                      uint32_t timeScale)
        {
            return (MP4Timestamp)MP4ConvertTime(timeStamp, timeScale,
                                                GetTrackTimeScale(trackId));
        }

        uint64_t MP4File::ConvertFromTrackDuration(MP4TrackId trackId,
                                                   MP4Duration duration,
                                                   uint32_t timeScale)
        {
        ...
        MP4Duration MP4File::ConvertToTrackDuration(MP4TrackId trackId,
                                                    uint64_t duration,
                                                    uint32_t timeScale)
        {
            return (MP4Duration)MP4ConvertTime(duration, timeScale,
                                               GetTrackTimeScale(trackId));
        }
        ...
        char* MP4File::MakeTrackEditName(MP4TrackId trackId, MP4EditId editId,
                                         const char* name)
        {
            ...
            snprintf(m_editName, 1024, "%s.edts.elst.entries[%u].%s", trakName,
                     editId - 1, name);
            return m_editName;
        }
        ...
        uint32_t MP4File::GetTrackNumberOfEdits(MP4TrackId trackId)
        {
            return GetTrackIntegerProperty(trackId, "edts.elst.entryCount");
        }

        MP4Duration MP4File::GetTrackEditTotalDuration(MP4TrackId trackId,
                                                       MP4EditId editId)
        {
            return m_pTracks[FindTrackIndex(trackId)]->GetEditTotalDuration(
                editId);
        }

        MP4Timestamp MP4File::GetTrackEditStart(MP4TrackId trackId,
                                                MP4EditId editId)
        {
            return m_pTracks[FindTrackIndex(trackId)]->GetEditStart(editId);
        }

        MP4Timestamp MP4File::GetTrackEditMediaStart(MP4TrackId trackId,
                                                     MP4EditId editId)
        {
        ...
        void MP4File::SetTrackEditMediaStart(MP4TrackId trackId,
                                             MP4EditId editId,
        ...
        MP4Duration MP4File::GetTrackEditDuration(MP4TrackId trackId,
                                                  MP4EditId editId)
        {
            return GetIntegerProperty(
        ...
        void MP4File::SetTrackEditDuration(MP4TrackId trackId, MP4EditId editId,
                                           MP4Duration duration)
        {
            SetIntegerProperty(
                MakeTrackEditName(trackId, editId, "segmentDuration"),
        ...
        void MP4File::SetTrackEditDwell(MP4TrackId trackId, MP4EditId editId,
                                        bool dwell)
        {
            SetIntegerProperty(MakeTrackEditName(trackId, editId, "mediaRate"),
                               (dwell ? 0 : 1));
        }
        ...
        void MP4File::SetTrackDurationPerChunk(MP4TrackId trackId,
                                               MP4Duration duration)
        {
            m_pTracks[FindTrackIndex(trackId)]->SetDurationPerChunk(duration);
        }

        void MP4File::CopySample(MP4File* srcFile, MP4TrackId srcTrackId,
                                 MP4SampleId srcSampleId, MP4File* dstFile,
                                 ...
                                 MP4Duration dstSampleDuration)
        {
            ...
            srcFile->ReadSample(srcTrackId, srcSampleId, &pBytes, &numBytes,
                                NULL, &sampleDuration, &renderingOffset,
                                &isSyncSample, &hasDependencyFlags,
                                &dependencyFlags);
            ...
            {
                dstFile->WriteSampleDependency(dstTrackId, pBytes, numBytes,
                                               sampleDuration, renderingOffset,
                                               isSyncSample, dependencyFlags);
            }
            else
            {
                dstFile->WriteSample(dstTrackId, pBytes, numBytes,
                                     sampleDuration, renderingOffset,
                                     isSyncSample);
            }
        ...
        void MP4File::EncAndCopySample(MP4File* srcFile, MP4TrackId srcTrackId,
                                       MP4SampleId srcSampleId,
                                       encryptFunc_t encfcnp,
                                       uint32_t encfcnparam1, MP4File* dstFile,
                                       MP4TrackId dstTrackId,
                                       MP4Duration dstSampleDuration)
        {
            // Note: we leave it up to the caller to ensure that the
            ...
            // if( ismacrypEncryptSampleAddHeader( ismaCryptSId, numBytes,
            // pBytes, &encSampleLength, &encSampleData ) != 0)
            if (encfcnp(encfcnparam1, numBytes, pBytes, &encSampleLength,
                        &encSampleData)
                != 0)
                log.errorf(
                    "%s(%s,%s) Can't encrypt the sample and add its header %u",
                    __FUNCTION__, srcFile->GetFilename().c_str(),
                    dstFile->GetFilename().c_str(), srcSampleId);
            ...
            {
                dstFile->WriteSample(dstTrackId, encSampleData, encSampleLength,
                                     sampleDuration, renderingOffset,
                                     isSyncSample);
            }
