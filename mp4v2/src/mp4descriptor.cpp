...
namespace mp4v2
{
    namespace impl
    {
        ...
        {
            log.verbose1f("\"%s\": ReadDescriptor: pos = 0x%" PRIx64,
                          file.GetFilename().c_str(), file.GetPosition());
            ...
            log.verbose1f(
                "\"%s\": ReadDescriptor: tag 0x%02x data size %u (0x%x)",
                file.GetFilename().c_str(), m_tag, m_size, m_size);
        }

        void MP4Descriptor::ReadProperties(MP4File& file,
                                           uint32_t propStartIndex,
                                           uint32_t propCount)
        {
            uint32_t numProperties =
                min(propCount, m_pProperties.Size() - propStartIndex);
                 ...
                 i < propStartIndex + numProperties; i++)
            {
                ...
                if (pProperty->GetType() == DescriptorProperty)
                {
                    if (remaining > 0)
                    {
                        ...
                        ((MP4DescriptorProperty*)pProperty)
                            ->SetSizeLimit(remaining);
                        pProperty->Read(file);
                ...
                else
                {
                    ...
                    {
                        log.errorf("%s: \"%s\": Overran descriptor, tag %u "
                                   "data size %u property %u",
                                   __FUNCTION__, file.GetFilename().c_str(),
                                   m_tag, m_size, i);
                        throw new Exception("overran descriptor", __FILE__,
        ...
        void MP4Descriptor::WriteToMemory(MP4File& file, uint8_t** ppBytes,
                                          uint64_t* pNumBytes)
        {
        ...
        uint8_t MP4Descriptor::GetDepth() { return m_parentAtom.GetDepth(); }
