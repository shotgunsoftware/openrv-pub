...
namespace mp4v2
{
    namespace impl
    {
        ...
        /*! \brief Patch class for read/write operations when string is
           0-length.

            We want to use string property, but mpeg4ip doesn't support ohdr way
           of encoding of string (in ohdr atom we first have 3 lengths of 3
           strings and then their string values, and it cannot be simulated with
           any of the current mpeg4ip string property parameters), so we have to
           write our own Read() and Write() routines.
        */
        ...
        public:
            ...
            */
            OhdrMP4StringProperty(MP4Atom& parentAtom, const char* name,
                                  bool useCountedFormat = false,
                                  bool useUnicode = false)
                : MP4StringProperty(parentAtom, name, useCountedFormat,
                                    useUnicode)
            {
            }
        ...
        /*! \brief OMA DRM headers atom.
            ...
            Contained in OMA DRM key management atom. It must contain content
           identifier.
        */
        /*! \brief Constructor.
         */
        MP4OhdrAtom::MP4OhdrAtom(MP4File& file)
            : MP4Atom(file, "ohdr")
        {
            AddVersionAndFlags();
            ...
            AddProperty(
                new MP4Integer16Property(*this, "RightsIssuerURLLength"));
            AddProperty(
                new MP4Integer16Property(*this, "TextualHeadersLength"));
        ...
        void MP4OhdrAtom::Read()
        {
            ...
            ((OhdrMP4StringProperty*)property)
                ->SetFixedLength(
            ...
            ((MP4BytesProperty*)property)
                ->SetFixedSize(((MP4Integer16Property*)lProperty)->GetValue());
            ReadProperties(8, 3);
        }
