...
namespace mp4v2
{
    namespace impl
    {
        ...
        MP4Atom* MP4Atom::CreateAtom(MP4File& file, MP4Atom* parent,
                                     const char* type)
        {
        ...
        MP4Atom* MP4Atom::ReadAtom(MP4File& file, MP4Atom* pParentAtom)
        {
            ...
            log.verbose1f("\"%s\": pos = 0x%" PRIx64,
                          file.GetFilename().c_str(), pos);
            ...
            log.verbose1f("\"%s\": type = \"%s\" data-size = %" PRIu64
                          " (0x%" PRIx64 ") hdr %u",
                          file.GetFilename().c_str(), type, dataSize, dataSize,
                          hdrSize);

            if (pos + hdrSize + dataSize > pParentAtom->GetEnd())
            {
                log.errorf(
                    "%s: \"%s\": invalid atom size, extends outside parent "
                    "atom - skipping to end of \"%s\" \"%s\" %" PRIu64
                    " vs %" PRIu64,
                    __FUNCTION__, file.GetFilename().c_str(),
                    pParentAtom->GetType(), type, pos + hdrSize + dataSize,
                    pParentAtom->GetEnd());
                log.verbose1f("\"%s\": parent %s (%" PRIu64 ") pos %" PRIu64
                              " hdr %d data %" PRIu64 " sum %" PRIu64,
                              file.GetFilename().c_str(),
                              pParentAtom->GetType(), pParentAtom->GetEnd(),
                              pos, hdrSize, dataSize, pos + hdrSize + dataSize);
            ...
            if (pAtom->IsUnknownType())
            {
                ...
                {
                    log.warningf("%s: \"%s\": atom type %s is suspect",
                                 __FUNCTION__, file.GetFilename().c_str(),
                                 pAtom->GetType());
                }
                else
                {
                    log.verbose1f("\"%s\": Info: atom type %s is unknown",
                                  file.GetFilename().c_str(), pAtom->GetType());
                }
        ...
        void MP4Atom::Read()
        {
            ...
            {
                log.verbose1f("%s: \"%s\": %s atom size %" PRIu64 " is suspect",
                              __FUNCTION__, m_File.GetFilename().c_str(),
                              m_type, m_size);
            }
        ...
        bool MP4Atom::FindProperty(const char* name, MP4Property** ppProperty,
                                   uint32_t* pIndex)
        {
            if (!IsMe(name))
            {
                                            ...
                                            uint32_t* pIndex)
        {
            ...
            for (i = 0; i < m_pChildAtoms.Size(); i++)
            {
                if (MP4NameFirstMatches(m_pChildAtoms[i]->GetType(), name))
                {
                    ...
                    {
                        // this is the one, ask it to match
                        return m_pChildAtoms[i]->FindProperty(name, ppProperty,
                                                              pIndex);
                    }
                    atomIndex--;
                }
        ...
        void MP4Atom::ReadProperties(uint32_t startIndex, uint32_t count)
        {
            uint32_t numProperties =
                min(count, m_pProperties.Size() - startIndex);
            ...
            for (uint32_t i = startIndex; i < startIndex + numProperties; i++)
            {
                ...
                if (m_File.GetPosition() > m_end)
                {
                    log.verbose1f("ReadProperties: insufficient data for "
                                  "property: %s pos 0x%" PRIx64
                                  " atom end 0x%" PRIx64,
                    ...
                    ostringstream oss;
                    oss << "atom '" << GetType()
                        << "' is too small; overrun at property: "
                        << m_pProperties[i]->GetName();
        ...
        void MP4Atom::ReadChildAtoms()
        {
            ...
            log.verbose1f("\"%s\": of %s", m_File.GetFilename().c_str(),
                          m_type[0] ? m_type : "root");
            for (uint64_t position = m_File.GetPosition(); position < m_end;
                 position = m_File.GetPosition())
            {
                ...
                if (m_end - position < 2 * sizeof(uint32_t))
                {
                    ...
                    if (this_is_udta && m_end - position == sizeof(uint32_t))
                    {
                        ...
                        if (mbz != 0)
                        {
                            log.warningf("%s: \"%s\": In udta atom, end value "
                                         "is not zero %x",
                                         __FUNCTION__,
                    ...
                    log.warningf("%s: \"%s\": In %s atom, extra %" PRId64
                                 " bytes at end of atom",
                                 __FUNCTION__, m_File.GetFilename().c_str(),
                ...
                MP4AtomInfo* pChildAtomInfo =
                    FindAtomInfo(pChildAtom->GetType());
                ...
                {
                    log.verbose1f(
                        "%s: \"%s\": In atom %s unexpected child atom %s",
                        __FUNCTION__, m_File.GetFilename().c_str(), GetType(),
                        pChildAtom->GetType());
                }
                ...
                if (pChildAtomInfo)
                {
                    ...
                    if (pChildAtomInfo->m_onlyOne
                        && pChildAtomInfo->m_count > 1)
                    {
                        log.warningf(
                            "%s: \"%s\": In atom %s multiple child atoms %s",
                            __FUNCTION__, m_File.GetFilename().c_str(),
            ...
            for (uint32_t i = 0; i < numAtomInfo; i++)
            {
                ...
                {
                    log.warningf("%s: \"%s\": In atom %s missing child atom %s",
                                 __FUNCTION__, m_File.GetFilename().c_str(),
                                 GetType(), m_pChildAtomInfos[i]->m_name);
                }
            }

            log.verbose1f("\"%s\": finished %s", m_File.GetFilename().c_str(),
                          m_type);
        }
        ...
        void MP4Atom::WriteProperties(uint32_t startIndex, uint32_t count)
        {
            ...
            log.verbose1f("Write: \"%s\": type %s",
                          m_File.GetFilename().c_str(), m_type);
        ...
        void MP4Atom::WriteChildAtoms()
        {
            ...
            log.verbose1f("Write: \"%s\": finished %s",
                          m_File.GetFilename().c_str(), m_type);
        }
        ...
        void MP4Atom::AddReserved(MP4Atom& parentAtom, const char* name,
                                  uint32_t size)
        {
            MP4BytesProperty* pReserved =
                new MP4BytesProperty(parentAtom, name, size);
            pReserved->SetReadOnly();
            AddProperty(pReserved);
        }

        void MP4Atom::ExpectChildAtom(const char* name, bool mandatory,
                                      bool onlyOne)
        {
        ...
        void MP4Atom::Dump(uint8_t indent, bool dumpImplicits)
        {
            ...
            for (i = 0; i < size; i++)
            {
                ...
                {
                    log.dump(indent + 1, MP4_LOG_VERBOSE1,
        ...
        bool MP4Atom::GetLargesizeMode() { return m_largesizeMode; }

        void MP4Atom::SetLargesizeMode(bool mode) { m_largesizeMode = mode; }

        bool MP4Atom::descendsFrom(MP4Atom* parent, const char* type)
        {
        ...
        MP4Atom* MP4Atom::factory(MP4File& file, MP4Atom* parent,
                                  const char* type)
        {
            ...
            if (parent)
            {
                ...
                else if (ATOMID(ptype) == ATOMID("tmcd"))
                {
                    if (ATOMID(type) == ATOMID("name"))
                        return new MP4TmcdNameAtom(file);
                }
            ...
            {
            case 'A':
                if (ATOMID(type) == ATOMID("AVdn"))
                    return new MP4AVdnAtom(file);
                if (ATOMID(type) == ATOMID("ACLR"))
                    return new MP4ACLRAtom(file);
                if (ATOMID(type) == ATOMID("APRG"))
                    return new MP4APRGAtom(file);
                if (ATOMID(type) == ATOMID("ARES"))
                    return new MP4ARESAtom(file);
                break;
            case 'S':
            ...
            case 't':
                ...
                if (ATOMID(type) == ATOMID("twos"))
                    return new MP4SoundAtom(file, type);
                if (ATOMID(type) == ATOMID("tmcd"))
                    return new MP4TmcdAtom(file);
                break;
