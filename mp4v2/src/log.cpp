...
namespace mp4v2
{
    namespace impl
    {
         ...
         */
        void Log::setLogCallback(MP4LogCallback value)
        {
            Log::_cb_func = value;
        }
         ...
         */
        void Log::setVerbosity(MP4LogLevel value) { _verbosity = value; }
         ...
         * newline.
         */
        void Log::verbose1f(const char* format, ...)
        {
            va_list ap;
         ...
         * newline.
         */
        void Log::verbose2f(const char* format, ...)
        {
            va_list ap;
         ...
         * newline.
         */
        void Log::verbose3f(const char* format, ...)
        {
            va_list ap;
         ...
         * newline.
         */
        void Log::verbose4f(const char* format, ...)
        {
            va_list ap;
         ...
         * newline.
         */
        void Log::dump(uint8_t indent, MP4LogLevel verbosity_,
                       const char* format, ...)
        {
            va_list ap;
         ...
         */
        void Log::vdump(uint8_t indent, MP4LogLevel verbosity_,
                        const char* format, va_list ap)
        {
            ...
            if (Log::_cb_func)
            {
                ...
                if (indent > 0)
                {
                    ...
                    // new_format << setw(indent) << setfill(' ') << "" <<
                    // setw(0); new_format << format;
         ...
         * newline.
         */
        void Log::printf(MP4LogLevel verbosity, const char* format, ...)
        {
            va_list ap;
         ...
         * @param ap varargs to build the message
         */
        void Log::vprintf(MP4LogLevel verbosity_, const char* format,
                          va_list ap)
        {
            // Make sure nothing gets logged with MP4_LOG_NONE.
            // That way people who ask for nothing to get logged
         ...
         */
        void Log::hexDump(uint8_t indent, MP4LogLevel verbosity_,
                          ...
                          const char* format, ...)
        {
            ...
            for (uint32_t i = 0; (i < numBytes); i += 16)
            {
                ...
                // operations
                oneLine << ':' << hex << setw(8) << setfill('0') << std::right
                        << i << setw(0) << setfill(' ') << ": ";
                ...
                for (j = 0; (j < curlen); j++)
                {
                    oneLine << hex << setw(2) << setfill('0') << right
         ...
         */
        void Log::errorf(const Exception& x)
        {
...
extern "C" void MP4SetLogCallback(MP4LogCallback cb_func)
{
    Log::setLogCallback(cb_func);
}

extern "C" MP4LogLevel MP4LogGetLevel(void)
{
    return mp4v2::impl::log.verbosity;
}

extern "C" void MP4LogSetLevel(MP4LogLevel verbosity)
{
    try
    {
