...
namespace mp4v2
{
    namespace impl
    {
        ...
        static char* PrintAudioInfo(MP4FileHandle mp4File, MP4TrackId trackId)
        {
            ...
            double msDuration = double(MP4ConvertFromTrackDuration(
                mp4File, trackId, trackDuration, MP4_MSECS_TIME_SCALE));

            uint32_t avgBitRate = MP4GetTrackBitRate(mp4File, trackId);
            ...
            if (foundType)
                snprintf(
                    ...
                    typeName, msDuration / 1000.0, (avgBitRate + 500) / 1000,
                    timeScale);
            else
                snprintf(
                    sInfo, 256,
                    ...
                    MP4IsIsmaCrypMediaTrack(mp4File, trackId) ? "enca - " : "",
                    typeName, type, msDuration / 1000.0,
                    (avgBitRate + 500) / 1000, timeScale);
        ...
        {
            size_t size =
                sizeof(VisualProfileToName) / sizeof(*VisualProfileToName);
        ...
        static char* PrintVideoInfo(MP4FileHandle mp4File, MP4TrackId trackId)
        {
            ...
            static const uint8_t mpegVideoTypes[] = {
                ...
                MP4_YUV12_VIDEO_TYPE,         MP4_H263_VIDEO_TYPE,
                MP4_H261_VIDEO_TYPE,
            };
            static const char* mpegVideoNames[] = {
                "MPEG-2 Simple", "MPEG-2 Main",  "MPEG-2 SNR", "MPEG-2 Spatial",
                "MPEG-2 High",   "MPEG-2 4:2:2", "MPEG-1",     "JPEG",
                "YUV12",         "H.263",        "H.261",
            };
            ...
            *oformatbuffer = 0;
            ...
            {
                if (MP4GetTrackMediaDataOriginalFormat(mp4File, trackId,
                                                       originalFormat,
                                                       sizeof(originalFormat))
                    == false)
                     ...
                     || (strcasecmp(originalFormat, "264b") == 0))
            {
                ...
                if (MP4GetTrackH264ProfileLevel(mp4File, trackId, &profile,
                                                &level))
                {
                    if (profile == 66)
                    {
            ...
            if (foundTypeName)
            {
                sprintf(sInfo,
                        ...
                        MP4IsIsmaCrypMediaTrack(mp4File, trackId) ? "encv - "
                                                                  : "",
                        typeName, msDuration / 1000.0,
                        (avgBitRate + 500) / 1000, width, height, fps);
            }
            else
            {
                sprintf(
                    ...
                    trackId, typeName, type, msDuration / 1000.0,
                    (avgBitRate + 500) / 1000, width, height, fps);
            }
        ...
        static char* PrintCntlInfo(MP4FileHandle mp4File, MP4TrackId trackId)
        {
            const char* media_data_name =
            ...
            snprintf(sInfo, 256, "%u\tcontrol\t%s, %.3f secs\n", trackId,
                     typeName, msDuration / 1000.0);
            return sInfo;
        }

        static char* PrintHintInfo(MP4FileHandle mp4File, MP4TrackId trackId)
        {
            ...
            snprintf(sInfo, 256, "%u\thint\tPayload %s for track %u\n", trackId,
                     payloadName, referenceTrackId);
        ...
        static char* PrintTrackInfo(MP4FileHandle mp4File, MP4TrackId trackId)
        {
            char* trackInfo = NULL;
            ...
            else
            {
                ...
                {
                    snprintf(trackInfo, 256, "%u\tod\tObject Descriptors\n",
                             trackId);
                }
                else if (!strcmp(trackType, MP4_SCENE_TRACK_TYPE))
                {
                    snprintf(trackInfo, 256, "%u\tscene\tBIFS\n", trackId);
                }
                else
                {
                    snprintf(trackInfo, 256, "%u\t%s\n", trackId, trackType);
                }
            }
...
extern "C" char* MP4FileInfo(const char* fileName, MP4TrackId trackId)
{
