...
namespace mp4v2
{
    namespace impl
    {
        ...
        {
        public:
            static void CopySample(MP4File* srcFile, MP4TrackId srcTrackId,
                                   MP4SampleId srcSampleId, MP4File* dstFile,
                                   MP4TrackId dstTrackId,
                                   MP4Duration dstSampleDuration);

            static void
            EncAndCopySample(MP4File* srcFile, MP4TrackId srcTrackId,
        ...
        public:
            ...
            void Create(const char* fileName, uint32_t flags, int add_ftyp = 1,
                        int add_iods = 1, char* majorBrand = NULL,
            ...
            void Optimize(const char* srcFileName,
            ...
            uint32_t GetNumberOfTracks(const char* type = NULL,
                                       uint8_t subType = 0);

            MP4TrackId AllocTrackId();
            MP4TrackId FindTrackId(uint16_t trackIndex, const char* type = NULL,
                                   uint8_t subType = 0);
            ...
            uint64_t GetTrackIntegerProperty(MP4TrackId trackId,
                                             const char* name);
            float GetTrackFloatProperty(MP4TrackId trackId, const char* name);
            const char* GetTrackStringProperty(MP4TrackId trackId,
                                               const char* name);
            void GetTrackBytesProperty(MP4TrackId trackId, const char* name,
                                       uint8_t** ppValue, uint32_t* pValueSize);

            void SetTrackIntegerProperty(MP4TrackId trackId, const char* name,
                                         int64_t value);
            void SetTrackFloatProperty(MP4TrackId trackId, const char* name,
                                       float value);
            void SetTrackStringProperty(MP4TrackId trackId, const char* name,
                                        const char* value);
            void SetTrackBytesProperty(MP4TrackId trackId, const char* name,
            ...
            MP4Timestamp GetSampleTime(MP4TrackId trackId,
                                       MP4SampleId sampleId);

            MP4Duration GetSampleDuration(MP4TrackId trackId,
                                          MP4SampleId sampleId);

            MP4Duration GetSampleRenderingOffset(MP4TrackId trackId,
                                                 MP4SampleId sampleId);
            ...
            void ReadSample(
                ...
                MP4Timestamp* pStartTime = NULL, MP4Duration* pDuration = NULL,
                MP4Duration* pRenderingOffset = NULL,
                bool* pIsSyncSample = NULL, bool* hasDependencyFlags = NULL,
                uint32_t* dependencyFlags = NULL);

            void WriteSample(MP4TrackId trackId, const uint8_t* pBytes,
                             uint32_t numBytes, MP4Duration duration = 0,
                             MP4Duration renderingOffset = 0,
                             bool isSyncSample = true);

            void WriteSampleDependency(MP4TrackId trackId,
            ...
            void SetSampleRenderingOffset(MP4TrackId trackId,
                                          MP4SampleId sampleId,
                                          MP4Duration renderingOffset);
            ...
            MP4TrackId AddAudioTrack(uint32_t timeScale,
                                     MP4Duration sampleDuration,
                                     uint8_t audioType);

            MP4TrackId AddULawAudioTrack(uint32_t timeScale);

            MP4TrackId AddALawAudioTrack(uint32_t timeScale);

            MP4TrackId AddAC3AudioTrack(uint32_t samplingRate, uint8_t fscod,
                                        uint8_t bsid, uint8_t bsmod,
                                        uint8_t acmod, uint8_t lfeon,
                                        uint8_t bit_rate_code);

            MP4TrackId AddEncAudioTrack( // ismacryp
                uint32_t timeScale, MP4Duration sampleDuration,
                uint8_t audioType, uint32_t scheme_type,
                uint16_t scheme_version, uint8_t key_ind_len, uint8_t iv_len,
                bool selective_enc, const char* kms_uri, bool use_ismacryp);

            void SetAmrVendor(MP4TrackId trackId, uint32_t vendor);

            void SetAmrDecoderVersion(MP4TrackId trackId,
                                      uint8_t decoderVersion);

            void SetAmrModeSet(MP4TrackId trackId, uint16_t modeSet);
            uint16_t GetAmrModeSet(MP4TrackId trackId);

            MP4TrackId AddAmrAudioTrack(uint32_t timeScale, uint16_t modeSet,
            ...
            MP4TrackId AddMP4VideoTrack(uint32_t timeScale,
                                        MP4Duration sampleDuration,
            ...
            MP4TrackId AddEncVideoTrack( // ismacryp
                ...
                uint16_t height, uint8_t videoType, mp4v2_ismacrypParams* icPp,
                const char* oFormat);

            void SetH263Vendor(MP4TrackId trackId, uint32_t vendor);

            void SetH263DecoderVersion(MP4TrackId trackId,
                                       uint8_t decoderVersion);

            void SetH263Bitrates(MP4TrackId, uint32_t avgBitrate,
                                 uint32_t maxBitrate);

            MP4TrackId AddH263VideoTrack(uint32_t timeScale,
                                         MP4Duration sampleDuration,
                                         uint16_t width, uint16_t height,
            ...
            MP4TrackId AddH264VideoTrack(uint32_t timeScale,
                                         MP4Duration sampleDuration,
                                         uint16_t width, uint16_t height,
            ...
            MP4TrackId AddEncH264VideoTrack(uint32_t timeScale,
                                            MP4Duration sampleDuration,
                                            uint16_t width, uint16_t height,
                                            MP4Atom* srcAtom,
             ...
             *  The optional parameter <b>timescale</b> may be supplied to give
             * the new chapter a specific timescale. Otherwise the chapter track
             * will have the same timescale as the reference track defined in
             * parameter refTrackId.
             *
             *  @param refTrackId ID of the track that will reference the
             * chapter track.
             *  @param timescale the timescale of the chapter track or 0 to use
             * the timescale of track specified by <b>refTrackId</b>.
             *
            ...
            MP4TrackId AddSubtitleTrack(uint32_t timescale, uint16_t width,
                                        uint16_t height);

            MP4TrackId AddSubpicTrack(uint32_t timescale, uint16_t width,
                                      uint16_t height);

            MP4TrackId AddPixelAspectRatio(MP4TrackId trackId,
                                           uint32_t hSpacing,
                                           uint32_t vSpacing);
            MP4TrackId AddColr(MP4TrackId trackId, uint16_t pri, uint16_t tran,
                               uint16_t mat);

            /** Add a QuickTime chapter.
             *
             *  @param chapterTrackId ID of chapter track or
             * #MP4_INVALID_TRACK_ID if unknown.
             *  @param chapterDuration duration (in the timescale of the chapter
             * track).
             *  @param chapterTitle title text for the chapter or NULL to use
             * default title format ("Chapter %03d", n) where n is the chapter
             * number.
             */
            ...
            /** Add a Nero chapter.
             *
             *  @param chapterStart the start time of the chapter in 100
             * nanosecond units
             *  @param chapterTitle title text for the chapter or NULL to use
             * default title format ("Chapter %03d", n) where n is the chapter
             * number.
             */
            void AddNeroChapter(MP4Timestamp chapterStart,
                                const char* chapterTitle = 0);

            /*! Returns the ID of the track referencing the chapter track
             * chapterTrackId. This function searches for a track of type
             * MP4_AUDIO_TRACK_TYPE that references the track chapterTrackId
             * through the atom "tref.chap".
             *
             * @param chapterTrackId the ID of the chapter track
             * @param trackName receives the name of the referencing track if
             * not null
             * @param trackNameSize the size of the memory pointed to by
             * trackName
             * @return the ID if the track referencing the chapter track or
             ...
             *
             * @param trackName receives the name of the chapter track if not
             * null
             * @param trackNameSize the size of the memory pointed to by
             * trackName
             * @return the ID of the chapter track or MP4_INVALID_TRACK_ID
             */
            MP4TrackId FindChapterTrack(char* trackName = 0,
                                        int trackNameSize = 0);
             ...
             */
            MP4ChapterType
            DeleteChapters(MP4ChapterType chapterType = MP4ChapterTypeQt,
                           MP4TrackId chapterTrackId = 0);

            /** Get list of chapters.
             *
             *  @param chapterList address receiving array of chapter items.
             *      If a non-NULL is received the caller is responsible for
             * freeing the memory with MP4Free().
             *  @param chapterCount address receiving count of items in array.
             *  @param chapterType the type of chapters to read:
             *      @li #MP4ChapterTypeAny (any chapters, searched in order of
             * Qt, Nero)
             *      @li #MP4ChapterTypeQt
             *      @li #MP4ChapterTypeNero
             *
            ...
            MP4ChapterType
            GetChapters(MP4Chapter_t** chapterList, uint32_t* chapterCount,
             ...
             */
            MP4ChapterType
            SetChapters(MP4Chapter_t* chapterList, uint32_t chapterCount,
             ...
             *  Conversion from Nero to QuickTime or QuickTime to Nero is
             * supported.
             *
             ...
             *
             *  @return the chapter type before conversion or
             * #MP4ChapterTypeNone if the source chapters do not exist or
             * invalid <b>toChapterType</b> was specified.
             */
            MP4ChapterType
            ConvertChapters(MP4ChapterType toChapterType = MP4ChapterTypeQt);

            /** Change the general timescale.
             *
             *  This function changes the general timescale to the new timescale
             *  <b>value</b> by recalculating all values that depend on the
             * timescale in "moov.mvhd".
             *
            ...
            void GetTrackESConfiguration(MP4TrackId trackId, uint8_t** ppConfig,
                                         uint32_t* pConfigSize);
            void SetTrackESConfiguration(MP4TrackId trackId,
                                         const uint8_t* pConfig,
                                         uint32_t configSize);

            void GetTrackVideoMetadata(MP4TrackId trackId, uint8_t** ppConfig,
                                       uint32_t* pConfigSize);
            ...
            void AppendHintTrackSdp(MP4TrackId hintTrackId,
                                    const char* sdpString);

            void MakeFtypAtom(char* majorBrand, uint32_t minorVersion,
                              char** compatibleBrands,
                              uint32_t compatibleBrandsCount);

            // 3GPP specific functions
            void Make3GPCompliant(const char* fileName, char* majorBrand,
            ...
            void CreateIsmaIodFromParams(
                uint8_t videoProfile, uint32_t videoBitrate,
            ...
            uint64_t ConvertFromMovieDuration(MP4Duration duration,
                                              uint32_t timeScale);

            uint64_t ConvertFromTrackTimestamp(MP4TrackId trackId,
                                               MP4Timestamp timeStamp,
                                               uint32_t timeScale);

            MP4Timestamp ConvertToTrackTimestamp(MP4TrackId trackId,
                                                 uint64_t timeStamp,
                                                 uint32_t timeScale);

            uint64_t ConvertFromTrackDuration(MP4TrackId trackId,
                                              MP4Duration duration,
                                              uint32_t timeScale);

            MP4Duration ConvertToTrackDuration(MP4TrackId trackId,
                                               uint64_t duration,
                                               uint32_t timeScale);

            // specialized operations

            void GetHintTrackRtpPayload(MP4TrackId hintTrackId,
                                        char** ppPayloadName = NULL,
                                        uint8_t* pPayloadNumber = NULL,
                                        uint16_t* pMaxPayloadSize = NULL,
                                        char** ppEncodingParams = NULL);

            void SetHintTrackRtpPayload(MP4TrackId hintTrackId,
            ...
            void ReadRtpHint(MP4TrackId hintTrackId, MP4SampleId hintSampleId,
                             uint16_t* pNumPackets = NULL);

            uint16_t GetRtpHintNumberOfPackets(MP4TrackId hintTrackId);

            int8_t GetRtpPacketBFrame(MP4TrackId hintTrackId,
                                      uint16_t packetIndex);

            int32_t GetRtpPacketTransmitOffset(MP4TrackId hintTrackId,
                                               uint16_t packetIndex);

            void ReadRtpPacket(MP4TrackId hintTrackId, uint16_t packetIndex,
                               uint8_t** ppBytes, uint32_t* pNumBytes,
                               uint32_t ssrc = 0, bool includeHeader = true,
                               bool includePayload = true);

            MP4Timestamp GetRtpTimestampStart(MP4TrackId hintTrackId);

            void SetRtpTimestampStart(MP4TrackId hintTrackId,
                                      MP4Timestamp rtpStart);

            void AddRtpHint(MP4TrackId hintTrackId, bool isBframe,
                            uint32_t timestampOffset);

            void AddRtpPacket(MP4TrackId hintTrackId, bool setMbit,
                              int32_t transmitOffset);

            void AddRtpImmediateData(MP4TrackId hintTrackId,
                                     const uint8_t* pBytes, uint32_t numBytes);

            void AddRtpSampleData(MP4TrackId hintTrackId, MP4SampleId sampleId,
                                  uint32_t dataOffset, uint32_t dataLength);

            void AddRtpESConfigurationPacket(MP4TrackId hintTrackId);

            void WriteRtpHint(MP4TrackId hintTrackId, MP4Duration duration,
                              bool isSyncSample);
            ...
            char* MakeTrackEditName(MP4TrackId trackId, MP4EditId editId,
                                    const char* name);

            MP4EditId AddTrackEdit(MP4TrackId trackId,
                                   MP4EditId editId = MP4_INVALID_EDIT_ID);

            void DeleteTrackEdit(MP4TrackId trackId, MP4EditId editId);

            uint32_t GetTrackNumberOfEdits(MP4TrackId trackId);

            MP4Timestamp GetTrackEditStart(MP4TrackId trackId,
                                           MP4EditId editId);

            MP4Duration GetTrackEditTotalDuration(MP4TrackId trackId,
                                                  MP4EditId editId);

            MP4Timestamp GetTrackEditMediaStart(MP4TrackId trackId,
                                                MP4EditId editId);

            void SetTrackEditMediaStart(MP4TrackId trackId, MP4EditId editId,
                                        MP4Timestamp startTime);

            MP4Duration GetTrackEditDuration(MP4TrackId trackId,
                                             MP4EditId editId);

            void SetTrackEditDuration(MP4TrackId trackId, MP4EditId editId,
                                      MP4Duration duration);

            bool GetTrackEditDwell(MP4TrackId trackId, MP4EditId editId);

            void SetTrackEditDwell(MP4TrackId trackId, MP4EditId editId,
                                   bool dwell);

            MP4SampleId GetSampleIdFromEditTime(MP4TrackId trackId,
                                                MP4Timestamp when,
            ...
            void WriteCountedString(char* string, uint8_t charSize = 1,
                                    bool allowExpandedCount = false,
            ...
            void EnableMemoryBuffer(uint8_t* pBytes = NULL,
                                    uint64_t numBytes = 0);
            void DisableMemoryBuffer(uint8_t** ppBytes = NULL,
                                     uint64_t* pNumBytes = NULL);
            ...
            MP4Atom* AddChildAtom(const char* parentName,
                                  const char* childName);

            MP4Atom* AddChildAtom(MP4Atom* pParentAtom, const char* childName);

            MP4Atom* InsertChildAtom(const char* parentName,
                                     const char* childName, uint32_t index);

            MP4Atom* InsertChildAtom(MP4Atom* pParentAtom,
                                     const char* childName, uint32_t index);

            MP4Atom* AddDescendantAtoms(const char* ancestorName,
                                        const char* childName);

            MP4Atom* AddDescendantAtoms(MP4Atom* pAncestorAtom,
                                        const char* childName);

        protected:
            void Init();
            void Open(const char* name, File::Mode mode,
            ...
            void ProtectWriteOperation(const char* file, int line,
                                       const char* func);
            ...
            MP4TrackId AddVideoTrackDefault(uint32_t timeScale,
                                            MP4Duration sampleDuration,
                                            uint16_t width, uint16_t height,
                                            const char* videoType);
            MP4TrackId AddCntlTrackDefault(uint32_t timeScale,
                                           MP4Duration sampleDuration,
                                           const char* videoType);
            void AddTrackToIod(MP4TrackId trackId);

            void RemoveTrackFromIod(MP4TrackId trackId,
                                    bool shallHaveIods = true);
            ...
            uint32_t FindTrackReference(const char* trefName,
                                        MP4TrackId refTrackId);

            void RemoveTrackReference(const char* trefName,
                                      MP4TrackId refTrackId);
            ...
            void CreateESD(MP4DescriptorProperty* pEsProperty, uint32_t esid,
                           uint8_t objectType, uint8_t streamType,
                           uint32_t bufferSize, uint32_t bitrate,
                           const uint8_t* pConfig, uint32_t configLength,
                           char* url);

            void CreateIsmaODUpdateCommandFromFileForFile(
                MP4TrackId odTrackId, MP4TrackId audioTrackId,
                MP4TrackId videoTrackId, uint8_t** ppBytes,
                uint64_t* pNumBytes);
            ...
            void CreateIsmaODUpdateCommandForStream(
                ...
                MP4DescriptorProperty* pVideoEsdProperty, uint8_t** ppBytes,
                uint64_t* pNumBytes);

            void CreateIsmaSceneCommand(bool hasAudio, bool hasVideo,
                                        uint8_t** ppBytes, uint64_t* pNumBytes);
