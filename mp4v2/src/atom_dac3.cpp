...
namespace mp4v2
{
    namespace impl
    {
            ...
            : MP4Atom(file, "dac3")
        {
            ...
            AddProperty(
                new MP4BitfieldProperty(*this, "bit_rate_code", 5));    /* 5 */
            AddProperty(new MP4BitfieldProperty(*this, "reserved", 5)); /* 6 */
            m_pProperties[6]->SetReadOnly(true);
        }

        void MP4DAc3Atom::Generate() { MP4Atom::Generate(); }

        /*
         * Normalling the Dump of the atom looks like the following, we want to
         * spruce this up a bit to give more information.
         *
         *      type = dac3
         *       fscod = 0 (0x0) <2 bits>
         *       bsid = 8 (0x08) <5 bits>
         ...
         *       bsid = 8 (0x08) <5 bits>
         *       bsmod = 0 (0x0) <3 bits> [Main audio service: complete main
         * (CM)] acmod = 7 (0x7) <3 bits> [3/2 (L, C, R, SL, SR)] lfeon = 1
         * (0x1) <1 bits> [ENABLED] bit_rate_code = 15 (0x0f) <5 bits> [448
         * kbit/s] reserved = 0 (0x00) <5 bits>
         *
         */
        void MP4DAc3Atom::Dump(uint8_t indent, bool dumpImplicits)
        {

            MP4BitfieldProperty* fscodProp =
                ((MP4BitfieldProperty*)m_pProperties[0]);
            MP4BitfieldProperty* bsidProp =
                ((MP4BitfieldProperty*)m_pProperties[1]);
            MP4BitfieldProperty* bsmodProp =
                ((MP4BitfieldProperty*)m_pProperties[2]);
            MP4BitfieldProperty* acmodProp =
                ((MP4BitfieldProperty*)m_pProperties[3]);
            MP4BitfieldProperty* lfeonProp =
                ((MP4BitfieldProperty*)m_pProperties[4]);
            MP4BitfieldProperty* brcProp =
                ((MP4BitfieldProperty*)m_pProperties[5]);
            MP4BitfieldProperty* resProp =
                ((MP4BitfieldProperty*)m_pProperties[6]);
            ...
            if (fscodProp)
            {
                ...
                log.dump(indent, MP4_LOG_VERBOSE2,
                         "\"%s\": fscod = %" PRIu64 " (0x%0*" PRIx64
                         ") <%u bits> [%s kHz]",
                         GetFile().GetFilename().c_str(), fscod, (int)hexWidth,
                         fscod, fscodProp->GetNumBits(), fscodString);
            }
            if (bsidProp)
                bsidProp->Dump(indent, dumpImplicits);
            ...
            if (bsmodProp)
            {
                ...
                const char* bsmods[] = {
                    ...
                    "Associated service: voice over (VO) or Main audio "
                    "service: karaoke",
                };
                ...
                log.dump(indent, MP4_LOG_VERBOSE2,
                         "\"%s\": bsmod = %" PRIu64 " (0x%0*" PRIx64
                         ") <%u bits> [%s]",
                         GetFile().GetFilename().c_str(), bsmod, (int)hexWidth,
                         bsmod, bsmodProp->GetNumBits(), bsmodString);
            }
            ...
            if (acmodProp)
            {
                ...
                const char* acmods[] = {
                    "1 + 1 (Ch1, Ch2)",   "1/0 (C)",
                    "2/0 (L, R)",         "3/0 (L, C, R)",
                    "2/1 (L, R, S)",      "3/1 (L, C, R, S)",
                    "2/2 (L, R, SL, SR)", "3/2 (L, C, R, SL, SR)",
                };
                ...
                log.dump(indent, MP4_LOG_VERBOSE2,
                         "\"%s\": acmod = %" PRIu64 " (0x%0*" PRIx64
                         ") <%u bits> [%s]",
                         GetFile().GetFilename().c_str(), acmod, (int)hexWidth,
                         acmod, acmodProp->GetNumBits(), acmodString);
            }
            ...
            if (lfeonProp)
            {
                ...
                log.dump(indent, MP4_LOG_VERBOSE2,
                         "\"%s\": lfeon = %" PRIu64 " (0x%0*" PRIx64
                         ") <%u bits> [%s]",
                         GetFile().GetFilename().c_str(), lfeon, (int)hexWidth,
                         lfeon, lfeonProp->GetNumBits(),
                         lfeon ? "ENABLED" : "DISABLED");
            }
            ...
            if (brcProp)
            {
                ...
                if (bit_rate_code
                    < (sizeof(bit_rate_codes) / sizeof(bit_rate_codes[0])))
                {
                ...
                log.dump(indent, MP4_LOG_VERBOSE2,
                         "\"%s\": bit_rate_code = %" PRIu64 " (0x%0*" PRIx64
                         ") <%u bits> [%" PRIu32 " kbit/s]",
                         GetFile().GetFilename().c_str(), bit_rate_code,
                         (int)hexWidth, bit_rate_code, brcProp->GetNumBits(),
                         bit_rate);
            }
            if (resProp)
                resProp->Dump(indent, dumpImplicits);
        }
