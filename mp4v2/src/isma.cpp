    ...
    namespace impl
    {
        ...
        void MP4File::MakeIsmaCompliant(bool addIsmaComplianceSdp)
        {
            ...
            if (audioTrackId != MP4_INVALID_TRACK_ID)
            {
                ...
                {
                    log.errorf("%s: \"%s\": can't make ISMA compliant when "
                               "file contains an %s track",
            ...
            if (videoTrackId != MP4_INVALID_TRACK_ID)
            {
                video_media_data_name =
                    MP4GetTrackMediaDataName(this, videoTrackId);
            ...
            SetTrackESConfiguration(sceneTrackId, BifsV2Config,
                                    sizeof(BifsV2Config));
            ...
            CreateIsmaODUpdateCommandFromFileForFile(
                m_odTrackId, audioTrackId, videoTrackId, &pBytes, &numBytes);
            ...
            CreateIsmaSceneCommand(MP4_IS_VALID_TRACK_ID(audioTrackId),
                                   MP4_IS_VALID_TRACK_ID(videoTrackId), &pBytes,
                                   &numBytes);
            ...
            CreateIsmaIodFromFile(m_odTrackId, sceneTrackId, audioTrackId,
                                  videoTrackId, &pBytes, &numBytes);
            ...
            snprintf(&sdpBuf[used], sdpBufLen,
                     "a=mpeg4-iod: "
                     "\042data:application/mpeg4-iod;base64,%s\042\015\012",
                     iodBase64);

            SetSessionSdp(sdpBuf);

            log.verbose1f("\"%s\": IOD SDP = %s", GetFilename().c_str(),
                          sdpBuf);
        ...
        static void CloneIntegerProperty(MP4Descriptor* pDest,
        ...
        void MP4File::CreateIsmaIodFromFile(MP4TrackId odTrackId,
                                            ...
                                            uint64_t* pNumBytes)
        {
            ...
            // OD
            MP4Descriptor* pOdEsd = pEsProperty->AddDescriptor(MP4ESDescrTag);
            pOdEsd->Generate();
            ...
            if (pOdEsd->FindProperty("URLFlag", (MP4Property**)&pSetProperty))
            ...
            CreateIsmaODUpdateCommandFromFileForStream(
                audioTrackId, videoTrackId, &pBytes, &numBytes);

            log.hexDump(0, MP4_LOG_VERBOSE1, pBytes, numBytes,
                        "\"%s\": OD data", GetFilename().c_str());
            ...
            snprintf(urlBuf, urlBufLen,
                     "data:application/mpeg4-od-au;base64,%s", odCmdBase64);

            MP4StringProperty* pUrlProperty;
            if (pOdEsd->FindProperty("URL", (MP4Property**)&pUrlProperty))
                pUrlProperty->SetValue(urlBuf);

            log.verbose1f("\"%s\": OD data URL = \042%s\042",
            ...
            (void)FindProperty(
                MakeTrackName(odTrackId,
                              "mdia.minf.stbl.stsd.mp4s.esds.decConfigDescr"),
                (MP4Property**)&pSrcDcd);
            ASSERT(pSrcDcd);
            MP4Property* pOrgOdEsdProperty = pOdEsd->GetProperty(8);
            ...
            if (pSceneEsd->FindProperty("ESID", (MP4Property**)&pSceneESID))
            {
            ...
            log.hexDump(0, MP4_LOG_VERBOSE1, pBytes, numBytes,
                        "\"%s\": Scene data", GetFilename().c_str());
            ...
            if (pSceneEsd->FindProperty("URL", (MP4Property**)&pUrlProperty))
                pUrlProperty->SetValue(urlBuf);
            ...
            // HACK temporarily point to scene decoder config
            ASSERT(FindProperty(
                MakeTrackName(sceneTrackId,
                              "mdia.minf.stbl.stsd.mp4s.esds.decConfigDescr"),
                (MP4Property**)&pSrcDcd));
            ASSERT(pSrcDcd);
            MP4Property* pOrgSceneEsdProperty = pSceneEsd->GetProperty(8);
            ...
            log.hexDump(0, MP4_LOG_VERBOSE1, *ppBytes, *pNumBytes,
                        "\"%s\": IOD data", GetFilename().c_str());
        }

        void MP4File::CreateIsmaIodFromParams(
            uint8_t videoProfile, uint32_t videoBitrate, uint8_t* videoConfig,
            uint32_t videoConfigLength, uint8_t audioProfile,
            uint32_t audioBitrate, uint8_t* audioConfig,
            uint32_t audioConfigLength, uint8_t** ppIodBytes,
            uint64_t* pIodNumBytes)
        {
            ...
            if (pIod->FindProperty("audioProfileLevelId", (MP4Property**)&pInt))
            ...
            // Scene
            CreateIsmaSceneCommand((audioProfile != 0xFF),
                                   (videoProfile != 0xFF), &pBytes, &numBytes);
            ...
            char* urlBuf = (char*)MP4Malloc(strlen(sceneCmdBase64) + 64);
            snprintf(urlBuf, strlen(sceneCmdBase64) + 64,
                     "data:application/mpeg4-bifs-au;base64,%s",
                     sceneCmdBase64);
            ...
            CreateESD(pEsProperty,
                      201, // esid
                      MP4SystemsV2ObjectType, MP4SceneDescriptionStreamType,
                      numBytes,     // bufferSize
                      numBytes * 8, // bitrate
                      BifsV2Config, sizeof(BifsV2Config), urlBuf);
            MP4Free(urlBuf);
            urlBuf = NULL;
            ...
            CreateESD(pVideoEsdProperty,
                      20, // esid
                      MP4_MPEG4_VIDEO_TYPE, MP4VisualStreamType,
                      videoBitrate / 8, // bufferSize
                      videoBitrate, videoConfig, videoConfigLength, NULL);
            ...
            CreateESD(pAudioEsdProperty,
                      10, // esid
                      MP4_MPEG4_AUDIO_TYPE, MP4AudioStreamType,
                      audioBitrate / 8, // bufferSize
                      audioBitrate, audioConfig, audioConfigLength, NULL);

            CreateIsmaODUpdateCommandForStream(
                pAudioEsdProperty, pVideoEsdProperty, &pBytes, &numBytes);
            ...
            if (urlBuf != NULL)
            {
                ...
                CreateESD(pEsProperty, 101, MP4SystemsV1ObjectType,
            ...
            log.hexDump(0, MP4_LOG_VERBOSE1, *ppIodBytes, *pIodNumBytes,
                        "\"%s\": IOD data", GetFilename().c_str());
        }

        void MP4File::CreateESD(MP4DescriptorProperty* pEsProperty,
                                uint32_t esid, uint8_t objectType,
                                uint8_t streamType, uint32_t bufferSize,
                                uint32_t bitrate, const uint8_t* pConfig,
                                uint32_t configLength, char* url)
        {
            MP4IntegerProperty* pInt;
            ...
            MP4Descriptor* pEsd = pEsProperty->AddDescriptor(MP4ESDescrTag);
            pEsd->Generate();

            if (pEsd->FindProperty("ESID", (MP4Property**)&pInt))
                pInt->SetValue(esid);
                                   ...
                                   (MP4Property**)&pConfigDescrProperty))
            {
                ...
                if (pConfigDescrProperty->FindProperty(
                        "decSpecificInfo[0].info", (MP4Property**)&pBytes))
            ...
            if (url)
            {
                if (pEsd->FindProperty("URLFlag", (MP4Property**)&pInt))
                    pInt->SetValue(1);

                if (pEsd->FindProperty("URL", (MP4Property**)&pString))
        ...
        void MP4File::CreateIsmaODUpdateCommandFromFileForFile(
            ...
            MP4TrackId videoTrackId, uint8_t** ppBytes, uint64_t* pNumBytes)
        {
            // Descriptor constructors need a parent atom.  In this
            // case we don't have one so create a dummy one.  This
            // is OK as we only build the descriptor and then dump
            ...
            for (uint8_t i = 0; i < 2; i++)
            {
                ...
                ASSERT(pOd->FindProperty(
                    "esIds", (MP4Property**)&pEsIdsDescriptorProperty));
                ...
                ASSERT(pRefDescriptor->FindProperty(
                    "refIndex", (MP4Property**)&pRefIndexProperty));
        ...
        void MP4File::CreateIsmaODUpdateCommandFromFileForStream(
            MP4TrackId audioTrackId, MP4TrackId videoTrackId, uint8_t** ppBytes,
            uint64_t* pNumBytes)
        {
            ...
            if (audioTrackId != MP4_INVALID_TRACK_ID)
            {
                ...
                MP4Atom* pEsdsAtom = FindAtom(
                    MakeTrackName(audioTrackId, "mdia.minf.stbl.stsd.*.esds"));
                ASSERT(pEsdsAtom);
            ...
            if (videoTrackId != MP4_INVALID_TRACK_ID)
            {
                ...
                MP4Atom* pEsdsAtom = FindAtom(
                    MakeTrackName(videoTrackId, "mdia.minf.stbl.stsd.*.esds"));
                ASSERT(pEsdsAtom);
            ...
            CreateIsmaODUpdateCommandForStream(pAudioEsd, pVideoEsd, ppBytes,
                                               pNumBytes);
            log.hexDump(
                0, MP4_LOG_VERBOSE1, *ppBytes, *pNumBytes,
        ...
        void MP4File::CreateIsmaSceneCommand(bool hasAudio, bool hasVideo,
                                             uint8_t** ppBytes,
                                             uint64_t* pNumBytes)
        {
            // from ISMA 1.0 Tech Spec Appendix E
            static const uint8_t bifsAudioOnly[] = {
                0xC0, 0x10, 0x12, 0x81, 0x30, 0x2A, 0x05, 0x6D, 0xC0};
            static const uint8_t bifsVideoOnly[] = {
                0xC0, 0x10, 0x12, 0x61, 0x04, 0x1F, 0xC0, 0x00, 0x00, 0x1F,
                0xC0, 0x00, 0x00, 0x44, 0x28, 0x22, 0x82, 0x9F, 0x80};
            static const uint8_t bifsAudioVideo[] = {
                0xC0, 0x10, 0x12, 0x81, 0x30, 0x2A, 0x05, 0x6D,
                0x26, 0x10, 0x41, 0xFC, 0x00, 0x00, 0x01, 0xFC,
                0x00, 0x00, 0x04, 0x42, 0x82, 0x28, 0x29, 0xF8};
