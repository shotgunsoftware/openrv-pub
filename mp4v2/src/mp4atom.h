...
#define Counted true
        ...
        public:
            ...
            static MP4Atom* CreateAtom(MP4File& file, MP4Atom* parent,
                                       const char* type);
            static bool IsReasonableType(const char* type);

        private:
            static MP4Atom* factory(MP4File& file, MP4Atom* parent,
                                    const char* type);
        ...
        public:
            ...
            void SetStart(uint64_t pos) { m_start = pos; };

            uint64_t GetEnd() { return m_end; };

            void SetEnd(uint64_t pos) { m_end = pos; };
            ...
            bool IsUnknownType() { return m_unknownType; }
            ...
            bool IsRootAtom() { return m_type[0] == '\0'; }

            MP4Atom* GetParentAtom() { return m_pParentAtom; }
            ...
            uint32_t GetNumberOfChildAtoms() { return m_pChildAtoms.Size(); }
            ...
            uint32_t GetCount() { return m_pProperties.Size(); }
        ...
        protected:
            ...
            void AddReserved(MP4Atom& parentAtom, const char* name,
                             uint32_t size);

            void ExpectChildAtom(const char* name, bool mandatory,
                                 bool onlyOne = true);
            ...
            void ReadProperties(uint32_t startIndex = 0,
                                uint32_t count = 0xFFFFFFFF);
            void ReadChildAtoms();

            void WriteProperties(uint32_t startIndex = 0,
                                 uint32_t count = 0xFFFFFFFF);
        ...
        inline uint32_t ATOMID(const char* type) { return STRTOINT32(type); }

        // inverse ATOMID - 32 bit id to string
        inline void IDATOM(uint32_t type, char* s) { INT32TOSTR(type, s); }
