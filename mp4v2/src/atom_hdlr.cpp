    ...
    namespace impl
    {
            ...
            : MP4Atom(file, "hdlr")
        {
            ...
            MP4StringProperty* pProp =
                new MP4StringProperty(*this, "handlerType");
        ...
        void MP4HdlrAtom::Read()
        {
            ...
            else
            {
                ...
                try
                {
                    // Unfortunately, there are some invalid mp4 writers that
                    // don't null the hdlr name string.  Generally this will be
                    // "automatically" terminated for them by the size field of
                    // the subsequent atom.  So if our size is off by one...let
                    // it slide.  otherwise, rethrow. The Skip() call will set
                    // our start to the correct location for the next Atom. See
                    // issue #52
                    ReadProperties(5);
                }
                catch (Exception* x)
