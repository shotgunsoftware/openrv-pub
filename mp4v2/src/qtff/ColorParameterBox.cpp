    ...
    namespace impl
    {
        namespace qtff
        {
            ...
            namespace
            {
                ...
                bool findColorParameterBox(MP4FileHandle file, MP4Atom& coding,
                                           MP4Atom*& colr);
            } // namespace

            ///////////////////////////////////////////////////////////////////////////////

            bool ColorParameterBox::add(MP4FileHandle file, uint16_t trackIndex,
                                        const Item& item)
            {
                MP4Atom* coding;

                if (!MP4_IS_VALID_FILE_HANDLE(file))
                    throw new Exception("invalid file handle", __FILE__,
                                        __LINE__, __FUNCTION__);

                if (findCoding(file, trackIndex, coding))
                    throw new Exception("supported coding not found", __FILE__,
                                        __LINE__, __FUNCTION__);

                MP4Atom* colr;
                if (!findColorParameterBox(file, *coding, colr))
                    throw new Exception("colr-box already exists", __FILE__,
                                        __LINE__, __FUNCTION__);

                colr = MP4Atom::CreateAtom(*((MP4File*)file), coding,
                                           BOX_CODE.c_str());
                ...
                if (colr->FindProperty("colr.colorParameterType",
                                       (MP4Property**)&type))
                    type->SetValue("nclc");

                if (colr->FindProperty("colr.primariesIndex",
                                       (MP4Property**)&primariesIndex))
                    primariesIndex->SetValue(item.primariesIndex);

                if (colr->FindProperty("colr.transferFunctionIndex",
                                       (MP4Property**)&transferFunctionIndex))
                    transferFunctionIndex->SetValue(item.transferFunctionIndex);

                if (colr->FindProperty("colr.matrixIndex",
                                       (MP4Property**)&matrixIndex))
            ...
            bool ColorParameterBox::add(MP4FileHandle file, MP4TrackId trackId,
                                        const Item& item)
            {
            ...
            bool ColorParameterBox::get(MP4FileHandle file, uint16_t trackIndex,
                                        Item& item)
            {
                item.reset();
                ...
                MP4Atom* colr;
                if (findColorParameterBox(file, *coding, colr))
                    throw new Exception("colr-box not found", __FILE__,
                                        __LINE__, __FUNCTION__);
                ...
                if (colr->FindProperty("colr.transferFunctionIndex",
                                       (MP4Property**)&transferFunctionIndex))
                    item.transferFunctionIndex =
                        transferFunctionIndex->GetValue();
            ...
            bool ColorParameterBox::get(MP4FileHandle file, MP4TrackId trackId,
                                        Item& item)
            {
                MP4File& mp4 = *((MP4File*)file);
            ...
            bool ColorParameterBox::list(MP4FileHandle file, ItemList& itemList)
            {
                itemList.clear();
                MP4File& mp4 = *((MP4File*)file);
            ...
            bool ColorParameterBox::remove(MP4FileHandle file,
                                           uint16_t trackIndex)
            {
                MP4Atom* coding;
                if (findCoding(file, trackIndex, coding))
            ...
            bool ColorParameterBox::set(MP4FileHandle file, uint16_t trackIndex,
                                        const Item& item)
            {
                MP4Atom* coding;
                if (findCoding(file, trackIndex, coding))
            ...
            bool ColorParameterBox::set(MP4FileHandle file, MP4TrackId trackId,
                                        const Item& item)
            {
                MP4File& mp4 = *((MP4File*)file);
            ...
            void ColorParameterBox::Item::convertFromCSV(const string& text)
            {
                ...
                if (iss.rdstate() != ios::eofbit)
                {
                    ...
                    xss << "invalid ColorParameterBox format"
                        ...
                        << " got: " << text;
                    throw new Exception(xss.str(), __FILE__, __LINE__,
                                        __FUNCTION__);
                }
            }
            ...
            string ColorParameterBox::Item::convertToCSV() const
            {
                string buffer;
            ...
            string& ColorParameterBox::Item::convertToCSV(string& buffer) const
            {
                ostringstream oss;
                oss << primariesIndex << ',' << transferFunctionIndex << ','
                    << matrixIndex;
                buffer = oss.str();
                return buffer;
            }
            ...
            void ColorParameterBox::Item::reset()
            {
                primariesIndex = 6;
                transferFunctionIndex = 1;
                matrixIndex = 6;
            ...
            namespace
            {
                                           ...
                                           MP4Atom*& colr)
                {
                    ...
                    MP4StringProperty* type;
                    if (!found->FindProperty("colr.colorParameterType",
                                             (MP4Property**)&type))
                        return true;
