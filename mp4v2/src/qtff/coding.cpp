...
namespace mp4v2
{
    namespace impl
    {
        namespace qtff
        {
            ...
            bool findCoding(MP4FileHandle file, uint16_t trackIndex,
                            MP4Atom*& coding)
            {
                coding = NULL;
                ...
                if (!hdlr)
                    throw new Exception("media handler not found", __FILE__,
                                        __LINE__, __FUNCTION__);

                MP4StringProperty* handlerType;
                if (!hdlr->FindProperty("hdlr.handlerType",
                                        (MP4Property**)&handlerType))
                    throw new Exception("media handler type-property not found",
                                        __FILE__, __LINE__, __FUNCTION__);

                const string video = "vide";
                if (video != handlerType->GetValue())
                    throw new Exception("video-track required", __FILE__,
                                        __LINE__, __FUNCTION__);
                ...
                for (uint32_t i = 0; i < atomc; i++)
                {
                    ...
                    if (STATIC_DATA.supportedCodings.find(atom->GetType())
                        == STATIC_DATA.supportedCodings.end())
                        continue;
                    coding = atom;
                }
