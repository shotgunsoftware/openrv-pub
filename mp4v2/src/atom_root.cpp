...
namespace mp4v2
{
    namespace impl
    {
        ...
        void MP4RootAtom::BeginWrite(bool use64)
        {
            ...
            {
                m_rewrite_free =
                    (MP4FreeAtom*)MP4Atom::CreateAtom(m_File, NULL, "free");
                m_rewrite_free->SetSize(32
                                        * 4); // room for 32 additional brands
        ...
        void MP4RootAtom::FinishWrite(bool use64)
        {
            if (m_rewrite_ftyp)
            {
                ...
                if (newpos > m_rewrite_freePosition)
                    m_rewrite_free->SetSize(
                        m_rewrite_free->GetSize()
                        - (newpos - m_rewrite_freePosition)); // shrink
                else if (newpos < m_rewrite_freePosition)
                    m_rewrite_free->SetSize(
                        m_rewrite_free->GetSize()
                        + (m_rewrite_freePosition - newpos)); // grow
