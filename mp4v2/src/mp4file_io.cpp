...
namespace mp4v2
{
    namespace impl
    {
        ...
        void MP4File::SetPosition(uint64_t pos, File* file)
        {
            if (m_memoryBuffer)
            {
                if (pos >= m_memoryBufferSize)
                    throw new Exception("position out of range", __FILE__,
            ...
            ASSERT(file);
            if (file->seek(pos))
                throw new PlatformException("seek failed", sys::getLastError(),
        ...
        void MP4File::ReadBytes(uint8_t* buf, uint32_t bufsiz, File* file)
        {
            ...
            if (m_memoryBuffer)
            {
                if (m_memoryBufferPosition + bufsiz > m_memoryBufferSize)
                    throw new Exception(
                        "not enough bytes, reached end-of-memory", __FILE__,
                        __LINE__, __FUNCTION__);
            ...
            if (file->read(buf, bufsiz, nin))
                throw new PlatformException("read failed", sys::getLastError(),
                                            __FILE__, __LINE__, __FUNCTION__);
            if (nin != bufsiz)
        ...
        void MP4File::DisableMemoryBuffer(uint8_t** ppBytes,
                                          uint64_t* pNumBytes)
        {
        ...
        void MP4File::WriteBytes(uint8_t* buf, uint32_t bufsiz, File* file)
        {
            ...
            if (m_memoryBuffer)
            {
                if (m_memoryBufferPosition + bufsiz > m_memoryBufferSize)
                {
                    ...
                    m_memoryBuffer = (uint8_t*)MP4Realloc(m_memoryBuffer,
                                                          m_memoryBufferSize);
                }
            ...
            if (file->write(buf, bufsiz, nout))
                throw new PlatformException("write failed", sys::getLastError(),
                                            __FILE__, __LINE__, __FUNCTION__);
            if (nout != bufsiz)
                throw new Exception("not all bytes written", __FILE__, __LINE__,
                                    __FUNCTION__);
        }
        ...
        void MP4File::WriteUInt8(uint8_t value) { WriteBytes(&value, 1); }
        ...
        void MP4File::WriteFixed16(float value)
        {
            if (value >= 0x100)
            {
                ...
                throw new PlatformException(msg.str().c_str(), ERANGE, __FILE__,
                                            __LINE__, __FUNCTION__);
            }
        ...
        char* MP4File::ReadCountedString(uint8_t charSize,
                                         bool allowExpandedCount,
                                         uint8_t fixedLength)
        {
            uint32_t charLength;
                ...
                do
                {
                    ...
                    if (ix > 25)
                        throw new PlatformException(
                            "Counted string too long 25 * 255", ERANGE,
                            __FILE__, __LINE__, __FUNCTION__);
                } while (b == 255);
            }
            else
        ...
        void MP4File::WriteCountedString(char* string, uint8_t charSize,
                                         bool allowExpandedCount,
                                         uint32_t fixedLength)
        {
            ...
            else
            {
                if (charLength > 255)
                {
                    ...
                    throw new PlatformException(msg.str().c_str(), ERANGE,
                                                __FILE__, __LINE__,
                                                __FUNCTION__);
                }
                // Write the count
