    ...
    namespace impl
    {
        ...
        void MP4SoundAtom::Read()
        {
            ...
            if (ATOMID(parent->GetType()) != ATOMID("stsd"))
            {
                ...
                // which is blank inside a wave atom, which is inside an mp4a
                // atom we have a mp4a inside an wave inside an mp4a - delete
                // all properties
                for (int i = 0; i < 9; ++i)
                    delete m_pProperties[i]; // make sure we delete the
                                             // properties themselves, then
                                             // remove from  m_pProperties
                ...
                {
                    AddProperty(
                        new MP4BytesProperty(*this, "decoderConfig", m_size));
                    ReadProperties();
                }
            ...
            else
            {
                ...
                AddProperties(
                    ((MP4IntegerProperty*)m_pProperties[2])->GetValue());
