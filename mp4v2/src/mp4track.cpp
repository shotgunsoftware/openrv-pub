...
#define AMR_FALSE 1
            ...
            , m_trakAtom(trakAtom)
        {
            ...
            // quicktime audio constant bit rate samples, which have non-1
            // values
            m_bytesPerSample = 1;
            m_samplesPerChunk = 0;
            ...
            success &= m_trakAtom.FindProperty(
                "trak.tkhd.trackId", (MP4Property**)&pTrackIdProperty);
            if (success)
            {
            ...
            success &=
                m_trakAtom.FindProperty("trak.mdia.mdhd.timeScale",
            ...
            success &= m_trakAtom.FindProperty(
                "trak.tkhd.duration", (MP4Property**)&m_pTrackDurationProperty);
            ...
            // get handles on information needed to map sample id's to file
            // offsets

            success &=
                m_trakAtom.FindProperty("trak.mdia.minf.stbl.stsc.entryCount",
            ...
            bool haveStco =
                m_trakAtom.FindProperty("trak.mdia.minf.stbl.stco.entryCount",
            ...
            bool haveCtts =
                m_trakAtom.FindProperty("trak.mdia.minf.stbl.ctts.entryCount",
            ...
            bool haveStss =
                m_trakAtom.FindProperty("trak.mdia.minf.stbl.stss.entryCount",
            ...
            if (!success)
            {
                throw new Exception("invalid track", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            MP4SdtpAtom* sdtp =
                (MP4SdtpAtom*)m_trakAtom.FindAtom("trak.mdia.minf.stbl.sdtp");
            if (sdtp)
            {
                uint8_t* buffer;
                uint32_t bufsize;
        ...
        const char* MP4Track::GetType() { return m_pTypeProperty->GetValue(); }
        ...
        void MP4Track::ReadSample(MP4SampleId sampleId, uint8_t** ppBytes,
                                  uint32_t* pNumBytes, MP4Timestamp* pStartTime,
                                  ...
                                  uint32_t* dependencyFlags)
        {
            if (sampleId == MP4_INVALID_SAMPLE_ID)
                throw new Exception("sample id can't be zero", __FILE__,
            ...
            if (dependencyFlags)
            {
                ...
                else
                {
                    ...
                    *dependencyFlags =
                        m_sdtpLog[sampleId - 1]; // sampleId is 1-based
                }
            }
            ...
            {
                throw new Exception("sample buffer is too small", __FILE__,
                                    __LINE__, __FUNCTION__);
            }
            *pNumBytes = sampleSize;

            log.verbose3f("\"%s\": ReadSample: track %u id %u offset 0x%" PRIx64
                          " size %u (0x%x)",
                          GetFile().GetFilename().c_str(), m_trackId, sampleId,
            ...
            uint64_t oldPos =
                m_File.GetPosition(fin); // only used in mode == 'w'
            try
            {
                ...
                if (pStartTime || pDuration)
                {
                    ...
                    log.verbose3f("\"%s\": ReadSample:  start %" PRIu64
                                  " duration %" PRId64,
                ...
                if (pRenderingOffset)
                {
                    *pRenderingOffset = GetSampleRenderingOffset(sampleId);
                    ...
                    log.verbose3f(
                        "\"%s\": ReadSample:  renderingOffset %" PRId64,
        ...
        void MP4Track::ReadSampleFragment(MP4SampleId sampleId,
                                          ...
                                          uint16_t sampleLength, uint8_t* pDest)
        {
            if (sampleId == MP4_INVALID_SAMPLE_ID)
            {
                throw new Exception("invalid sample id", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            if (sampleId != m_cachedReadSampleId)
            {
                ...
                ReadSample(sampleId, &m_pCachedReadSample,
        ...
        void MP4Track::WriteSample(const uint8_t* pBytes, uint32_t numBytes,
                                   ...
                                   bool isSyncSample)
        {
            ...
            log.verbose3f("\"%s\": WriteSample: track %u id %u size %u (0x%x) ",
                          GetFile().GetFilename().c_str(), m_trackId,
                          m_writeSampleId, numBytes, numBytes);

            if (pBytes == NULL && numBytes > 0)
            {
                throw new Exception("no sample data", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            {
                curMode =
                    (pBytes[0] >> 3) & 0x000F; // The mode is in the first byte
            }
            ...
            log.verbose3f("\"%s\": duration %" PRIu64,
                          GetFile().GetFilename().c_str(), duration);
            ...
            {
                m_pChunkBuffer = (uint8_t*)MP4Realloc(
                    m_pChunkBuffer, m_chunkBufferSize + numBytes);
            ...
            memcpy(&m_pChunkBuffer[m_sizeOfDataInChunkBuffer], pBytes,
                   numBytes);
        ...
        void MP4Track::WriteSampleDependency(const uint8_t* pBytes,
                                             uint32_t numBytes,
                                             MP4Duration duration,
                                             MP4Duration renderingOffset,
                                             bool isSyncSample,
                                             uint32_t dependencyFlags)
        {
            m_sdtpLog.push_back(dependencyFlags); // record dependency flags for
                                                  // processing at finish
            WriteSample(pBytes, numBytes, duration, renderingOffset,
                        isSyncSample);
        }
        ...
        void MP4Track::WriteChunkBuffer()
        {
            ...
            log.verbose3f("\"%s\": WriteChunk: track %u offset 0x%" PRIx64
                          " size %u (0x%x) numSamples %u",
                          GetFile().GetFilename().c_str(), m_trackId,
                          chunkOffset, m_sizeOfDataInChunkBuffer,
            ...
            // note: we do not free our chunk buffer; we reuse it, expanding as
            // needed. It gets zapped when this class goes out of scope
            m_sizeOfDataInChunkBuffer = 0;
            m_chunkSamples = 0;
            m_chunkDuration = 0;
        }
        ...
        void MP4Track::FinishWrite(uint32_t options)
        {
                ...
                && m_stsz_sample_bits == 4)
            {
                ...
                {
                    ((MP4Integer8Property*)m_pStszSampleSizeProperty)
                        ->AddValue(m_stz2_4bit_sample_value);
            ...
            if (m_trakAtom.FindProperty("trak.mdia.minf.stbl.stsd.*.esds."
            ...
            m_trakAtom.FindProperty("trak.udta.name.value",
                                    (MP4Property**)&nameProperty);
            if (nameProperty != NULL && nameProperty->GetValueSize() == 0)
            {
                // Zero length name value--delete name, and then udta if no
                // child atoms
        ...
        // from encodings using low number of bframes. It's expected sdtp may
        // help other QT-based players.
        //
        void MP4Track::FinishSdtp()
        {
            // bail if log is empty -- indicates dependency information was not
            // written
            if (m_sdtpLog.empty())
                return;
            ...
            sdtp->data.SetValue((const uint8_t*)m_sdtpLog.data(),
                                (uint32_t)m_sdtpLog.size());
        ...
        uint32_t MP4Track::GetSampleSize(MP4SampleId sampleId)
        {
            ...
            {
                uint8_t value =
                    m_pStszSampleSizeProperty->GetValue((sampleId - 1) / 2);
                if ((sampleId - 1) / 2 == 0)
                {
                    value >>= 4;
                }
        ...
        uint64_t MP4Track::GetTotalOfSampleSizes()
        {
            ...
            if (m_pStszFixedSampleSizeProperty != NULL)
            {
                ...
                // if fixed sample size, just need to multiply by number of
                // samples
                if (fixedSampleSize != 0)
                {
        ...
        void MP4Track::SampleSizePropertyAddValue(uint32_t size)
        {
            ...
            case Integer16Property:
                ((MP4Integer16Property*)m_pStszSampleSizeProperty)
                    ->AddValue(size);
                break;
        ...
        void MP4Track::UpdateSampleSizes(MP4SampleId sampleId,
                                         uint32_t numBytes)
        {
            if (m_bytesPerSample > 1)
            {
                ...
                {
                    // error
                    log.errorf("%s: \"%s\": numBytes %u not divisible by "
                               "bytesPerSample %u sampleId %u",
            ...
            {
                if (m_pStszFixedSampleSizeProperty == NULL || numBytes == 0)
                {
            ...
            { // sampleId > 1
                ...
                if (fixedSampleSize == 0 || numBytes != fixedSampleSize)
                {
                    if (fixedSampleSize != 0)
                    {
                        // fixed size was set; we need to clear fixed sample
                        // size
                        m_pStszFixedSampleSizeProperty->SetValue(0);

                        // and create sizes for all previous samples
                        // use GetNumberOfSamples due to needing the total
                        // number not just the appended part of the file
        ...
        uint32_t MP4Track::GetMaxBitrate()
        {
            ...
            for (MP4SampleId sid = 1; sid <= numSamples; sid++)
            {
                ...
                else
                {
                    ...
                    // now, calculate the number of bytes we overflowed.  Round
                    // up.
                    if (lastSampleDur > 0)
                    {
                        uint32_t overflow_bytes = 0;
                        overflow_bytes = ((lastSampleSize * overflow_dur)
                                          + (lastSampleDur - 1))
                                         / lastSampleDur;
        ...
        uint32_t MP4Track::GetSampleStscIndex(MP4SampleId sampleId)
        {
            ...
            if (numStscs == 0)
            {
                throw new Exception("No data chunks exist", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
        ...
        File* MP4Track::GetSampleFile(MP4SampleId sampleId)
        {
            ...
            MP4Atom* pStsdAtom =
                m_trakAtom.FindAtom("trak.mdia.minf.stbl.stsd");
            ASSERT(pStsdAtom);
            ...
            if (!pStsdEntryAtom->FindProperty(
                    "*.dataReferenceIndex", (MP4Property**)&pDrefIndexProperty)
                || pDrefIndexProperty == NULL)
            {
                throw new Exception("invalid stsd entry", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            MP4Atom* pDrefAtom =
                m_trakAtom.FindAtom("trak.mdia.minf.dinf.dref");
            ASSERT(pDrefAtom);
            ...
            // make sure this is actually a url atom (somtimes it's "cios", like
            // in iTunes videos)
            if (strcmp(pUrlAtom->GetType(), "url ") || pUrlAtom->GetFlags() & 1)
            {
            ...
            else
            {
                ...
                ASSERT(pUrlAtom->FindProperty(
                    "*.location", (MP4Property**)&pLocationProperty));
                ASSERT(pLocationProperty);

                const char* url = pLocationProperty->GetValue();

                log.verbose3f("\"%s\": dref url = %s",
                              GetFile().GetFilename().c_str(), url);
        ...
        uint64_t MP4Track::GetSampleFileOffset(MP4SampleId sampleId)
        {
            ...
            // is stored in.
            MP4ChunkId chunkId =
                firstChunk + ((sampleId - firstSample) / samplesPerChunk);

            // chunkOffset is the file offset (absolute) for the start of the
            // chunk
                                           ...
                                           uint32_t samplesPerChunk)
        {
            ...
            else
            {
                ...
                m_pStscFirstSampleProperty->AddValue(sampleId - samplesPerChunk
                                                     + 1);
        ...
        void MP4Track::UpdateChunkOffsets(uint64_t chunkOffset)
        {
            if (m_pChunkOffsetProperty->GetType() == Integer32Property)
            {
                ((MP4Integer32Property*)m_pChunkOffsetProperty)
                    ->AddValue(chunkOffset);
            }
            else
            {
                ((MP4Integer64Property*)m_pChunkOffsetProperty)
                    ->AddValue(chunkOffset);
            }
                                      ...
                                      MP4Duration* pDuration)
        {
            ...
            if (m_cachedSttsSid != MP4_INVALID_SAMPLE_ID
                && sampleId >= m_cachedSttsSid)
            {
                sid = m_cachedSttsSid;
            ...
            for (uint32_t sttsIndex = m_cachedSttsIndex; sttsIndex < numStts;
                 sttsIndex++)
            {
            ...
            throw new Exception("sample id out of range", __FILE__, __LINE__,
                                __FUNCTION__);
        }

        MP4SampleId MP4Track::GetSampleIdFromTime(MP4Timestamp when,
                                                  bool wantSyncSample)
        {
            uint32_t numStts = m_pSttsCountProperty->GetValue();
            MP4SampleId sid = 1;
            ...
            for (uint32_t sttsIndex = 0; sttsIndex < numStts; sttsIndex++)
            {
                ...
                {
                    log.warningf(
                        "%s: \"%s\": Zero sample duration, stts entry %u",
                        __FUNCTION__, GetFile().GetFilename().c_str(),
                        sttsIndex);
                }
            ...
            throw new Exception("time out of range", __FILE__, __LINE__,
                                __FUNCTION__);
        ...
        void MP4Track::UpdateSampleTimes(MP4Duration duration)
        {
            ...
            if (numStts
                && duration
                       == m_pSttsSampleDeltaProperty->GetValue(numStts - 1))
            {
                                              ...
                                              MP4SampleId* pFirstSampleId)
        {
            ...
            if (m_cachedCttsSid != MP4_INVALID_SAMPLE_ID
                && sampleId >= m_cachedCttsSid)
            {
                sid = m_cachedCttsSid;
            }
            else
            ...
            for (uint32_t cttsIndex = m_cachedCttsIndex; cttsIndex < numCtts;
                 cttsIndex++)
            {
                uint32_t sampleCount =
                                              ...
                                              MP4Duration renderingOffset)
        {
            ...
            if (m_pCttsCountProperty == NULL)
            {
                ...
                ASSERT(pCttsAtom->FindProperty(
                    "ctts.entryCount", (MP4Property**)&m_pCttsCountProperty));
            ...
            {
                // add ctts entry, sampleCount = 1, sampleOffset =
                // renderingOffset
                                                ...
                                                MP4Duration renderingOffset)
        {
            ...
            if (sampleCount == 1)
            {
                ...
                m_pCttsSampleOffsetProperty->SetValue(renderingOffset,
                                                      cttsIndex);
                return;
            }
            ...
            if (sampleId == firstSampleId)
            {
                ...
                m_pCttsSampleCountProperty->InsertValue(1, cttsIndex);
                m_pCttsSampleOffsetProperty->InsertValue(renderingOffset,
                                                         cttsIndex);

                m_pCttsSampleCountProperty->SetValue(sampleCount - 1,
                                                     cttsIndex + 1);
            ...
            else if (sampleId == lastSampleId)
            {
                ...
                m_pCttsSampleCountProperty->InsertValue(1, cttsIndex + 1);
                m_pCttsSampleOffsetProperty->InsertValue(renderingOffset,
                                                         cttsIndex + 1);
            ...
            else
            {
                ...
                m_pCttsSampleCountProperty->SetValue(sampleId - firstSampleId,
                                                     cttsIndex);

                // insert new entry for those samples beyond our sample
                m_pCttsSampleCountProperty->InsertValue(lastSampleId - sampleId,
                                                        cttsIndex + 2);
                uint32_t oldRenderingOffset =
                    m_pCttsSampleOffsetProperty->GetValue(cttsIndex);
                m_pCttsSampleOffsetProperty->InsertValue(oldRenderingOffset,
                                                         cttsIndex + 2);
        ...
        void MP4Track::UpdateSyncSamples(MP4SampleId sampleId,
                                         bool isSyncSample)
        {
            if (isSyncSample)
            {
        ...
        MP4Atom* MP4Track::AddAtom(const char* parentName,
                                   const char* childName)
        {
            ...
            MP4Atom* pChildAtom =
                MP4Atom::CreateAtom(m_File, pParentAtom, childName);
        ...
        void MP4Track::UpdateDurations(MP4Duration duration)
        {
            ...
            MP4Duration movieDuration =
                ToMovieDuration(m_pMediaDurationProperty->GetValue());
        ...
        void MP4Track::ReadChunk(MP4ChunkId chunkId, uint8_t** ppChunk,
                                 uint32_t* pChunkSize)
        {
            ASSERT(chunkId);
            ASSERT(ppChunk);
            ...
            *ppChunk = (uint8_t*)MP4Malloc(*pChunkSize);
            ...
            log.verbose3f("\"%s\": ReadChunk: track %u id %u offset 0x%" PRIx64
                          " size %u (0x%x)",
                          GetFile().GetFilename().c_str(), m_trackId, chunkId,
                          chunkOffset, *pChunkSize, *pChunkSize);
        ...
        void MP4Track::RewriteChunk(MP4ChunkId chunkId, uint8_t* pChunk,
                                    uint32_t chunkSize)
        {
            uint64_t chunkOffset = m_File.GetPosition();
            ...
            log.verbose3f(
                "\"%s\": RewriteChunk: track %u id %u offset 0x%" PRIx64
                " size %u (0x%x)",
                GetFile().GetFilename().c_str(), m_trackId, chunkId,
        ...
        bool MP4Track::InitEditListProperties()
        {
            ...
            MP4Atom* pElstAtom = m_trakAtom.FindAtom("trak.edts.elst");
            ...
            (void)pElstAtom->FindProperty("elst.entryCount",
            ...
            (void)pElstAtom->FindProperty("elst.entries.mediaRate",
            ...
            return m_pElstCountProperty && m_pElstMediaTimeProperty
                   && m_pElstDurationProperty && m_pElstRateProperty
        ...
        void MP4Track::DeleteEdit(MP4EditId editId)
        {
            if (editId == MP4_INVALID_EDIT_ID)
            {
                throw new Exception("edit id can't be zero", __FILE__, __LINE__,
                                    __FUNCTION__);
            }

            if (!m_pElstCountProperty || m_pElstCountProperty->GetValue() == 0)
            {
                throw new Exception("no edits exist", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            if (m_pElstCountProperty->GetValue() == 0)
            {
                ...
                m_trakAtom.DeleteChildAtom(m_trakAtom.FindAtom("trak.edts"));
            }
        }

        MP4Timestamp MP4Track::GetEditStart(MP4EditId editId)
        {
            if (editId == MP4_INVALID_EDIT_ID)
            {
        ...
        MP4Duration MP4Track::GetEditTotalDuration(MP4EditId editId)
        {
            uint32_t numEdits = 0;
            ...
            {
                totalDuration += m_pElstDurationProperty->GetValue(eid - 1);
            }

            return totalDuration;
        }

        MP4SampleId MP4Track::GetSampleIdFromEditTime(MP4Timestamp editWhen,
                                                      MP4Timestamp* pStartTime,
                                                      MP4Duration* pDuration)
        {
            ...
            if (numEdits)
            {
                ...
                for (MP4EditId editId = 1; editId <= numEdits; editId++)
                {
                    ...
                    MP4Duration editOffset = editWhen - editStartTime;
                    ...
                    // would naturally start and when it starts in the edit
                    // timeline
                    MP4Duration sampleStartOffset = mediaWhen - sampleStartTime;

                    // calculate the start time for the sample in the edit time
                    // line
                    ...
                    // calculate how long this sample lasts in the edit list
                    // timeline
                    ...
                    log.verbose2f(
                        "\"%s\": GetSampleIdFromEditTime: when %" PRIu64 " "
                        "sampleId %u start %" PRIu64 " duration %" PRId64,
                        GetFile().GetFilename().c_str(), editWhen, sampleId,
        ...
        void MP4Track::CalculateBytesPerSample()
        {
                ...
                || (ATOMID(media_data_name) == ATOMID("sowt")))
            {
                ...
                m_bytesPerSample =
                    chan->GetValue() * (sampleSize->GetValue() / 8);
            }
        }
