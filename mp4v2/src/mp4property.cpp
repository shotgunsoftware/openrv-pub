...
namespace mp4v2
{
    namespace impl
    {
        ...
        void MP4IntegerProperty::IncrementValue(int32_t increment,
                                                uint32_t index)
        {
            SetValue(GetValue() + increment);
        }

        void MP4Integer8Property::Dump(uint8_t indent, bool dumpImplicits,
                                       uint32_t index)
        {
            ...
            if (index != 0)
                log.dump(indent, MP4_LOG_VERBOSE1,
                         ...
                         index, m_values[index], m_values[index]);
            else
        ...
        void MP4Integer16Property::Dump(uint8_t indent, bool dumpImplicits,
                                        uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
        ...
        void MP4Integer24Property::Dump(uint8_t indent, bool dumpImplicits,
                                        uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
        ...
        void MP4Integer32Property::Dump(uint8_t indent, bool dumpImplicits,
                                        uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
        ...
        void MP4Integer64Property::Dump(uint8_t indent, bool dumpImplicits,
                                        uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
            ...
            if (index != 0)
                log.dump(indent, MP4_LOG_VERBOSE1,
                         "\"%s\": %s[%u] = %" PRIu64 " (0x%016" PRIx64 ")",
                         m_parentAtom.GetFile().GetFilename().c_str(), m_name,
                         index, m_values[index], m_values[index]);
            else
                log.dump(indent, MP4_LOG_VERBOSE1,
                         "\"%s\": %s = %" PRIu64 " (0x%016" PRIx64 ")",
                         m_parentAtom.GetFile().GetFilename().c_str(), m_name,
                         m_values[index], m_values[index]);
        }
        ...
        void MP4BitfieldProperty::Dump(uint8_t indent, bool dumpImplicits,
                                       uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
            ...
            if (index != 0)
                log.dump(
                    indent, MP4_LOG_VERBOSE1,
                    "\"%s\": %s[%u] = %" PRIu64 " (0x%0*" PRIx64 ") <%u bits>",
                    m_parentAtom.GetFile().GetFilename().c_str(), m_name, index,
                    m_values[index], (int)hexWidth, m_values[index], m_numBits);
            else
                log.dump(indent, MP4_LOG_VERBOSE1,
                         "\"%s\": %s = %" PRIu64 " (0x%0*" PRIx64 ") <%u bits>",
                         m_parentAtom.GetFile().GetFilename().c_str(), m_name,
                         m_values[index], (int)hexWidth, m_values[index],
                         m_numBits);
        }
        ...
        void MP4Float32Property::Dump(uint8_t indent, bool dumpImplicits,
                                      uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
            {
        ...
        MP4StringProperty::MP4StringProperty(MP4Atom& parentAtom,
                                             const char* name,
                                             bool useCountedFormat,
                                             bool useUnicode, bool arrayMode)
        ...
        void MP4StringProperty::SetValue(const char* value, uint32_t index)
        {
            if (m_readOnly)
            {
                ...
                throw new PlatformException(msg.str().c_str(), EACCES, __FILE__,
        ...
        void MP4StringProperty::Read(MP4File& file, uint32_t index)
        {
            ...
            for (uint32_t i = begin; i < max; i++)
            {
                ...
                // Generally a default atom setting, e.g. see atom_avc1.cpp,
                // "JVT/AVC Coding"; we'll leak this string if we don't free.
                // Note that MP4Free checks for null.
                MP4Free(value);
        ...
        void MP4StringProperty::Write(MP4File& file, uint32_t index)
        {
            ...
            for (uint32_t i = begin; i < max; i++)
            {
                ...
                if (m_useCountedFormat)
                {
                    file.WriteCountedString(value, (m_useUnicode ? 2 : 1),
        ...
        void MP4StringProperty::Dump(uint8_t indent, bool dumpImplicits,
                                     uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
                return;
        ...
        MP4BytesProperty::MP4BytesProperty(MP4Atom& parentAtom,
                                           const char* name, uint32_t valueSize,
        ...
        void MP4BytesProperty::SetValue(const uint8_t* pValue,
                                        uint32_t valueSize, uint32_t index)
        {
            if (m_readOnly)
            {
                ostringstream msg;
            ...
            if (m_fixedValueSize)
            {
                ...
                {
                    ostringstream msg;
                    msg << GetParentAtom().GetType() << "." << GetName()
                        << " value size " << valueSize
                        << " exceeds fixed value size " << m_fixedValueSize;
                    throw new Exception(msg.str().c_str(), __FILE__, __LINE__,
                                        __FUNCTION__);
                }
        ...
        void MP4BytesProperty::SetValueSize(uint32_t valueSize, uint32_t index)
        {
            ...
            {
                m_values[index] =
                    (uint8_t*)MP4Realloc(m_values[index], valueSize);
            }
            m_valueSizes[index] = valueSize;
        }
        ...
        void MP4BytesProperty::Dump(uint8_t indent, bool dumpImplicits,
                                    uint32_t index)
        {
            if (m_implicit && !dumpImplicits)
                return;
            ...
            if (size <= 16)
            {
                ...
                for (uint32_t i = 0; i < size; i++)
                {
                    if (i)
                        oss << ' ';
                    oss << hex << setw(2) << setfill('0') << right
            ...
            // specialization for ilst item data always show all bytes except
            // for covr
            bool showall = false;
            MP4Atom* const datac =
                m_parentAtom.GetParentAtom(); // data container
            MP4Atom* const datacc = datac->GetParentAtom();
            if (datacc && ATOMID(datacc->GetType()) == ATOMID("ilst")
            ...
            if (showall || size < 128 || log.verbosity >= MP4_LOG_VERBOSE2)
            {
                adjsize = size;
                supressed = false;
            }
        ...
        MP4TableProperty::MP4TableProperty(MP4Atom& parentAtom,
                                           const char* name,
        ...
        void MP4TableProperty::Write(MP4File& file, uint32_t index)
        {
            ...
            {
                log.errorf("%s: \"%s\": %s %s \"%s\"table entries %u doesn't "
                           "match count %u",
                           __FUNCTION__,
                           m_parentAtom.GetFile().GetFilename().c_str(),
                           GetParentAtom().GetType(), GetName(),
                           m_pProperties[0]->GetName(),
        ...
        void MP4TableProperty::Dump(uint8_t indent, bool dumpImplicits,
                                    uint32_t index)
        {
            ASSERT(index == 0);
        ...
        MP4DescriptorProperty::MP4DescriptorProperty(
            MP4Atom& parentAtom, const char* name, uint8_t tagsStart,
            uint8_t tagsEnd, bool mandatory, bool onlyOne)
            : MP4Property(parentAtom, name)
        {
                                                 ...
                                                 uint32_t* pIndex)
        {
            ...
            if (haveDescrIndex)
            {
                return m_pDescriptors[descrIndex]->FindProperty(
                    name, ppProperty, pIndex);
            }
            else
            {
                return FindContainedProperty(name, ppProperty, pIndex);
        ...
        bool MP4DescriptorProperty::FindContainedProperty(
            const char* name, MP4Property** ppProperty, uint32_t* pIndex)
        {
            for (uint32_t i = 0; i < m_pDescriptors.Size(); i++)
            {
        ...
        void MP4DescriptorProperty::Read(MP4File& file, uint32_t index)
        {
            ...
            while (true)
            {
                ...
                MP4Descriptor* pDescriptor = AddDescriptor(tag);
            ...
            if (m_mandatory && m_pDescriptors.Size() == 0)
            {
                log.warningf("%s: \"%s\": Mandatory descriptor 0x%02x missing",
                             __FUNCTION__,
                             GetParentAtom().GetFile().GetFilename().c_str(),
                             m_tagsStart);
            }
            else if (m_onlyOne && m_pDescriptors.Size() > 1)
            {
                log.warningf(
                    "%s: \"%s\": Descriptor 0x%02x has more than one instance",
                    __FUNCTION__,
                    GetParentAtom().GetFile().GetFilename().c_str(),
                    m_tagsStart);
            }
        ...
        void MP4DescriptorProperty::Dump(uint8_t indent, bool dumpImplicits,
                                         uint32_t index)
        {
            ASSERT(index == 0);
        ...
        MP4LanguageCodeProperty::MP4LanguageCodeProperty(
            MP4Atom& parentAtom, const char* name, bmff::LanguageCode value)
            : MP4Property(parentAtom, name)
        {
            SetValue(value);
        }

        MP4LanguageCodeProperty::MP4LanguageCodeProperty(MP4Atom& parentAtom,
                                                         const char* name,
                                                         const string& code)
            : MP4Property(parentAtom, name)
        {
        ...
        void MP4LanguageCodeProperty::Dump(uint8_t indent, bool dumpImplicits,
                                           uint32_t index)
        {
            uint16_t data = 0;
            ...
            log.dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s = %s (0x%04x)",
                     m_parentAtom.GetFile().GetFilename().c_str(), m_name,
                     bmff::enumLanguageCode.toString(_value, true).c_str(),
                     data);
        }

        uint32_t MP4LanguageCodeProperty::GetCount() { return 1; }

        MP4PropertyType MP4LanguageCodeProperty::GetType()
        {
            return LanguageCodeProperty;
        }

        bmff::LanguageCode MP4LanguageCodeProperty::GetValue()
        {
            return _value;
        }

        void MP4LanguageCodeProperty::Read(MP4File& file, uint32_t index)
        {
        ...
        void MP4LanguageCodeProperty::SetCount(uint32_t count)
        {
            // do nothing; count is always 1
        }

        void MP4LanguageCodeProperty::SetValue(bmff::LanguageCode value)
        {
            _value = value;
        }

        void MP4LanguageCodeProperty::Write(MP4File& file, uint32_t index)
        {
            uint16_t data = 0;
        ...
        MP4BasicTypeProperty::MP4BasicTypeProperty(MP4Atom& parentAtom,
                                                   const char* name,
                                                   itmf::BasicType type)
            : MP4Property(parentAtom, name)
        {
        ...
        void MP4BasicTypeProperty::Dump(uint8_t indent, bool dumpImplicits,
                                        uint32_t index)
        {
            log.dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %s (0x%02x)",
                     m_parentAtom.GetFile().GetFilename().c_str(), m_name,
                     itmf::enumBasicType.toString(_value, true).c_str(),
                     _value);
        }

        uint32_t MP4BasicTypeProperty::GetCount() { return 1; }

        MP4PropertyType MP4BasicTypeProperty::GetType()
        {
            return BasicTypeProperty;
        }

        itmf::BasicType MP4BasicTypeProperty::GetValue() { return _value; }

        void MP4BasicTypeProperty::Read(MP4File& file, uint32_t index)
        {
            SetValue(static_cast<itmf::BasicType>(file.ReadBits(8)));
        }

        void MP4BasicTypeProperty::SetCount(uint32_t count)
        {
            // do nothing; count is always 1
        }

        void MP4BasicTypeProperty::SetValue(itmf::BasicType value)
        {
            _value = value;
        }

        void MP4BasicTypeProperty::Write(MP4File& file, uint32_t index)
        {
