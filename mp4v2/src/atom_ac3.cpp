...
namespace mp4v2
{
    namespace impl
    {
        ...
        void MP4Ac3Atom::Generate()
        {
            ...
            ((MP4Integer16Property*)m_pProperties[1])
                ->SetValue(1); // data-reference-index
            ((MP4Integer16Property*)m_pProperties[3])
                ->SetValue(2); // channelCount - ignored
            ((MP4Integer16Property*)m_pProperties[4])
                ->SetValue(0x0010); // sampleSize - ignored

            // The user should set the samplingRate as appropriate - and create
            // the dac3 atom
        }
