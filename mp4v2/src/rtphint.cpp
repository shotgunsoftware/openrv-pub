...
namespace mp4v2
{
    namespace impl
    {
        ...
        void MP4RtpHintTrack::InitRtpStart()
        {
            ...
            (void)m_trakAtom.FindProperty("trak.udta.hnti.rtp .snro.offset",
            ...
            (void)m_trakAtom.FindProperty("trak.udta.hnti.rtp .tsro.offset",
        ...
        void MP4RtpHintTrack::ReadHint(MP4SampleId hintSampleId,
                                       uint16_t* pNumPackets)
        {
            ...
            ReadSample(hintSampleId, &m_pReadHintSample, &m_readHintSampleSize,
                       &m_readHintTimestamp);
        ...
        uint16_t MP4RtpHintTrack::GetHintNumberOfPackets()
        {
            if (m_pReadHint == NULL)
            {
                throw new Exception("no hint has been read", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
        ...
        bool MP4RtpHintTrack::GetPacketBFrame(uint16_t packetIndex)
        {
            ...
            MP4RtpPacket* pPacket = m_pReadHint->GetPacket(packetIndex);
        ...
        void MP4RtpHintTrack::ReadPacket(uint16_t packetIndex,
                                         uint8_t** ppBytes, uint32_t* pNumBytes,
                                         uint32_t ssrc, bool addHeader,
                                         bool addPayload)
        {
            if (m_pReadHint == NULL)
            {
            ...
            {
                throw new Exception("no data requested", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            *pNumBytes = 0;
            ...
            try
            {
                ...
                if (addHeader)
                {
                    *pDest++ = 0x80 | (pPacket->GetPBit() << 5)
                               | (pPacket->GetXBit() << 4);

                    *pDest++ =
                        (pPacket->GetMBit() << 7) | pPacket->GetPayload();

                    *((uint16_t*)pDest) = MP4V2_HTONS(
                        m_rtpSequenceStart + pPacket->GetSequenceNumber());
                    pDest += 2;

                    *((uint32_t*)pDest) = MP4V2_HTONL(
                        m_rtpTimestampStart + (uint32_t)m_readHintTimestamp);
                    pDest += 4;

                    *((uint32_t*)pDest) = MP4V2_HTONL(ssrc);
                    pDest += 4;
                }
        ...
        void MP4RtpHintTrack::SetRtpTimestampStart(MP4Timestamp start)
        {
            if (!m_pTsroProperty)
            {
                MP4Atom* pTsroAtom = m_File.AddDescendantAtoms(
                    &m_trakAtom, "udta.hnti.rtp .tsro");
                                         ...
                                         bool include_mpeg4_esid)
        {
            ...
            snprintf(rtpMapBuf, len, "%s/%u%c%s", payloadName, GetTimeScale(),
        ...
        void MP4RtpHintTrack::AddHint(bool isBFrame, uint32_t timestampOffset)
        {
            ...
            if (m_pWriteHint)
            {
                throw new Exception("unwritten hint is still pending", __FILE__,
        ...
        void MP4RtpHintTrack::AddPacket(bool setMbit, int32_t transmitOffset)
        {
            ...
            {
                throw new Exception("no hint pending", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            ...
            pPacket->Set(m_pPayloadNumberProperty->GetValue(),
                         m_writePacketId++, setMbit);
        ...
        void MP4RtpHintTrack::AddImmediateData(const uint8_t* pBytes,
                                               uint32_t numBytes)
        {
            if (m_pWriteHint == NULL)
            {
            ...
            if (pPacket == NULL)
            {
                throw new Exception("no packet pending", __FILE__, __LINE__,
                                    __FUNCTION__);
            }

            if (pBytes == NULL || numBytes == 0)
            {
                throw new Exception("no data", __FILE__, __LINE__,
                                    __FUNCTION__);
            }
            if (numBytes > 14)
            {
        ...
        void MP4RtpHintTrack::AddSampleData(MP4SampleId sampleId,
                                            uint32_t dataOffset,
                                            uint32_t dataLength)
        {
            if (m_pWriteHint == NULL)
        ...
        void MP4RtpHintTrack::AddESConfigurationPacket()
        {
            ...
            m_File.GetTrackESConfiguration(m_pRefTrack->GetId(), &pConfig,
                                           &configSize);
            ...
            {
                throw new Exception(
                    "ES configuration is too large for RTP payload", __FILE__,
                    __LINE__, __FUNCTION__);
            }
        ...
        void MP4RtpHintTrack::FinishWrite(uint32_t option)
        {
            if (m_writeHintId != MP4_INVALID_SAMPLE_ID)
            {
                ...
                if (m_pNump->GetValue())
                {
                    m_pAvgPdu->SetValue(m_pTrpy->GetValue()
                                        / m_pNump->GetValue());
                }
                ...
                if (GetDuration())
                {
                    m_pAvgBitRate->SetValue(m_pTrpy->GetValue() * 8
                                            * GetTimeScale() / GetDuration());
                }
            }
        ...
        void MP4RtpHintTrack::InitStats()
        {
            ...
            (void)pHinfAtom->FindProperty("hinf.trpy.bytes",
                                          (MP4Property**)&m_pTrpy);
            (void)pHinfAtom->FindProperty("hinf.nump.packets",
                                          (MP4Property**)&m_pNump);
            (void)pHinfAtom->FindProperty("hinf.tpyl.bytes",
                                          (MP4Property**)&m_pTpyl);
            (void)pHinfAtom->FindProperty("hinf.maxr.bytes",
                                          (MP4Property**)&m_pMaxr);
            (void)pHinfAtom->FindProperty("hinf.dmed.bytes",
                                          (MP4Property**)&m_pDmed);
            (void)pHinfAtom->FindProperty("hinf.dimm.bytes",
                                          (MP4Property**)&m_pDimm);
            (void)pHinfAtom->FindProperty("hinf.pmax.bytes",
                                          (MP4Property**)&m_pPmax);
            (void)pHinfAtom->FindProperty("hinf.dmax.milliSecs",
                                          (MP4Property**)&m_pDmax);
            ...
            (void)pHmhdAtom->FindProperty("hmhd.maxPduSize",
                                          (MP4Property**)&m_pMaxPdu);
            (void)pHmhdAtom->FindProperty("hmhd.avgPduSize",
                                          (MP4Property**)&m_pAvgPdu);
            (void)pHmhdAtom->FindProperty("hmhd.maxBitRate",
                                          (MP4Property**)&m_pMaxBitRate);
            (void)pHmhdAtom->FindProperty("hmhd.avgBitRate",
                                          (MP4Property**)&m_pAvgBitRate);
        ...
        {
            AddProperty(/* 0 */
                        new MP4Integer16Property(this->GetTrack().GetTrakAtom(),
                                                 "packetCount"));
            AddProperty(/* 1 */
                        new MP4Integer16Property(this->GetTrack().GetTrakAtom(),
                                                 "reserved"));
        ...
        void MP4RtpHint::Read(MP4File& file)
        {
            ...
            {
                log.verbose1f("\"%s\": ReadHint:",
                              GetTrack().GetFile().GetFilename().c_str());
                Dump(10, false);
            }
        }
            ...
            : m_hint(hint)
        {
            AddProperty(/* 0 */
                        new MP4Integer32Property(
                            this->GetHint().GetTrack().GetTrakAtom(),
                            "relativeXmitTime"));
            AddProperty(/* 1 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(),
                            "reserved1", 2));
            AddProperty(/* 2 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(), "Pbit",
                            1));
            AddProperty(/* 3 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(), "Xbit",
                            1));
            AddProperty(/* 4 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(),
            ...
            AddProperty(/* 5 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(), "Mbit",
                            1));
            AddProperty(/* 6 */
                        new MP4BitfieldProperty(
                            this->GetHint().GetTrack().GetTrakAtom(),
        ...
        void MP4RtpPacket::AddExtraProperties()
        {
            AddProperty(/* 13 */
                        new MP4Integer32Property(
                            this->GetHint().GetTrack().GetTrakAtom(),
                            "extraInformationLength"));

            // This is a bit of a hack, since the tlv entries are really defined
            // as atoms but there is only one type defined now, rtpo, and
            // getting our atom code hooked up here would be a major pain with
            // little gain
        ...
        void MP4RtpPacket::ReadExtra(MP4File& file)
        {
            ...
            if (extraLength < 4)
            {
                throw new Exception("bad packet extra info length", __FILE__,
                                    __LINE__, __FUNCTION__);
            }
            extraLength -= 4;
        ...
        void MP4RtpPacket::Set(uint8_t payloadNumber, uint32_t packetId,
                               bool setMbit)
        {
        ...
        void MP4RtpPacket::SetTimestampOffset(uint32_t timestampOffset)
        {
            ...
            ((MP4Integer32Property*)m_pProperties[16])
        ...
        void MP4RtpPacket::Dump(uint8_t indent, bool dumpImplicits)
        {
            ...
            for (uint32_t i = 0; i < m_rtpData.Size(); i++)
            {
                log.dump(indent, MP4_LOG_VERBOSE1, "\"%s\": RtpData: %u",
                         GetHint().GetTrack().GetFile().GetFilename().c_str(),
                         i);
        ...
        {
            AddProperty(/* 0 */
                        new MP4Integer8Property(this->GetPacket()
                                                    .GetHint()
                                                    .GetTrack()
                                                    .GetTrakAtom(),
                                                "type"));
        ...
        MP4Track* MP4RtpData::FindTrackFromRefIndex(uint8_t refIndex)
        {
            ...
            {
                // some other track
                MP4RtpHintTrack* pHintTrack = &GetPacket().GetHint().GetTrack();

                MP4Atom& trakAtom = pHintTrack->GetTrakAtom();

                MP4Integer32Property* pTrackIdProperty = NULL;
                (void)trakAtom.FindProperty("trak.tref.hint.entries",
                                            (MP4Property**)&pTrackIdProperty);
                ASSERT(pTrackIdProperty);

                uint32_t refTrackId = pTrackIdProperty->GetValue(refIndex - 1);
            ...
            : MP4RtpData(packet)
        {
            ...
            AddProperty(/* 1 */
                        new MP4BytesProperty(this->GetPacket()
                                                 .GetHint()
                                                 .GetTrack()
                                                 .GetTrakAtom(),
                                             "pad", 15));
            ...
            : MP4RtpData(packet)
        {
            ...
            AddProperty(/* 1 */
                        new MP4Integer8Property(this->GetPacket()
                                                ...
                                                "trackRefIndex"));
            AddProperty(/* 2 */
                        new MP4Integer16Property(this->GetPacket()
                                                     .GetHint()
                                                     .GetTrack()
                                                     .GetTrakAtom(),
                                                 "length"));
            AddProperty(/* 3 */
                        new MP4Integer32Property(this->GetPacket()
                                                     .GetHint()
                                                     .GetTrack()
                                                     .GetTrakAtom(),
                                                 "sampleNumber"));
        ...
        void MP4RtpSampleData::SetReferenceSample(MP4SampleId refSampleId,
                                                  uint32_t refSampleOffset,
                                                  uint16_t sampleLength)
        {
            ...
            ((MP4Integer32Property*)m_pProperties[4])
                ->SetValue(refSampleOffset);
        }

        void MP4RtpSampleData::SetEmbeddedSample(MP4SampleId sampleId,
                                                 MP4Track* pRefTrack,
                                                 MP4SampleId refSampleId,
                                                 uint32_t refSampleOffset,
                                                 uint16_t sampleLength)
        {
            ((MP4Integer8Property*)m_pProperties[1])->SetValue((uint8_t)-1);
        ...
        void MP4RtpSampleData::GetData(uint8_t* pDest)
        {
            ...
            pSampleTrack->ReadSampleFragment(
                ((MP4Integer32Property*)m_pProperties[3])
                    ->GetValue(), // sampleId
                ((MP4Integer32Property*)m_pProperties[4])
                    ->GetValue(), // sampleOffset
                ((MP4Integer16Property*)m_pProperties[2])
                    ->GetValue(), // sampleLength
                pDest);
        }

        void MP4RtpSampleData::WriteEmbeddedData(MP4File& file,
                                                 uint64_t startPos)
        {
            // if not using embedded data, nothing to do
            if (((MP4Integer8Property*)m_pProperties[1])->GetValue()
                != (uint8_t)-1)
            {
                return;
            }

            // figure out the offset within this hint sample for this embedded
            // data
            uint64_t offset = file.GetPosition() - startPos;
            ASSERT(offset <= 0xFFFFFFFF);
            ((MP4Integer32Property*)m_pProperties[4])
                ->SetValue((uint32_t)offset);
        ...
        void MP4RtpSampleDescriptionData::GetData(uint8_t* pDest)
        {
            ...
            // next find the desired atom in the track's sample description
            // table
            uint32_t sampleDescrIndex =
                ((MP4Integer32Property*)m_pProperties[3])->GetValue();

            MP4Atom& trakAtom = pSampleTrack->GetTrakAtom();

            char sdName[64];
            snprintf(sdName, 64, "trak.mdia.minf.stbl.stsd.*[%u]",
                     sampleDescrIndex);

            MP4Atom* pSdAtom = trakAtom.FindAtom(sdName);
            ...
            // the start of the sample descirption atom, or the start of the
            // atom's data. I believe it is the former, but the commented out
            // code will realize the latter interpretation if I turn out to be
            // wrong.
