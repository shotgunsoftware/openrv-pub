    ...
    namespace impl
    {
        namespace itmf
        {
            namespace
            {
                ...
                void __dataInit(MP4ItmfData& data)
                {
                ...
                void __dataClear(MP4ItmfData& data)
                {
                    if (data.value)
                        free(data.value);
                    __dataInit(data);
                ...
                void __dataListInit(MP4ItmfDataList& list)
                {
                    list.elements = NULL;
                    list.size = 0;
                }

                void __dataListClear(MP4ItmfDataList& list)
                {
                    if (list.elements)
                    {
                ...
                void __dataListResize(MP4ItmfDataList& list, uint32_t size)
                {
                    __dataListClear(list);

                    list.elements =
                        (MP4ItmfData*)malloc(sizeof(MP4ItmfData) * size);
                    list.size = size;
                ...
                void __itemInit(MP4ItmfItem& item)
                {
                    item.__handle = NULL;
                    item.code = NULL;
                    item.mean = NULL;
                ...
                void __itemClear(MP4ItmfItem& item)
                {
                    if (item.code)
                        free(item.code);
                    if (item.mean)
                ...
                void __itemListInit(MP4ItmfItemList& list)
                {
                    list.elements = NULL;
                    list.size = 0;
                }

                void __itemListClear(MP4ItmfItemList& list)
                {
                    if (list.elements)
                    {
                        for (uint32_t i = 0; i < list.size; i++)
                ...
                void __itemListResize(MP4ItmfItemList& list, uint32_t size)
                {
                    ...
                    list.elements =
                        (MP4ItmfItem*)malloc(sizeof(MP4ItmfItem) * size);
                    list.size = size;
                ...
                MP4ItmfItemList* __itemListAlloc()
                {
                    MP4ItmfItemList& list =
                        *(MP4ItmfItemList*)malloc(sizeof(MP4ItmfItemList));
                    __itemListInit(list);
                    return &list;
                }
                ...
                static bool __itemAtomToModel(MP4ItemAtom& item_atom,
                                              MP4ItmfItem& model)
                {
                    __itemClear(model);
                    ...
                    if (ATOMID(item_atom.GetType()) == ATOMID("----"))
                    {
                        ...
                        MP4MeanAtom* mean =
                            (MP4MeanAtom*)item_atom.FindAtom("----.mean");
                        if (!mean)
                            return true;

                        // copy atom UTF-8 value (not NULL-terminated) to model
                        // (NULL-terminated)
                        model.mean = mean->value.GetValueStringAlloc();

                        // name is optional
                        MP4NameAtom* name =
                            (MP4NameAtom*)item_atom.FindAtom("----.name");
                        if (name)
                        {
                            // copy atom UTF-8 value (not NULL-terminated) to
                    ...
                    const uint32_t childCount =
                        item_atom.GetNumberOfChildAtoms();
                    uint32_t dataCount = 0;
                    for (uint32_t i = 0; i < childCount; i++)
                    {
                        if (ATOMID(item_atom.GetChildAtom(i)->GetType())
                            != ATOMID("data"))
                            continue;
                        dataCount++;
                    }
                    ...
                    for (uint32_t i = 0, idata = 0; i < childCount; i++)
                    {
                        ...
                        MP4ItmfData& data_model =
                            model.dataList.elements[idata];

                        data_model.typeSetIdentifier =
                            data_atom.typeSetIdentifier.GetValue();
                        data_model.typeCode =
                            (MP4ItmfBasicType)data_atom.typeCode.GetValue();
                        data_model.locale = data_atom.locale.GetValue();

                        data_atom.metadata.GetValue(&data_model.value,
                                                    &data_model.valueSize);
                        idata++;
                    }
                ...
                static bool __itemModelToAtom(const MP4ItmfItem& model,
                                              MP4ItemAtom& atom)
                {
                    if (ATOMID(atom.GetType()) == ATOMID("----"))
                    {
                        ASSERT(model.mean); // mandatory
                        MP4MeanAtom& meanAtom =
                            *(MP4MeanAtom*)MP4Atom::CreateAtom(atom.GetFile(),
                                                               &atom, "mean");
                        atom.AddChildAtom(&meanAtom);
                        meanAtom.value.SetValue((const uint8_t*)model.mean,
                                                (uint32_t)strlen(model.mean));

                        if (model.name)
                        {
                            MP4NameAtom& nameAtom =
                                *(MP4NameAtom*)MP4Atom::CreateAtom(
                                    atom.GetFile(), &atom, "name");
                            atom.AddChildAtom(&nameAtom);
                            nameAtom.value.SetValue(
                                (const uint8_t*)model.name,
                                (uint32_t)strlen(model.name));
                        }
                    }
                    ...
                    for (uint32_t i = 0; i < model.dataList.size; i++)
                    {
                        ...
                        MP4DataAtom& dataAtom =
                            *(MP4DataAtom*)MP4Atom::CreateAtom(atom.GetFile(),
                                                               &atom, "data");
                        atom.AddChildAtom(&dataAtom);

                        dataAtom.typeSetIdentifier.SetValue(
                            dataModel.typeSetIdentifier);
                        dataAtom.typeCode.SetValue(
                            (itmf::BasicType)dataModel.typeCode);
                        dataAtom.locale.SetValue(dataModel.locale);
                        dataAtom.metadata.SetValue(dataModel.value,
                                                   dataModel.valueSize);
                    }
            ...
            MP4ItmfItemList* genericGetItems(MP4File& file)
            {
                ...
                for (uint32_t i = 0; i < list.size; i++)
                    __itemAtomToModel(*(MP4ItemAtom*)ilst->GetChildAtom(i),
                                      list.elements[i]);
            ...
            MP4ItmfItemList* genericGetItemsByCode(MP4File& file,
                                                   const string& code)
            {
                MP4Atom* ilst = file.FindAtom("moov.udta.meta.ilst");
                if (!ilst)
                ...
                for (uint32_t i = 0; i < childCount; i++)
                {
                    if (ATOMID(ilst->GetChildAtom(i)->GetType())
                        != ATOMID(code.c_str()))
                        continue;
                ...
                for (vector<uint32_t>::size_type i = 0; i < max; i++)
                {
                    ...
                    __itemAtomToModel(*(MP4ItemAtom*)ilst->GetChildAtom(aidx),
                                      list.elements[i]);
                }
            ...
            MP4ItmfItemList* genericGetItemsByMeaning(MP4File& file,
                                                      ...
                                                      const string& name)
            {
                ...
                for (uint32_t i = 0; i < childCount; i++)
                {
                    ...
                    MP4MeanAtom* meanAtom =
                        (MP4MeanAtom*)atom.FindAtom("----.mean");
                    if (!meanAtom)
                        continue;
                    ...
                    if (!name.empty())
                    {
                        ...
                        MP4MeanAtom* nameAtom =
                            (MP4MeanAtom*)atom.FindAtom("----.name");
                        if (!nameAtom)
                            continue;
            ...
            bool genericAddItem(MP4File& file, const MP4ItmfItem* item)
            {
                ...
                MP4ItemAtom& itemAtom =
                    *(MP4ItemAtom*)MP4Atom::CreateAtom(file, ilst, item->code);
            ...
            bool genericSetItem(MP4File& file, const MP4ItmfItem* item)
            {
                ...
                MP4ItemAtom* const old =
                    static_cast<MP4ItemAtom*>(item->__handle);
                const uint32_t childCount = ilst->GetNumberOfChildAtoms();
