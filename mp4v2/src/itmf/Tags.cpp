...
namespace mp4v2
{
    namespace impl
    {
        namespace itmf
        {
            ...
            void Tags::c_addArtwork(MP4Tags*& tags, MP4TagArtwork& c_artwork)
            {
            ...
            void Tags::c_alloc(MP4Tags*& tags)
            {
                tags = new MP4Tags();
            ...
            void Tags::c_fetch(MP4Tags*& tags, MP4FileHandle hFile)
            {
                MP4Tags& c = *tags;
                 ...
                 * map will only be used for items which do not repeat; we do
                 * not care if cover-art is inserted multiple times.
                 */
                CodeItemMap cim;
                ...
                fetchInteger(cim, CODE_CONTENTRATING, contentRating,
                             c.contentRating);
                fetchInteger(cim, CODE_GAPLESS, gapless, c.gapless);

                fetchString(cim, CODE_ITUNESACCOUNT, iTunesAccount,
                            c.iTunesAccount);
                fetchInteger(cim, CODE_ITUNESACCOUNTTYPE, iTunesAccountType,
                             c.iTunesAccountType);
                fetchInteger(cim, CODE_ITUNESCOUNTRY, iTunesCountry,
                             c.iTunesCountry);
            ...
            void Tags::c_removeArtwork(MP4Tags*& tags, uint32_t index)
            {
            ...
            void Tags::c_setArtwork(MP4Tags*& tags, uint32_t index,
                                    MP4TagArtwork& c_artwork)
            {
                if (!(index < artwork.size()))
                    return;
                ...
                default:
                    item.type =
                        computeBasicType(c_artwork.data, c_artwork.size);
                    break;
                }
            ...
            void Tags::c_setInteger(const uint8_t* value, uint8_t& cpp,
                                    const uint8_t*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setInteger(const uint16_t* value, uint16_t& cpp,
                                    const uint16_t*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setInteger(const uint32_t* value, uint32_t& cpp,
                                    const uint32_t*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setInteger(const uint64_t* value, uint64_t& cpp,
                                    const uint64_t*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setString(const char* value, string& cpp,
                                   const char*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setTrack(const MP4TagTrack* value, MP4TagTrack& cpp,
                                  const MP4TagTrack*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_setDisk(const MP4TagDisk* value, MP4TagDisk& cpp,
                                 const MP4TagDisk*& c)
            {
                if (!value)
                {
            ...
            void Tags::c_store(MP4Tags*& tags, MP4FileHandle hFile)
            {
                MP4Tags& c = *tags;
                MP4File& file = *static_cast<MP4File*>(hFile);
                ...
                storeInteger(file, CODE_COMPILATION, compilation,
                             c.compilation);
                ...
                storeInteger(file, CODE_CONTENTRATING, contentRating,
                             c.contentRating);
                storeInteger(file, CODE_GAPLESS, gapless, c.gapless);

                storeString(file, CODE_ITUNESACCOUNT, iTunesAccount,
                            c.iTunesAccount);
                storeInteger(file, CODE_ITUNESACCOUNTTYPE, iTunesAccountType,
                             c.iTunesAccountType);
                storeInteger(file, CODE_ITUNESCOUNTRY, iTunesCountry,
                             c.iTunesCountry);
            ...
            void Tags::fetchGenre(const CodeItemMap& cim, uint16_t& cpp,
                                  const uint16_t*& c)
            {
                cpp = 0;
                c = NULL;
                ...
                cpp =
                    (uint16_t(data.value[0]) << 8) | (uint16_t(data.value[1]));
            ...
            void Tags::fetchDisk(const CodeItemMap& cim, MP4TagDisk& cpp,
                                 const MP4TagDisk*& c)
            {
                cpp.index = 0;
                cpp.total = 0;
                ...
                cpp.index =
                    (uint16_t(data.value[2]) << 8) | (uint16_t(data.value[3]));

                cpp.total =
                    (uint16_t(data.value[4]) << 8) | (uint16_t(data.value[5]));
            ...
            void Tags::fetchTrack(const CodeItemMap& cim, MP4TagTrack& cpp,
                                  const MP4TagTrack*& c)
            {
                cpp.index = 0;
                cpp.total = 0;
            ...
            void Tags::fetchInteger(const CodeItemMap& cim, const string& code,
                                    uint8_t& cpp, const uint8_t*& c)
            {
                cpp = 0;
                c = NULL;
            ...
            void Tags::fetchString(const CodeItemMap& cim, const string& code,
                                   string& cpp, const char*& c)
            {
                cpp.clear();
                c = NULL;
            ...
            void Tags::remove(MP4File& file, const string& code)
            {
                MP4ItmfItemList* itemList =
                    genericGetItemsByCode(file, code); // alloc
            ...
            void Tags::store(MP4File& file, const string& code,
                             MP4ItmfBasicType basicType, const void* buffer,
                             uint32_t size)
            {
            ...
            void Tags::storeGenre(MP4File& file, uint16_t cpp,
                                  const uint16_t* c)
            {
                if (c)
                {
                    ...
                    // it's not clear if you must use implicit in these
                    // situations and iirc iTunes and other software are not
                    // consistent in this regard. many other tags must be
                    // integer type yet no issues there. Silly that iTunes
                    // insists it must be implict, which is then hardcoded to
                    // interpret as genres anyways.
                    store(file, CODE_GENRETYPE, MP4_ITMF_BT_IMPLICIT, buf,
                          sizeof(buf));
                }
                else
                {
            ...
            void Tags::storeDisk(MP4File& file, const MP4TagDisk& cpp,
                                 const MP4TagDisk* c)
            {
                if (c)
                {
                    ...
                    store(file, CODE_DISK, MP4_ITMF_BT_IMPLICIT, buf,
                          sizeof(buf));
                }
                else
                {
            ...
            void Tags::storeTrack(MP4File& file, const MP4TagTrack& cpp,
                                  const MP4TagTrack* c)
            {
                if (c)
                {
                    ...
                    store(file, CODE_TRACK, MP4_ITMF_BT_IMPLICIT, buf,
                          sizeof(buf));
                }
                else
                {
            ...
            void Tags::storeInteger(MP4File& file, const string& code,
                                    uint8_t cpp, const uint8_t* c)
            {
                if (c)
            ...
            void Tags::storeString(MP4File& file, const string& code,
                                   const string& cpp, const char* c)
            {
                if (c)
                    store(file, code, MP4_ITMF_BT_UTF8, cpp.c_str(),
                          (uint32_t)cpp.size());
                else
                    remove(file, code);
            }
            ...
            void Tags::updateArtworkShadow(MP4Tags*& tags)
            {
                if (tags->artwork)
                {
                    delete[] tags->artwork;
                ...
                MP4TagArtwork* const cartwork =
                    new MP4TagArtwork[artwork.size()];
            ...
            const string Tags::CODE_NAME = "\xa9"
                                           "nam";
            const string Tags::CODE_ARTIST = "\xa9"
                                             "ART";
            const string Tags::CODE_ALBUMARTIST = "aART";
            const string Tags::CODE_ALBUM = "\xa9"
                                            "alb";
            const string Tags::CODE_GROUPING = "\xa9"
                                               "grp";
            const string Tags::CODE_COMPOSER = "\xa9"
                                               "wrt";
            const string Tags::CODE_COMMENTS = "\xa9"
                                               "cmt";
            const string Tags::CODE_GENRE = "\xa9"
                                            "gen";
            const string Tags::CODE_GENRETYPE = "gnre";
            const string Tags::CODE_RELEASEDATE = "\xa9"
                                                  "day";
            ...
            const string Tags::CODE_LYRICS = "\xa9"
                                             "lyr";
            ...
            const string Tags::CODE_ENCODINGTOOL = "\xa9"
                                                   "too";
            const string Tags::CODE_ENCODEDBY = "\xa9"
                                                "enc";
