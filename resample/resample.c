...
//
// Taken from resample_open(); this
// call sets the internal param values to the state of the resample_open()
// method. Must only be called after resample_open().
//
void resample_reset(void* handle)
{
    if (handle)
    {
        int i;
...
{
    //
    //  We only ever use highQuality and other params are fixed at compile time,
    //  so Imp64 can be static, hence calculated only once.
    //
    static double* staticImp64 = 0;
    double* Imp64;
    double Rolloff, Beta;
    rsdata* hp;
    ...
    {
        fprintf(stderr,
                "WARNING: audio resampler only supports high quality.\n");
    }
    ...
    hp->LpScl = 1.0;
    hp->Nwing =
        Npc * (hp->Nmult - 1) / 2; /* # of filter coeffs in right wing */
    ...
    if (0 == staticImp64)
    {
        staticImp64 = (double*)malloc(hp->Nwing * sizeof(double));
        memset(staticImp64, 0, sizeof(double) * hp->Nwing);

        lrsLpFilter(staticImp64, hp->Nwing, 0.5 * Rolloff, Beta, Npc);
    }
    Imp64 = staticImp64;
    ...
    *inBufferUsed = 0;
    ...
    for (;;)
    {
        ...
        printf("XSize: %d Xoff: %d Xread: %d Xp: %d lastFlag: %d\n", hp->XSize,
               hp->Xoff, hp->Xread, hp->Xp, lastFlag);
#endif
        ...
        *inBufferUsed += len;
        ...
        { /* SrcUp() is faster if we can use it */
            Nout = lrsSrcUp(hp->X, hp->Y, factor, &hp->Time, Nx, Nwing, LpScl,
                            Imp, ImpD, interpFilt);
        }
        else
        {
            Nout = lrsSrcUD(hp->X, hp->Y, factor, &hp->Time, Nx, Nwing, LpScl,
                            Imp, ImpD, interpFilt);
        }
